...
const IndexAdjustedRents = ({ attributes, indexAdjustedRents }: Props) => {
  const getColumns = () => {
    ...
    const intendedUseOptions = getFieldOptions(
      attributes,
      LeaseIndexAdjustedRentsFieldPaths.INTENDED_USE,
    );

    if (
      isFieldAllowedToRead(attributes, LeaseIndexAdjustedRentsFieldPaths.AMOUNT)
    ) {
      columns.push({
        key: "amount",
    ...
    if (
      ...
      columns.push({
        ...
        ascSortFunction: (a, b, key) =>
          sortByOptionsAsc(a, b, key, intendedUseOptions),
        descSortFunction: (a, b, key) =>
          sortByOptionsDesc(a, b, key, intendedUseOptions),
      });
    }
    ...
    if (
      isFieldAllowedToRead(
        attributes,
        LeaseIndexAdjustedRentsFieldPaths.START_DATE,
      )
    ) {
      columns.push({
        key: "start_date",
    ...
    if (
      isFieldAllowedToRead(
        attributes,
        LeaseIndexAdjustedRentsFieldPaths.END_DATE,
      )
    ) {
      columns.push({
        key: "end_date",
    ...
    if (
      isFieldAllowedToRead(attributes, LeaseIndexAdjustedRentsFieldPaths.FACTOR)
    ) {
      columns.push({
        key: "factor",
