...
const PayableRents = ({ leaseAttributes, payableRents }: Props) => {
  const getColumns = () => {
    const columns = [];

    if (
      isFieldAllowedToRead(leaseAttributes, LeasePayableRentsFieldPaths.AMOUNT)
    ) {
      columns.push({
        key: "amount",
    ...
    if (
      isFieldAllowedToRead(
        leaseAttributes,
        LeasePayableRentsFieldPaths.START_DATE,
      )
    ) {
      columns.push({
        key: "start_date",
    ...
    if (
      isFieldAllowedToRead(
        leaseAttributes,
        LeasePayableRentsFieldPaths.END_DATE,
      )
    ) {
      columns.push({
        key: "end_date",
    ...
    if (
      isFieldAllowedToRead(
        leaseAttributes,
        LeasePayableRentsFieldPaths.DIFFERENCE_PERCENT,
      )
    ) {
      columns.push({
    ...
    if (
      isFieldAllowedToRead(
        leaseAttributes,
        LeasePayableRentsFieldPaths.CALENDAR_YEAR_RENT,
      )
    ) {
      columns.push({
