...
import { AreaNoteFieldTitles } from "@/areaNote/enums";
...
import { getContentOverdueInvoices } from "@/invoices/helpers";
import {
...
const StatusIndicator = ({
  ...
  <p
    className={classNames(
      {
        "summary__status-indicator summary__status-indicator--neutral":
          !researchState || researchState === ConstructabilityStatus.UNVERIFIED,
      },
      {
        "summary__status-indicator summary__status-indicator--alert":
          researchState === ConstructabilityStatus.REQUIRES_MEASURES,
      },
      {
        "summary__status-indicator summary__status-indicator--success":
          researchState === ConstructabilityStatus.COMPLETE,
      },
      {
        "summary__status-indicator summary__status-indicator--enquiry-sent":
          researchState === ConstructabilityStatus.ENQUIRY_SENT,
      },
    )}
  >
    {getLabelOfOption(
      stateOptions,
      researchState || ConstructabilityStatus.UNVERIFIED,
    )}
  </p>
);
...
const Tenants = ({ largeScreen, tenants, tenantWarnings }: TenantsProps) => {
  if (largeScreen) {
    return (
      <Fragment>
        {!!tenants.length && (
          <Fragment>
            ...
            <ListItems>
              {tenants.map((contact, index) => {
                return (
                  <Row key={index}>
                    ...
                    <Column large={4}>
                      <ListItem>
                        {contact.share_numerator} / {contact.share_denominator}
                      </ListItem>
                    </Column>
                    <Column large={4}>
                      <ListItem>
                        {formatDateRange(
                          contact.tenant.start_date,
                          contact.tenant.end_date,
                        ) || "-"}
                      </ListItem>
                    </Column>
                  </Row>
              ...
              {!!tenantWarnings.length &&
                tenantWarnings.map((error, index) => (
                  <Row key={index}>
...
const SummaryLeaseInfo = ({
  ...
  const constructabilityStateOptions = getFieldOptions(
      attributes,
                  ...
                  <ListItems key={index}>
                    <Row>
                      ...
                      >
                        <Column small={6} large={8}>
                          {!area.addresses ||
                            (!area.addresses.length && <ListItem>-</ListItem>)}
      ...
      >
        <>
          ...
          {!constructabilityAreas.length && (
            <FormText>Ei vuokrakohteita</FormText>
          )}
          {!!constructabilityAreas.length &&
            constructabilityAreas.map((area, index) => {
              const descriptionsDemolition = area.descriptionsDemolition.filter(
                (item) => item.is_static,
              );
              const descriptionsOther = area.descriptionsOther.filter(
                (item) => item.is_static,
              );
              const descriptionsPollutedLand =
                area.descriptionsPollutedLand.filter((item) => item.is_static);
              const descriptionsPreconstruction =
                area.descriptionsPreconstruction.filter(
                  (item) => item.is_static,
                );
              const descriptionsReport = area.descriptionsReport.filter(
                (item) => item.is_static,
              );
              return (
                <ListItems key={index}>
                  ...
                  >
                    <Row>
                      <Column small={6} large={4}>
                        <ListItem>
                          Esirakentaminen, johtosiirrot, kunnallistekniikka
                        </ListItem>
                      </Column>
                      <Column small={6} large={4}>
                        <StatusIndicator
              ...
              {summary.area_notes.map((areaNote, index) => {
                return (
                  <Row key={index}>
                    <Column small={4}>
                      <ListItem>
                        {formatDate(areaNote.modified_at) || "-"}
                      </ListItem>
                    </Column>
                    <Column small={4}>
                      <ListItem>
                        {getUserFullName(areaNote.user) || "-"}
                      </ListItem>
                    </Column>
                    <Column
                      small={4}
                    ...
                    return (
                      <Row key={index}>
                        <Column large={4}>
                          ...
                          >
                            <ListItem>
                              {getContactFullName(invoice.recipientFull) || "-"}
                            </ListItem>
                          </Authorization>
                        </Column>
                        <Column large={2}>
