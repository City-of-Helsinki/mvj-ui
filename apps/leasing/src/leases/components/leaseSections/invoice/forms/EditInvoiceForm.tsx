                ...
                const handleRemove = () => {
                  appDispatch({
                    ...
                    confirmationModalButtonText:
                      ConfirmationModalTexts.DELETE_INVOICE_PAYMENT.BUTTON,
                    confirmationModalLabel:
                      ConfirmationModalTexts.DELETE_INVOICE_PAYMENT.LABEL,
                    confirmationModalTitle:
                      ConfirmationModalTexts.DELETE_INVOICE_PAYMENT.TITLE,
                  });
                };
...
const EditInvoiceForm: React.FC<Props> = ({
  ...
  const isEditable = (fieldPath: string): boolean => {
    // Invoice can't be edited if sent to SAP
    if (!invoice || isSentToSap) return false;
    // Invoice notes can be edited if it is a credit note and not sent to sap.
    if (isCreditNote) return fieldPath === InvoiceFieldPaths.NOTES;
    // Payments can be edited if the invoice is autogenerated and not sent to sap.
    if (isGenerated) return fieldPath === InvoicePaymentsFieldPaths.PAYMENTS;
    // All fields can be edited on a manual invoice.
    return true;
  };
  ...
  const deliveryMethodOptions = getFieldOptions(
    invoiceAttributes,
            ...
            <>
              ...
              <Row>
                <Column small={12} medium={8}>
                  ...
                  >
                    {isEditable(InvoicePaymentsFieldPaths.PAYMENTS) ? (
