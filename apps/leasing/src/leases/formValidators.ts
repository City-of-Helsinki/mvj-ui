 ...
 * @returns {Object}
 */
export const validateSummaryForm = (
 ...
 * @returns {Object[]}
 */
const getFixedInitialYearRentsErrors = (
 ...
 * @returns {Object[]}
 */
const getRentAdjustmentsErrors = (
...
const getRentErrors = (rent: Record<string, any>): Record<string, any> => {
  ...
  const equalizationEndDateError = dateGreaterOrEqual(
  ...
  const fixedInitialYearRentErrors = getFixedInitialYearRentsErrors(
    rent,
  ...
  const contractRentErrors = getContractRentsErrors(
  ...
  const rentAdjustmentErrors = getRentAdjustmentsErrors(
 ...
 * @returns {Object}
 */
const getContractErrors = (
  contract: Record<string, any>,
): Record<string, any> | null | undefined => {
  const errors: any = {};
  const collateralErrors = getContractCollateralErrors(
    get(contract, "collaterals", []),
  );
...
export const validateInvoiceForm = (
  ...
  const startDateError = billingPeriodRequired
    ? required(values.billing_period_start_date)
    : undefined;
  const endDateError =
    (billingPeriodRequired
      ? required(values.billing_period_end_date)
      : undefined) ||
    dateGreaterOrEqual(
      values.billing_period_end_date,
      values.billing_period_start_date,
    );
 ...
 * @returns {Object}
 */
export const validateSteppedDiscountForm = (
  values: Record<string, any>,
): Record<string, any> => {
  const errors: any = {};
  const endDateError = dateGreaterOrEqual(values.end_date, values.start_date);
