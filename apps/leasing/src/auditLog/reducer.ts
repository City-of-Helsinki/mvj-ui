...
const isFetchingByContactReducer: Reducer<AuditLogIsFetchingMap> =
  handleActions(
    {
      ["mvj/auditLog/FETCH_BY_CONTACT"]: (
        state: AuditLogIsFetchingMap,
        { payload: contactId }: FetchAuditLogByContactAction,
      ) => ({ ...state, [contactId]: true }),
      ["mvj/auditLog/RECEIVE_BY_CONTACT"]: (
        state: AuditLogIsFetchingMap,
        { payload }: ReceiveAuditLogByContactAction,
      ) => ({
        ...state,
      ...
      ["mvj/auditLog/NOT_FOUND_BY_CONTACT"]: (
        state: AuditLogIsFetchingMap,
        { payload: contactId }: NotFoundByContactAction,
...
const isFetchingByLeaseReducer: Reducer<AuditLogIsFetchingMap> = handleActions(
  {
    ["mvj/auditLog/FETCH_BY_LEASE"]: (
      state: AuditLogIsFetchingMap,
      { payload }: FetchAuditLogByLeaseAction,
    ) => ({ ...state, [payload.id]: true }),
    ["mvj/auditLog/RECEIVE_BY_LEASE"]: (
      state: AuditLogIsFetchingMap,
      { payload }: ReceiveAuditLogByLeaseAction,
    ) => ({
      ...state,
      ...Object.keys(payload).reduce(
        (obj, key) => ({ ...obj, [key]: false }),
    ...
    ["mvj/auditLog/NOT_FOUND_BY_LEASE"]: (
      state: AuditLogIsFetchingMap,
      { payload: leaseId }: NotFoundByLeaseAction,
    ) => ({ ...state, [leaseId]: false }),
  },
  {},
);
...
const isFetchingByAreaSearchReducer: Reducer<AuditLogIsFetchingMap> =
  handleActions(
    {
      ...
      ["mvj/auditLog/RECEIVE_BY_AREASEARCH"]: (
        state: AuditLogIsFetchingMap,
        { payload }: ReceiveAuditLogByAreaSearchAction,
      ) => ({
        ...state,
        ...Object.keys(payload).reduce(
          (obj, key) => ({ ...obj, [key]: false }),
      ...
      ["mvj/auditLog/NOT_FOUND_BY_AREASEARCH"]: (
        state: AuditLogIsFetchingMap,
        { payload: areaSearchId }: NotFoundByAreaSearchAction,
      ) => ({ ...state, [areaSearchId]: false }),
    },
    {},
  );
