...
const jobRunAttributesReducer: Reducer<Attributes> = handleActions(
  {
    ["mvj/batchrun/RECEIVE_JOB_RUN_ATTRIBUTES"]: (
      state: Attributes,
      { payload: attributes }: ReceiveJobRunAttributesAction,
    ) => {
      return attributes;
    },
  },
...
const jobRunMethodsReducer: Reducer<Methods> = handleActions(
  {
    ["mvj/batchrun/RECEIVE_JOB_RUN_METHODS"]: (
      state: Methods,
      { payload: methods }: ReceiveJobRunMethodsAction,
    ) => {
      return methods;
    },
  },
...
const jobRunsReducer: Reducer<any> = handleActions(
  {
    ["mvj/batchrun/RECEIVE_JOB_RUNS"]: (
      state: JobRuns,
      { payload: jobRuns }: ReceiveJobRunsAction,
    ) => {
      return jobRuns;
    },
  },
...
const isFetchingJobRunLogEntryAttributesReducer: Reducer<boolean> =
  handleActions(
    {
...
const isFetchingJobRunLogEntriesByRunReducer: Reducer<Record<string, any>> =
  handleActions(
    {
      ["mvj/batchrun/FETCH_JOB_RUN_LOG_ENTRIES_BY_ID"]: (
        state: Record<string, any>,
        { payload: id }: FetchJobRunLogEntriesByRunAction,
      ) => {
        return { ...state, [id]: true };
      },
      ["mvj/batchrun/RECEIVE_JOB_RUN_LOG_ENTRIES_BY_ID"]: (
        state: Record<string, any>,
        { payload }: ReceiveJobRunLogEntriesByRunAction,
      ) => {
        return { ...state, [payload.run]: false };
      },
      ["mvj/batchrun/NOT_FOUND_JOB_RUN_LOG_ENTRIES_BY_ID"]: (
        state: Record<string, any>,
        { payload: id }: NotFoundJobRunLogEntriesByRunAction,
      ) => {
...
const jobRunLogEntryAttributesReducer: Reducer<Attributes> = handleActions(
  {
    ["mvj/batchrun/RECEIVE_JOB_RUN_LOG_ENTRY_ATTRIBUTES"]: (
      state: Attributes,
      { payload: attributes }: ReceiveJobRunLogEntryAttributesAction,
    ) => {
      return attributes;
    },
  },
...
const jobRunLogEntryMethodsReducer: Reducer<Methods> = handleActions(
  {
    ["mvj/batchrun/RECEIVE_JOB_RUN_LOG_ENTRY_METHODS"]: (
      state: Methods,
      { payload: methods }: ReceiveJobRunLogEntryMethodsAction,
    ) => {
      return methods;
    },
  },
...
const jobRunLogEntriesbyRunReducer: Reducer<Record<string, any>> =
  handleActions(
    {
      ["mvj/batchrun/RECEIVE_JOB_RUN_LOG_ENTRIES_BY_ID"]: (
        state: Record<string, any>,
        ...
        return { ...state, [payload.run]: payload.data };
      },
    },
    {},
  );
...
const scheduledJobAttributesReducer: Reducer<Attributes> = handleActions(
  {
    ["mvj/batchrun/RECEIVE_SCHEDULED_JOB_ATTRIBUTES"]: (
      state: Attributes,
      { payload: attributes }: ReceiveScheduledJobAttributesAction,
    ) => {
      return attributes;
    },
  },
...
const scheduledJobMethodsReducer: Reducer<Methods> = handleActions(
  {
    ["mvj/batchrun/RECEIVE_SCHEDULED_JOB_METHODS"]: (
      state: Methods,
      { payload: methods }: ReceiveScheduledJobMethodsAction,
    ) => {
      return methods;
    },
  },
...
const scheduledJobsReducer: Reducer<any> = handleActions(
  {
    ["mvj/batchrun/RECEIVE_SCHEDULED_JOBS"]: (
      state: ScheduledJobs,
      { payload: scheduledJobs }: ReceiveScheduledJobsAction,
    ) => {
