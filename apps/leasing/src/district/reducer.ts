...
const byMunicipalityReducer: Reducer<DistrictListMap> = handleActions(
  {
    ["mvj/district/RECEIVE_BY_MUNICIPALITY"]: (
      state: DistrictListMap,
      { payload: list }: ReceiveDistrictsByMunicipalityAction,
    ) => {
      return { ...state, [list.municipality]: list.districts };
    },
  },
  {},
);
