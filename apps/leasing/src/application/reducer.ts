...
const methodsReducer: Reducer<Methods> = handleActions(
  {
    ["mvj/application/RECEIVE_METHODS"]: (
      state: Methods,
...
const applicantInfoCheckAttributesReducer: Reducer<Attributes> = handleActions(
  {
    ["mvj/application/RECEIVE_APPLICANT_INFO_CHECK_ATTRIBUTES"]: (
      state: Attributes,
      { payload: attributes }: ReceiveApplicantInfoCheckAttributesAction,
    ) => attributes,
    ["mvj/application/APPLICANT_INFO_CHECK_ATTRIBUTES_NOT_FOUND"]: () => null,
  },
  null,
);
const isFetchingApplicantInfoCheckAttributesReducer: Reducer<boolean> =
  handleActions(
    {
      ["mvj/application/FETCH_APPLICANT_INFO_CHECK_ATTRIBUTES"]: () => true,
      ["mvj/application/RECEIVE_APPLICANT_INFO_CHECK_ATTRIBUTES"]: () => false,
      ["mvj/application/APPLICANT_INFO_CHECK_ATTRIBUTES_NOT_FOUND"]: () =>
        false,
    },
    false,
  );
...
export const formAttributesReducer: Reducer<Attributes> = handleActions(
  {
    ["mvj/application/RECEIVE_FORM_ATTRIBUTES"]: (
      state: Attributes,
      { payload: attributes }: ReceiveFormAttributesAction,
    ) => {
      return attributes;
    },
  },
...
export const fieldTypeMappingReducer: Reducer<Record<number, string>> =
  handleActions(
    {
      ["mvj/application/RECEIVE_FORM_ATTRIBUTES"]: (
        ...
        return (
          attributes?.sections?.child?.children.fields?.child?.children.type?.choices?.reduce(
            (acc, choice) => {
...
const attachmentAttributesReducer: Reducer<Attributes> = handleActions(
  {
    ["mvj/application/RECEIVE_ATTACHMENT_ATTRIBUTES"]: (
      state: Attributes,
      { payload: attributes }: ReceiveAttachmentAttributesAction,
    ) => attributes,
...
const attachmentMethodsReducer: Reducer<Methods> = handleActions(
  {
    ["mvj/application/RECEIVE_ATTACHMENT_METHODS"]: (
      state: Methods,
      { payload: methods }: ReceiveAttachmentMethodsAction,
    ) => methods,
    ["mvj/application/ATTACHMENT_ATTRIBUTES_NOT_FOUND"]: () => null,
  },
  null,
...
const applicationAttachmentsReducer: Reducer<Record<string, any>> =
  handleActions(
    {
      ["mvj/application/FETCH_ATTACHMENTS"]: () => null,
      ["mvj/application/RECEIVE_ATTACHMENTS"]: (
        state: Record<string, any>,
        { payload: attachments }: ReceiveApplicationRelatedAttachmentsAction,
      ) => attachments,
    },
    null,
  );
...
export const pendingUploadsReducer: Reducer<Array<Record<string, any>>> =
  handleActions(
    {
      ["mvj/application/FETCH_PENDING_UPLOADS"]: () => [],
      ["mvj/application/RECEIVE_PENDING_UPLOADS"]: (state, { payload }) =>
        payload,
    },
    [],
  );
...
export default combineReducers<Record<string, any>, Action<any>>({
  ...
  isFetchingApplicantInfoCheckAttributes:
    isFetchingApplicantInfoCheckAttributesReducer,
