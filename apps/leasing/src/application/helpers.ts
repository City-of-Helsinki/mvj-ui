...
export const transformTargetSectionTitle =
  ...
  (
    ...
    if (
      ...
      const target = plotSearch?.plot_search_targets.find(
        (target) => target.id === answer.metadata?.identifier,
      );
...
export const transformApplicantSectionTitle = (
  title: string,
  section: FormSection,
  answer: SavedApplicationFormSection,
): string => {
  if (section.identifier === APPLICANT_SECTION_IDENTIFIER) {
    if (answer?.metadata?.identifier) {
      if (answer?.metadata?.applicantType) {
        ...
        const sectionsWithIdentifier =
          answer.sections[identifiers?.DATA_SECTION];
...
export const getApplicantInfoCheckItems = (
  existingData: Array<Record<string, any>>,
): Array<Record<string, any>> => {
  return APPLICATION_INFO_CHECK_DEFINITIONS.map((item) => {
    const existingItem = existingData.find(
      (existingItem) => existingItem.name === item.type,
    );
...
export const prepareApplicantInfoCheckForSubmission = (
...
export const valueToApplicantType = (
  value: PlotApplicationFormValue,
): string | null => {
  if (value === "1") {
...
export const getInitialApplicationForm = (
  ...
  const buildSectionItem = (section, parent, sectionAnswer?: any) => {
    ...
    section.subsections.forEach((subsection) =>
      buildSection(
        subsection,
        workingItem.sections,
        sectionAnswer?.sections?.[subsection.identifier],
      ),
    );
    section.fields.forEach((field) =>
      buildField(
        field,
        workingItem.fields,
        sectionAnswer?.fields?.[field.identifier],
      ),
    );
  ...
  const buildSection = (section, parent, sectionAnswers): void => {
    ...
    if (section.add_new_allowed && !root.sectionTemplates[section.identifier]) {
      root.sectionTemplates[section.identifier] = buildSectionItem(
        section,
        parent,
      );
    }
    ...
    if (sectionAnswers) {
      if (section.add_new_allowed) {
        ...
        parent[section.identifier] = sectionAnswers.map((sectionAnswer) =>
          buildSectionItem(section, parent, sectionAnswer),
        );
      } else {
        parent[section.identifier] = buildSectionItem(
          section,
          parent,
          sectionAnswers,
        );
    ...
    switch (fieldTypes[field.type]) {
      case "uploadfiles":
        ...
        if (reformattedAnswer) {
          root.attachments.push(
            ...reformattedAnswer.value.map((file) => file.id),
          );
        }
  ...
  form.sections.forEach((section) =>
    buildSection(
      section,
      root.sections,
      existingValues?.sections?.[section.identifier],
    ),
  );
  return root;
};
export const getApplicationAttachmentDownloadLink = (id: number): string =>
  createUrl(`attachment/${id}/download`);
export const getAreaSearchApplicationAttachmentDownloadLink = (
  id: number,
): string => createUrl(`area_search_attachment/${id}/download`);
export const getSectionTemplate = (
  ...
  const templates = formValueSelector(formName)(
    state,
    (formPath ? `${formPath}.` : "") + "sectionTemplates",
  );
            ...
            section.forEach((applicant) => {
              ...
              applicant.sections = _.pickBy(
                ...
                (subsection, sectionIdentifier: string) =>
                  [
                    ApplicantTypes.BOTH,
                    applicant.metadata.applicantType,
              ...
              try {
                ...
                const identifier =
                  sectionWithIdentifier?.fields[
                    identifiers?.IDENTIFIER_FIELD
                  ] ||
                  sectionWithIdentifier[0]?.fields[
                    identifiers?.IDENTIFIER_FIELD
                  ];
                ...
                if (!identifier?.value) {
                  ...
                  throw new Error(
                    `no identifier of type ${applicant.metadata?.applicantType || "unknown"} found for applicant section`,
                  );
                }
