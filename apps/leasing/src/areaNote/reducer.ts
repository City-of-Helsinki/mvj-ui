...
const initialValuesReducer: Reducer<Record<string, any>> = handleActions(
  {
    ["mvj/areaNote/INITIALIZE"]: (
      state: Record<string, any>,
      { payload: values }: InitializeAction,
    ) => {
      return values;
    },
  },
...
const attributesReducer: Reducer<Attributes> = handleActions(
  {
    ["mvj/areaNote/RECEIVE_ATTRIBUTES"]: (
      state: Attributes,
...
const areaNoteListReducer: Reducer<any> = handleActions(
  {
    ["mvj/areaNote/RECEIVE_ALL"]: (
      state: AreaNoteList,
      { payload: list }: ReceiveAreaNoteListAction,
    ) => {
      return list;
    },
    ["mvj/areaNote/RECEIVE_DELETED"]: (
      state: any,
      { payload: areaNoteId }: ReceiveDeletedAreaNoteAction,
    ) => {
      const newList = [...state];
      return newList.filter((area) => area.id !== areaNoteId);
    },
    ["mvj/areaNote/RECEIVE_EDITED"]: (
      state: AreaNoteList,
      { payload: areaNote }: ReceiveEditedAreaNoteAction,
    ) => {
      const newList = [...state];
