...
export const createDirectReservationLinkRequest = (
  payload: Record<string, any>,
): Generator<any, any, any> => {
  const body = JSON.stringify(payload);
  return callApi(
    new Request(createUrl("direct_reservation_link/"), {
      method: "POST",
      body,
    }),
  );
};
export const fetchPlotSearches = (
...
export const editPlotSearch = (
  PlotSearch: PlotSearch,
): Generator<any, any, any> => {
  const { id } = PlotSearch;
  const body = JSON.stringify(PlotSearch);
  return callApi(
    new Request(createUrl(`plot_search/${id}/`), {
      method: "PATCH",
      body,
    }),
  );
...
export const fetchPlanUnitAttributes = (): Generator<any, any, any> => {
  return callApi(
    new Request(createUrl(`plan_unit/1/`), {
      method: "OPTIONS",
    }),
  );
};
...
export const fetchCustomDetailedPlanAttributes = (): Generator<
  any,
  any,
  any
> => {
  return callApi(
    new Request(createUrl(`custom_detailed_plan/1/`), {
      method: "OPTIONS",
    }),
  );
};
...
export const editFormRequest = (
  form: Record<string, any>,
): Generator<any, any, any> => {
  return callApi(
    new Request(createUrl(`form/${form.id}/`), {
      method: "PATCH",
      body: JSON.stringify(form),
    }),
  );
};
export const fetchStagesRequest = (
  params?: Record<string, any> | null | undefined,
): Generator<any, any, any> => {
  return callApi(new Request(createUrl("plot_search_stage/", params)));
};
export const fetchPlotSearchApplicationsRequest = (
  id: number,
): Generator<any, any, any> => {
  return callApi(
    new Request(
...
export const editTargetPlotSearchRelationRequest = (
  payload: Record<string, any>,
): Generator<any, any, any> => {
  return callApi(
    new Request(createUrl(`plot_search_target/${payload.id}/`), {
      method: "PATCH",
      body: JSON.stringify({
...
export const createPlotSearchApplicationsOpeningRecords = (
  id: number,
  data: Record<string, any>,
): Generator<any, any, any> => {
  return callApi(
    new Request(createUrl(`plot_search/${id}/open_answers/`), {
      method: "POST",
      body: JSON.stringify({
        note: data.note,
