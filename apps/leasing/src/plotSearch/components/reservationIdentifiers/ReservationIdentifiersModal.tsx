...
import { flowRight, padStart } from "lodash-es";
...
import {
  ...
  getIsBatchCreatingReservationIdentifiers,
  getLastBatchReservationCreationError,
  getReservationIdentifierUnitLists,
...
class ReservationIdentifiersModal extends Component<Props, State> {
  ...
  componentDidMount(): void {
    const { fetchReservationIdentifierUnitLists } = this.props;
    fetchReservationIdentifierUnitLists();
  }
  ...
  componentDidUpdate(prevProps: Props): void {
    ...
    if (
      reservationIdentifierUnitLists &&
      !prevProps.reservationIdentifierUnitLists
    ) {
      this.setState((state) => ({
        rowData: state.rowData.map((row) => ({
          ...row,
          suggestedLeaseIdentifier: this.convertLeaseOptionsToIdentifier(
            row.leaseOptions,
          ),
        })),
      }));
  ...
  initializeRowData = () => {
    const { currentPlotSearch, initialize } = this.props;
    const data = currentPlotSearch.plot_search_targets.reduce((acc, target) => {
      ...
      return [
        ...acc,
        {
          ...
          suggestedLeaseIdentifier:
            this.convertLeaseOptionsToIdentifier(leaseOptions),
          reservationIdentifier: target.reservation_readable_identifier,
          intendedUse: (
            target.plan_unit?.intended_use ||
            target.custom_detailed_plan?.intended_use
          )?.name,
          applicants: target.reservation_recipients
            .map(
              (group) =>
                `${group.reservation_recipients.join(", ")} (${group.share_of_rental})`,
            )
            .join("; "),
          leaseOptions,
    ...
    if (this.state.editorTarget !== null) {
      ...
      this.setState((state) => {
        ...
        newRowData.splice(index, 1, {
          ...state.rowData[index],
          suggestedLeaseIdentifier:
            this.convertLeaseOptionsToIdentifier(leaseData),
  ...
  handleSubmit = () => {
    ...
    batchCreateReservationIdentifiers({
      data: rowData
        .filter((row) => rowsToCreate[row.index].selected)
        .map((row) => ({
          ...row.leaseOptions,
  ...
  convertLeaseOptionsToIdentifier = (options: Record<string, any>) => {
    const { reservationIdentifierUnitLists } = this.props;

    if (!reservationIdentifierUnitLists) {
      return "...";
    }

    const type = reservationIdentifierUnitLists.types.find(
      (candidate) => candidate.id === options.type,
    );
    const municipality = reservationIdentifierUnitLists.municipalities.find(
      (candidate) => candidate.id === options.municipality,
    );
    const district = reservationIdentifierUnitLists.districts.find(
      (candidate) => candidate.id === options.district,
    );
  ...
  render(): JSX.Element {
    ...
    const { editorTarget, rowData } = this.state;
    const selectedRowsCount =
      rowsToCreate?.filter((row) => row.selected).length || 0;
    ...
    const currentModalTitle =
      editorTarget !== null ? "Muokkaa varaustunnusta" : "Varaustunnukset";
    return (
      <Modal
        isOpen={isOpen}
        onClose={() => {
        ...
        className={classNames("ReservationIdentifiersModal", {
        ...
        {editorTarget === null ? (
          <>
                    ...
                    <>
                      {!row.disabled && (
                        <FormFieldLegacy
                          ...
                          invisibleLabel
                          disabled={isBatchCreatingReservationIdentifiers}
                        />
                      )}
                    </>
                  ),
                    ...
                    <>
                      {!row.disabled && (
                        <EditButton
                          ...
                          onClick={() => this.selectEditorTarget(id, row)}
                          title="Muokkaa"
                        />
                      )}
                    </>
            ...
            {errors && (
              <ErrorBlock
                error={
                  "Varaustunnusten luonti keskeytyi virheeseen. Osa tunnuksista on saattanut jäädä luomatta ja osa luoduista tunnuksista on saattanut jäädä linkittymättä haettuihin kohteisiin. Tarkista ajankohtainen tilanne sivu päivittämällä ja yritä luontia uudelleen."
                }
              />
            )}
            <ModalButtonWrapper>
              <LoaderWrapper className="small-inline-wrapper">
                <Loader
                  isLoading={isBatchCreatingReservationIdentifiers}
                  className="small"
                />
              </LoaderWrapper>
              <Button
...
export default flowRight(
  connect(
    (state) => ({
      ...
      isBatchCreatingReservationIdentifiers:
        getIsBatchCreatingReservationIdentifiers(state),
