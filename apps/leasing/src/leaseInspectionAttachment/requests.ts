...
export const createLeaseInspectionAttachment = (
  ...
  return callUploadRequest(
    new Request(createUrl("inspection_attachment/"), {
      method: "POST",
      body,
    }),
  );
};
export const deleteLeaseInspectionAttachment = (
  id: number,
): Generator<any, any, any> => {
  return callApi(
    new Request(createUrl(`inspection_attachment/${id}/`), {
      method: "DELETE",
    }),
  );
};
