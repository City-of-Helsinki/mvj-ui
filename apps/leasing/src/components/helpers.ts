...
export const getRentExplanationDescription = (
...
export const getRentSubItemDescription = (
  subItem: Record<string, any>,
  attributes: Attributes,
) => {
  const rentAdjustementTypeOptions = getFieldOptions(
    attributes,
...
const getContentBillingPeriodInvoiceRowTenant = (row: Record<string, any>) => {
  const tenant = get(row, "tenant.tenantcontact_set").find(
    (tenant) => tenant.type === TenantContactType.TENANT,
  );
  return {
...
const getContentBillingPeriod = (billingPeriod: BillingPeriod) => {
  const totalAmount = billingPeriod.reduce(
    (sum, invoice) => sum + Number(invoice.billed_amount),
    0,
  );
  return {
    ...
    invoices: billingPeriod.map((invoice) =>
      getContentBillingPeriodInvoice(invoice),
    ),
...
export const getContentPreviewInvoiceBillingPeriods = (
