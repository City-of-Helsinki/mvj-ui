 ...
 * @return {Object[]}
 */
export const getContentPlotApplicationsListResults = (
  content: any,
): Array<Record<string, any>> =>
  getApiResponseResults(content).map((plotApplication) =>
    getContentApplicationListItem(plotApplication),
  );
 ...
 * @return {Object}
 */
export const getContentApplicationListItem = (
  plotApplication: Record<string, any>,
): Record<string, any> => {
  return {
    ...
    target_address: plotApplication.targets.map(
 ...
 * @returns {Object[]}
 */
export const getApplicationTargetFeatures = (
 ...
 * @returns {Object}
 */
export const getApplicationTargetGeoJson = (
  applications: Array<Record<string, any>>,
): LeafletGeoJson => {
...
export const reshapeSavedApplicationObject = (
  ...
  const reshapeSingleSectionData = (section, answersNode, sectionPath) => {
    ...
    section.subsections.forEach((subsection) => {
      reshapeArrayOrSingleSection(
        subsection,
        data,
        answersNode,
        `${sectionPath}.${subsection.identifier}`,
      );
    });
    section.fields.forEach((field) => {
      ...
      switch (
        fieldTypes?.find((fieldType) => fieldType.value === field.type)
          ?.display_name
      ) {
        case "radiobutton":
        ...
        case "uploadfiles":
          ...
          value = attachments.filter(
            (attachment) =>
              attachment.field === field.identifier &&
              attachment.path === sectionPath,
          );
          extra_value = "";
          break;
      }
  ...
  const reshapeArrayOrSingleSection = (
    section,
    parentResultNode,
    answersNode,
    sectionPath,
    ...
    if (section.add_new_allowed) {
      ...
      sectionAnswers.forEach((answer, i) => {
        parentResultNode.sections[section.identifier].push(
          reshapeSingleSectionData(section, answer, `${sectionPath}[${i}]`),
        );
      });
    } else {
      ...
      if (sectionAnswers) {
        parentResultNode.sections[section.identifier] =
          reshapeSingleSectionData(section, sectionAnswers, sectionPath);
      }
    }
  };
...
export const preparePlotApplicationForSubmission = (): Record<
  string,
  any
> | null => {
...
export const getSectionTargetFromMeta = (field: string): string => {
  const state = store.getState();
  const meta = formValueSelector(FormNames.PLOT_APPLICATION)(
    state,
    `${field}.metadata`,
  );

  if (meta) {
    const target = getCurrentEditorTargets(state).find(
      (target) => target.id === meta.identifier,
    );
...
export const getInitialTargetInfoCheckValues = (
  plotSearch: PlotSearch,
  infoCheckData: Record<string, any>,
  id: number,
  ...
  const infoCheck = infoCheckData.find(
    (infoCheck) => infoCheck.identifier === targetIdentifier,
  );
...
export const getMeetingMemoDownloadLink = (id: number): string =>
  createUrl(`meeting_memo/${id}/download`);
