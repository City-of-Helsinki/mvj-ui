...
const attributesReducer: Reducer<Attributes> = handleActions(
  {
    ["mvj/collectionNote/RECEIVE_ATTRIBUTES"]: (
      state: Attributes,
...
const methodsReducer: Reducer<Methods> = handleActions(
  {
    ["mvj/collectionNote/RECEIVE_METHODS"]: (
      state: Methods,
...
const isFetchingByLeaseReducer: Reducer<Record<string, any>> = handleActions(
  {
    ["mvj/collectionNote/FETCH_BY_LEASE"]: (
      ...
      { payload: lease }: FetchCollectionNotesByLeaseAction,
    ) => {
      return { ...state, [lease]: true };
    },
    ["mvj/collectionNote/RECEIVE_BY_LEASE"]: (
      state: Record<string, any>,
      { payload: { lease } }: ReceiveCollectionNotesByLeaseAction,
    ) => {
      return { ...state, [lease]: false };
    },
    ["mvj/collectionNote/NOT_FOUND_BY_LEASE"]: (
      state: Record<string, any>,
      { payload: lease }: CollectionNotesNotFoundByLeaseAction,
    ) => {
      return { ...state, [lease]: false };
    },
  },
...
const byLeaseReducer: Reducer<Record<string, any>> = handleActions(
  {
    ["mvj/collectionNote/RECEIVE_BY_LEASE"]: (
      state: Record<string, any>,
      { payload }: ReceiveCollectionNotesByLeaseAction,
    ) => {
      return { ...state, [payload.lease]: payload.collectionNotes };
    },
  },
  {},
);
