...
class AreaSearchApplicationCreateSpecs extends Component<Props> {
  componentDidMount() {
    const { receiveFormValidFlags, valid } = this.props;
    receiveFormValidFlags({
  ...
  render(): JSX.Element {
    ...
    const geometryHasError =
      geometryError && (isSaveClicked || formMeta.geometry?.touched);
    const today = startOfToday();
    return (
      <div className="AreaSearchApplicationCreate">
        <Title>Aluehaku</Title>
        ...
        <Row>
          <Column small={12}>
            <FormFieldLabel required>Haettava alue</FormFieldLabel>
            <AreaSearchMap
              change={(features) => {
        ...
        <Row>
          <Authorization allow={true}>
            <Column small={12} medium={4}>
              <FormFieldLabel htmlFor="areaSearchApplicationAttachments">
                Liitteet
              </FormFieldLabel>
              ...
              {attachments.map((attachment) => (
                <Row key={attachment.id}>
                  ...
                  <Column small={4} medium={2}>
                    <RemoveButton
                      onClick={() => this.handleFileRemoved(attachment.id)}
                    />
                  </Column>
                </Row>
              ))}
              <AddFileButton
                ...
                name="AreaSearchApplicationCreateSpecsAttachment"
                disabled={
                  isPerformingFileOperation || isSubmittingAreaSearchSpecs
                }
              />
            </Column>
          </Authorization>
  ...
  connect(
    (state) => {
      return {
        ...
        geometryError: getFormSyncErrors(FormNames.AREA_SEARCH_CREATE_SPECS)(
          state,
        )?.geometry,
      };
    },
