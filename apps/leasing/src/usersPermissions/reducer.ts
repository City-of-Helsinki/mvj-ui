...
const usersPermissionsReducer: Reducer<UsersPermissions> = handleActions(
  {
    ["mvj/usersPermissions/RECEIVE_ALL"]: (
      state: UsersPermissions,
      { payload: permissions }: ReceiveUsersPermissionsAction,
    ) => {
      return permissions;
    },
  },
...
const groupsReducer: Reducer<UserGroups> = handleActions(
  {
    ["mvj/usersPermissions/RECEIVE_GROUPS"]: (
      state: UserGroups,
      { payload: groups }: ReceiveUserGroupsAction,
    ) => {
      return groups;
    },
  },
...
const serviceUnitsReducer: Reducer<UserServiceUnits> = handleActions(
  {
    ["mvj/usersPermissions/RECEIVE_SERVICE_UNITS"]: (
      state: UserServiceUnits,
      { payload: serviceUnits }: ReceiveUserServiceUnitsAction,
    ) => {
      return serviceUnits;
    },
  },
...
const activeServiceUnitReducer: Reducer<UserServiceUnit> = handleActions(
  {
    ["mvj/usersPermissions/SET_ACTIVE_SERVICE_UNIT"]: (
      state: UserServiceUnit,
      { payload: serviceUnit }: SetUserActiveServiceUnitAction,
    ) => {
      return serviceUnit;
    },
  },
