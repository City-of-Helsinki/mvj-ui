/**
 * Users permissions enumerable.
 * @readonly
 * @enum {string}
 */
export const UsersPermissions = {
  ADD_ANSWEROPENINGRECORD: "add_answeropeningrecord",
  ADD_BASISOFRENTDECISION: "add_basisofrentdecision",
  ADD_BASISOFRENTPROPERTYIDENTIFIER: "add_basisofrentpropertyidentifier",
  ADD_BASISOFRENTRATE: "add_basisofrentrate",
  ADD_CUSTOMDETAILEDPLAN: "add_customdetailedplan",
  ADD_COLLATERAL: "add_collateral",
  ADD_COLLECTIONCOURTDECISION: "add_collectioncourtdecision",
  ADD_COLLECTIONLETTER: "add_collectionletter",
  ADD_COLLECTIONNOTE: "add_collectionnote",
  ADD_CONDITION: "add_condition",
  ADD_CONSTRUCTABILITYDESCRIPTION: "add_constructabilitydescription",
  ADD_CONTRACT: "add_contract",
  ADD_CONTRACTCHANGE: "add_contractchange",
  ADD_CONTRACTRENT: "add_contractrent",
  ADD_DECISION: "add_decision",
  ADD_EMAILLOG: "add_emaillog",
  ADD_FIXEDINITIALYEARRENT: "add_fixedinitialyearrent",
  ADD_INFILLDEVELOPMENTCOMPENSATIONDECISION:
    "add_infilldevelopmentcompensationdecision",
  ADD_INFILLDEVELOPMENTCOMPENSATIONINTENDEDUSE:
    "add_infilldevelopmentcompensationintendeduse",
  ADD_INFILLDEVELOPMENTCOMPENSATIONLEASE:
    "add_infilldevelopmentcompensationlease",
  ADD_INSPECTION: "add_inspection",
  ADD_INSPECTIONATTACHMENT: "add_inspectionattachment",
  ADD_INVOICE: "add_invoice",
  ADD_LEASEAREA: "add_leasearea",
  ADD_LEASEAREAADDRESS: "add_leaseareaaddress",
  ADD_LEASEAREAATTACHMENT: "add_leaseareaattachment",
  ADD_LEASEBASISOFRENT: "add_leasebasisofrent",
  ADD_MANAGEMENTSUBVENTION: "add_managementsubvention",
  ADD_PLANUNIT: "add_planunit",
  ADD_PLOT: "add_plot",
  ADD_LEASE_HISTORY_ITEM: "add_relatedlease",
  ADD_RENT: "add_rent",
  ADD_RENTADJUSTMENT: "add_rentadjustment",
  ADD_TEMPORARYSUBVENTION: "add_temporarysubvention",
  ADD_TENANT: "add_tenant",
  ADD_TENANTCONTACT: "add_tenantcontact",
  ADD_TENANTRENTSHARE: "add_tenantrentshare",
  ADD_USAGEDISTRIBUTION: "add_usagedistribution",
  CHANGE_LEASE_INVOICING_ENABLED_AT: "change_lease_invoicing_enabled_at",
  CHANGE_LEASE_RENT_INFO_COMPLETED_AT: "change_lease_rent_info_completed_at",
  CHANGE_LEASE_LEASE_AREAS: "change_lease_lease_areas",
  CHANGE_RENT_DUE_DATES: "change_rent_due_dates",
  CHANGE_CUSTOMDETAILEDPLAN_INFOLINKS: "change_customdetailedplan_info_links",
  DELETE_BASISOFRENTDECISION: "delete_basisofrentdecision",
  DELETE_BASISOFRENTPROPERTYIDENTIFIER: "delete_basisofrentpropertyidentifier",
  DELETE_BASISOFRENTRATE: "delete_basisofrentrate",
  DELETE_COLLATERAL: "delete_collateral",
  DELETE_COLLECTIONCOURTDECISION: "delete_collectioncourtdecision",
  DELETE_COLLECTIONLETTER: "delete_collectionletter",
  DELETE_COLLECTIONNOTE: "delete_collectionnote",
  DELETE_CONDITION: "delete_condition",
  DELETE_CONSTRUCTABILITYDESCRIPTION: "delete_constructabilitydescription",
  DELETE_CONTRACT: "delete_contract",
  DELETE_CONTRACTCHANGE: "delete_contractchange",
  DELETE_CONTRACTRENT: "delete_contractrent",
  DELETE_CUSTOMDETAILEDPLAN: "delete_customdetailedplan",
  DELETE_DECISION: "delete_decision",
  DELETE_FIXEDINITIALYEARRENT: "delete_fixedinitialyearrent",
  DELETE_INFILLDEVELOPMENTCOMPENSATIONDECISION:
    "delete_infilldevelopmentcompensationdecision",
  DELETE_INFILLDEVELOPMENTCOMPENSATIONINTENDEDUSE:
    "delete_infilldevelopmentcompensationintendeduse",
  DELETE_INFILLDEVELOPMENTCOMPENSATIONLEASE:
    "delete_infilldevelopmentcompensationlease",
  DELETE_INSPECTION: "delete_inspection",
  DELETE_INSPECTIONATTACHMENT: "delete_inspectionattachment",
  DELETE_INVOICE: "delete_invoice",
  DELETE_LEASEAREA: "delete_leasearea",
  DELETE_LEASEAREAADDRESS: "delete_leaseareaaddress",
  DELETE_LEASEAREAATTACHMENT: "delete_leaseareaattachment",
  DELETE_LEASEBASISOFRENT: "delete_leasebasisofrent",
  DELETE_MANAGEMENTSUBVENTION: "delete_managementsubvention",
  DELETE_NONEMPTY_LEASE: "delete_nonempty_lease",
  DELETE_PLANUNIT: "delete_planunit",
  DELETE_PLOT: "delete_plot",
  DELETE_LEASE_HISTORY_ITEM: "delete_relatedlease",
  DELETE_RENT: "delete_rent",
  DELETE_RENTADJUSTMENT: "delete_rentadjustment",
  DELETE_TEMPORARYSUBVENTION: "delete_temporarysubvention",
  DELETE_TENANT: "delete_tenant",
  DELETE_TENANTCONTACT: "delete_tenantcontact",
  DELETE_TENANTRENTSHARE: "delete_tenantrentshare",
  DELETE_USAGEDISTRIBUTION: "delete_usagedistribution",
  EDIT_GLOBAL_UI_DATA: "edit_global_ui_data",
  SEND_CREDITDECISION_INQUIRY: "send_creditdecision_inquiry",
  SEND_SANCTIONS_INQUIRY: "send_sanctions_inquiry",
  VIEW_ANSWER: "view_answer",
  VIEW_AREANOTE: "view_areanote",
  VIEW_AREASEARCH: "view_areasearch",
  VIEW_BASISOFRENT: "view_basisofrent",
  VIEW_COLLECTIONCOURTDECISION: "view_collectioncourtdecision",
  VIEW_COLLECTIONLETTER: "view_collectionletter",
  VIEW_COLLECTIONNOTE: "view_collectionnote",
  VIEW_COMMENT: "view_comment",
  VIEW_CONTACT: "view_contact",
  VIEW_CREDITDECISION: "view_creditdecision",
  VIEW_EMAILLOG: "view_emaillog",
  VIEW_INFILLDEVELOPMENTCOMPENSATION: "view_infilldevelopmentcompensation",
  VIEW_INDEX: "view_index",
  VIEW_INVOICE: "view_invoice",
  VIEW_INVOICENOTE: "view_invoicenote",
  VIEW_INVOICESET: "view_invoiceset",
  VIEW_JOB: "view_job",
  VIEW_JOBRUN: "view_jobrun",
  VIEW_LEASE_INVOICING_CONFIRMATION_REPORT:
    "view_lease_invoicing_confirmation_report",
  VIEW_LEASE_LEASE_AREAS: "view_lease_lease_areas",
  VIEW_LEASE: "view_lease",
  VIEW_LEASEHOLDTRANSFER: "view_leaseholdtransfer",
  VIEW_LEASETYPE: "view_leasetype",
  VIEW_PLOTSEARCH: "view_plotsearch",
  VIEW_SCHEDULEDJOB: "view_scheduledjob",
  VIEW_VAT: "view_vat",
};
