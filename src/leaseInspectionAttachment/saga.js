...
function* createLeaseInspectionAttachmentSaga({payload}): Generator<any, any, any> {
  try {
    const {response: {status: statusCode}, bodyAsJson} = yield call(createLeaseInspectionAttachment, payload);
    ...
    switch (statusCode) {
      case 201:
        yield put(fetchSingleLeaseAfterEdit({
          ...
          callbackFunctions: [
            () => displayUIMessage({title: '', body: 'Tiedosto lisätty'}),
          ],
        }));
        break;
      case 404:
...
function* deleteLeaseInspectionAttachmentSaga({payload}): Generator<any, any, any> {
  try {
    const {response: {status: statusCode}, bodyAsJson} = yield call(deleteLeaseInspectionAttachment, payload.id);
