  ...
  describe("Reducer", () => {
    describe("plotSearchReducer", () => {
      ...
      it("should update plotSearch list", () => {
        ...
        const state = plotSearchReducer(
          {},
          receivePlotSearchList(dummyPlotSearchList),
        );
        expect(state).to.deep.equal(newState);
      });
      ...
      it("should update current plotSearch", () => {
        ...
        const state = plotSearchReducer(
          {},
          receiveSinglePlotSearch(dummyPlotSearch),
        );
        expect(state).to.deep.equal(newState);
      });
      ...
      it("should update pendingPlanUnitFetches array by planUnitNotFound", () => {
        const newState = { ...baseState, pendingPlanUnitFetches: [] };
        const state = plotSearchReducer(
          {
      ...
      it("should update isFetchingPlanUnitAttributes to false by planUnitAttributesNotFound", () => {
        const newState = { ...baseState, isFetchingPlanUnitAttributes: false };
        const state = plotSearchReducer(
          {
      ...
      it("should update pendingPlanUnitFetches array when fetching fetchPlanUnit", () => {
        const newState = { ...baseState, pendingPlanUnitFetches: [1] };
        const state = plotSearchReducer(
          {},
          fetchPlanUnit({
            value: 1,
      ...
      it("should update subTypes", () => {
        ...
        const state = plotSearchReducer(
          {},
          receivePlotSearchSubtype(dummyData),
        );
        expect(state).to.deep.equal(newState);
      });
      ...
      it("should update planUnitAttributes", () => {
        ...
        const state = plotSearchReducer(
          {},
          receivePlanUnitAttributes(dummyAttributes),
        );
        expect(state).to.deep.equal(newState);
      });
      ...
      it("should add and annotate decision candidates from plan units", () => {
        const state = mockData[0].plot_search_targets.reduce(
          (state, nextTarget) =>
            plotSearchReducer(
              state,
              addPlanUnitDecisions(nextTarget.plan_unit),
            ),
          {},
        );
        ...
        expect(state.decisionCandidates[1][1].reference_number).to.equal(
          "HEL 2021-000002",
        );
        expect(
          state.decisionCandidates[1][1].relatedPlanUnitIdentifier,
        ).to.equal("91-1-1-1");
      });
      ...
      it("should disallow editing most fields if the plot search has become public", () => {
        const state = plotSearchReducer(
          {},
          receiveSinglePlotSearch({ ...mockData[0] }),
        );
        expect(
