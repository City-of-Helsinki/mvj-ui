...
import { formatDate, getApiResponseResults } from "/src/util/helpers";
 ...
 * @return {Object}
 */
export const getContentBasicInformation = (plotSearch: PlotSearch): Record<string, any> => {
  return {
    id: plotSearch.id,
    ...
    plot_search_targets: plotSearch.plot_search_targets?.map(target => ({ ...target,
 ...
 * @return {Object}
 */
export const getContentApplication = (plotSearch: PlotSearch, plotSearchForm: Form | null = null): Record<string, any> => {
  return {
    template: null,
 ...
 * @return {Object}
 */
export const getContentSearchProperties = (searchProperties: Record<string, any>): Record<string, any> => {
 ...
 * @return {Object}
 */
export const getContentPlotSearchListItem = (plotSearch: PlotSearch): Record<string, any> => {
  return {
    id: plotSearch.id,
 ...
 * @returns {string}
 */
export const getContentPlanUnitIdentifier = (plan_unit: Record<string, any>): string | null | undefined => {
  return !isEmpty(plan_unit) ? `${get(plan_unit, 'identifier')} ${get(plan_unit, 'plan_unit_status') || ''} ${get(plan_unit, 'lease_identifier') || ''}` : null;
};
...
export const getPlanUnitFromObjectKeys = (planUnit: Record<string, any>, index: any): Record<string, any> | null | undefined => {
  if (planUnit && Object.keys(planUnit) && planUnit[Object.keys(planUnit)[index]]) return planUnit[Object.keys(planUnit)[index]];else return null;
};
 ...
 * @returns {Object}
 */
export const cleanTargets = (payload: Record<string, any>, shouldFixTargetType: boolean): Record<string, any> => {
 ...
 * @returns {Object}
 */
export const cleanDecisions = (payload: Record<string, any>): Record<string, any> => {
  return { ...payload,
    decisions: payload.decisions.filter(decision => !!decision.id && !!decision.relatedPlanUnitId).map(decision => decision.id)
  };
};
 ...
 * @returns {Object}
 */
export const filterSubTypes = (subTypes: Record<string, any>, type: string): Record<string, any> => {
...
export const hasMinimumRequiredFieldsFilled = (state: PlotSearchState): boolean => {
  return ['name', 'preparers', 'stage', 'type', 'subtype'].every(fieldName => !!formValueSelector(FormNames.PLOT_SEARCH_BASIC_INFORMATION)(state, fieldName));
};
export const formatDecisionName = (decision: Record<string, any>): string => {
  const textsFromFields = [decision.relatedPlanUnitIdentifier && `(${decision.relatedPlanUnitIdentifier})`, decision.reference_number, decision.decision_maker?.name, formatDate(decision.decision_date), decision.section ? decision.section + '\u00a0§' : null, decision.type?.name, decision.conditions?.length > 0 ? `(${decision.conditions.length} ehto${decision.conditions.length === 1 ? '' : 'a'})` : null].filter(text => !!text);
  return textsFromFields.join('\u00a0\u00a0');
};
export const annotatePlanUnitDecision = (decision: Record<string, any>, plotUnit: Record<string, any>): Record<string, any> => {
  return { ...decision,
...
export const getDefaultNewFormSection = (peerIdentifiers: Array<string>): Record<string, any> => ({
...
export const getDefaultNewFormField = (peerIdentifiers: Array<string>): Record<string, any> => ({
  temporary_id: getUniqueTemporaryId(),
...
export const getInitialFormSectionEditorData = (fieldTypeMapping: Record<string, any>, section: FormSection): Record<string, any> => {
  ...
  const addUIPropertiesToField = (field: FormField, peerFieldIdentifiers: Array<string>, isProtected: boolean, protectedValues: Array<string>): Record<string, any> => {
    ...
    return { ...cloneDeep(field),
      ...
      auto_fill_identifier: generateFieldIdentifierFromName(field.label, peerFieldIdentifiers) === field.identifier,
      auto_fill_choice_values: protectedValues.length === 0 && field.choices.every((choice, index) => choice.value.toString() === (index + 1).toString()),
  ...
  const addUIPropertiesToSection = (section: FormSection, peerSectionIdentifiers: Array<string>, protectedPaths: ProtectedFormPathsSectionNode): Record<string, any> => {
    ...
    return { ...cloneDeep(section),
      ...
      subsections: section.subsections.map((subsection, index) => addUIPropertiesToSection(subsection, subsectionIdentifiers.filter((_, i) => i !== index), protectedPaths?.subsections?.[subsection.identifier] || {})),
      fields: section.fields.map((field, index) => addUIPropertiesToField(field, fieldIdentifiers.filter((_, i) => i !== index), protectedPaths?.fields?.includes(field.identifier) || false, protectedPaths?.fieldChoices?.[field.identifier] || [])),
      auto_fill_identifier: generateFieldIdentifierFromName(section.title, peerSectionIdentifiers) === section.identifier
    };
  };
...
export const generateSectionIdentifierFromName = (name: string, peerIdentifiers: Array<string>): string => generateIdentifierFromName(name, peerIdentifiers, 'Uusi osio');
export const generateFieldIdentifierFromName = (name: string, peerIdentifiers: Array<string>): string => generateIdentifierFromName(name, peerIdentifiers, 'Uusi kenttä');
export const transformCommittedFormSectionEditorData = (section: Record<string, any>): FormSection => {
  const transformChoice = (choice: Record<string, any>, index: number): Record<string, any> => {
    const {
      text,
      ...rest
    } = choice;
  ...
  const transformField = (field: Record<string, any>, index: number): Record<string, any> => {
    const {
