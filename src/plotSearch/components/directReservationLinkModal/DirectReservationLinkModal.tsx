...
import { FieldArray, getFormValues, reduxForm, change, initialize } from "redux-form";
...
const renderTargets = ({
  ...
  {!!targets && targets.map(target => {
    ...
    const handleChange = () => {
      if (isChecked) {
        change('targets', formValues['targets'].filter(id => id !== target.id));
      } else {
        change('targets', [...formValues['targets'], target.id]);
      }
    };

    return <Column lg={4} sm={4} key={target.id}>
      <FormFieldLabel>Kohteen tunnus</FormFieldLabel>
      <label className='option-label'>
        <input type='checkbox' checked={isChecked} name={`${target.id}-selected`} onChange={handleChange} value={target.id} />
...
class DirectReservationLinkModal extends Component<Props> {
  ...
  render(): JSX.Element {
    ...
    return <Modal isOpen={isOpen} onClose={onClose} title="Lähetä henkilökohtainen hakemuslinkki" className="DirectReservationLinkModal">
        <p>Tällä lomakkeella voit luoda uniikin hakemuslinkin vastaanottajalle</p>
        <Row>
          <FieldArray component={renderTargets} disabled={false} formName={FormNames.PLOT_SEARCH_DIRECT_RESERVATION_LINK} name={'targets'} targets={targets} change={change} formValues={formValues} />
        </Row>
        <Row>
        ...
        <ModalButtonWrapper>
          <LoaderWrapper className='small-inline-wrapper'>
            <Loader isLoading={isCreatingDirectReservationLink} className='small' />
          </LoaderWrapper>
          <Button className={ButtonColors.SECONDARY} onClick={onClose} text="Peruuta" />
          <Button disabled={isCreatingDirectReservationLink} text="Lähetä" onClick={this.handleSubmit} />
        </ModalButtonWrapper>
      </Modal>;
  }
...
export default (flowRight(connect(state => ({
  ...
  isCreatingDirectReservationLink: getIsCreatingDirectReservationLink(state)
}), {
  change,
  initialize,
  createDirectReservationLink
