...
import {
  ...
  getLastBatchReservationCreationError, getReservationIdentifierUnitLists,
...
type Props = {
  ...
  batchCreateReservationIdentifiers: Function,
  isBatchCreatingReservationIdentifiers: boolean,
  fetchSinglePlotSearch: Function,
  rowsToCreate: Array<Object>,
  errors: any,
  fetchReservationIdentifierUnitLists: Function,
...
class ReservationIdentifiersModal extends Component<Props, State> {
    ...
    const data = currentPlotSearch.plot_search_targets.reduce((acc, target) => {
      ...
      return [
        ...acc,
        {
          ...
          applicants: target.reservation_recipients.map((group) =>
            `${group.reservation_recipients.join(', ')} (${group.share_of_rental})`).join('; '),
          leaseOptions,
  ...
  selectEditorTarget = (index: number | null, row: Object) => {
  ...
  handleEditFinished = (leaseData: Object) => {
  ...
  handleSubmit = () => {
    ...
    batchCreateReservationIdentifiers({
      data: rowData.filter((row) => rowsToCreate[row.index].selected).map((row) => ({
        ...row.leaseOptions,
  ...
  convertLeaseOptionsToIdentifier = (options: Object) => {
  ...
  render(): React$Node {
    ...
    const currentModalTitle = editorTarget !== null
      ? 'Muokkaa varaustunnusta'
      : 'Varaustunnukset';
          ...
          ? <>
            <SortableTable
              ...
              columns={[
                ...
                },
                {
                  ...
                  text: 'Muokkaa',
                  sortable: false,
                  renderer: (id, row) => <>{!row.disabled && <EditButton
            ...
            <ModalButtonWrapper>
              ...
              <Button
                ...
                disabled={selectedRowsCount === 0 || isBatchCreatingReservationIdentifiers}
              />
            </ModalButtonWrapper>
          </>
          : <>
...
)(ReservationIdentifiersModal): React$ComponentType<OwnProps>);
