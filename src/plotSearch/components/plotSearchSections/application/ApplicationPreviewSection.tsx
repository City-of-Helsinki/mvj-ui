...
class ApplicationPreviewSection extends PureComponent<Props> {
    ...
    return (
      <Fragment>
        {!isFirstLevel && (
          <Row>
            <Column large={12}>
              <SubTitle>{get(section, "title")}</SubTitle>
            </Column>
          </Row>
        )}
        <Row>
        ...
        {section.subsections &&
          section.subsections
            ...
            .map((subsection, i) => (
              <Fragment key={i}>
                {this.renderSection(subsection, false)}
              </Fragment>
            ))}
  ...
  renderField = (field: Record<string, any>) => {
    ...
    switch (matchingType) {
        ...
        fieldSpecificComponents = (
          <>
            <FormField
              ...
              disabled
              className={
                field.type === "radiobuttoninline"
                  ? "form-field__radio-button-inline"
                  : ""
              }
            />
          </>
      ...
      default:
        console.error(
          `Form field type ${matchingType} (${field?.type}) is not implemented`,
        );
        fieldSpecificComponents = (
          <>
            <FormTextTitle>{get(field, "label")}</FormTextTitle>
        ...
        >
              ...
              openEditPlotApplicationSectionModal && (
                <Button
                  className={classNames(
                    ...
                    "application-preview-section__edit-button",
                  )}
                  onClick={() => {
                    openEditPlotApplicationSectionModal();
                  }}
          ...
          >
            {section.visible ? (
              ...
              >
                <em>Osio on piilotettu kokonaisuudessaan.</em>
              </div>
            )}
          </Collapse>
        </Column>
