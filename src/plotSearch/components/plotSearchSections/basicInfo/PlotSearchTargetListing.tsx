...
const PlotSearchTargetListing = ({
  ...
  const searchableTargets = targets.filter(
    (target) => target.target_type === PlotSearchTargetType.SEARCHABLE,
  );
  const proceduralReservationTargets = targets.filter(
    (target) => target.target_type === PlotSearchTargetType.PROCEDURAL,
  );
  const directReservationTargets = targets.filter(
    (target) => target.target_type === PlotSearchTargetType.DIRECT,
  );
  return (
    <>
      {searchableTargets.length > 0 && (
        <WhiteBox>
          <SubTitle>HAETTAVAT KOHTEET</SubTitle>
      ...
      {proceduralReservationTargets.length > 0 && (
        <WhiteBox>
          <SubTitle>MENETTELYVARAUS</SubTitle>
