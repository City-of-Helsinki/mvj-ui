...
import { receiveCollapseStates, receiveIsSaveClicked, receiveFormValidFlags, removePlanUnitDecisions, fetchCustomDetailedPlanAttributes, fetchPlanUnitAttributes } from "src/plotSearch/actions";
import { getAttributes, getCollapseStateByKey, getIsSaveClicked, getErrorsByFormName, getPlotSearchSubTypes, getDecisionCandidates } from "src/plotSearch/selectors";
import { filterSubTypes } from "src/plotSearch/helpers";
...
import PlotSearchApplicationsOpeningSection from "src/plotSearch/components/plotSearchSections/basicInfo/PlotSearchApplicationsOpeningSection";
type DecisionsProps = {
...
const renderDecisions = ({
  ...
  return <AppConsumer>
      ...
      return <Fragment>
            ...
            return <BasicInfoDecisionEdit key={`${fields.get(index)?.id}_${index}`} disabled={disabled} field={field} formName={formName} onRemove={handleRemove} onChange={handleChange} attributes={attributes} getPlotUnitDecisions={getPlotUnitDecisions} cacheKey={cacheKey} />;
          })}
      ...
      return <Fragment>
          ...
          const handleRemove = () => {
            dispatch({
              type: ActionTypes.SHOW_CONFIRMATION_MODAL,
              confirmationFunction: () => {
                const id = fields.get(index)?.plan_unit_id || fields.get(index)?.custom_detailed_plan_id;
                fields.remove(index);
          ...
          return <PlotSearchSiteEdit key={`${fields.get(index)?.id}_${index}`} index={index} disabled={disabled} field={field} form={form} formName={formName} change={change} onRemove={handleRemove} onReplace={handleChange} />;
        })}
  ...
  render() {
    ...
    if (stageAttributes && canEditStage) {
      stageAttributes = { ...stageAttributes,
        choices: stages.filter(stage => !AUTOMATIC_PLOT_SEARCH_STAGES.includes(stage.stage)).map(stage => ({
    ...
    return <form>
        ...
        <Row className='summary__content-wrapper'>
          <Column small={12}>
            ...
            <Collapse defaultOpen={collapseStateBasic !== undefined ? collapseStateBasic : true} hasErrors={isSaveClicked && !isEmpty(errors)} headerTitle={PlotSearchFieldTitles.BASIC_INFO} onToggle={this.handleBasicInfoCollapseToggle}>
              <Row>
                <Authorization allow={isFieldAllowedToRead(attributes, 'name')}>
                  <Column small={12} medium={12} large={5}>
                    <FormField disableTouched={isSaveClicked} fieldAttributes={get(attributes, 'name')} name='name' overrideValues={{
                    label: PlotSearchFieldTitles.NAME
                  }} enableUiDataEdit uiDataKey={getUiDataPlotSearchKey('name')} />
                  </Column>
                </Authorization>
                <Authorization allow={isFieldAllowedToRead(attributes, 'preparers')}>
                  <Column small={12} medium={6} large={4}>
                    <FormField disableTouched={isSaveClicked} fieldAttributes={get(attributes, 'preparers')} name='preparers' overrideValues={{
                    fieldType: FieldTypes.USER,
                    label: PlotSearchFieldTitles.PREPARERS,
                    required: true,
                    multiSelect: true
                  }} enableUiDataEdit uiDataKey={getUiDataPlotSearchKey('preparers')} />
                  </Column>
                </Authorization>
                <Authorization allow={isFieldAllowedToRead(attributes, 'search_class')}>
                  <Column small={12} medium={6} large={3}>
                    <FormField disableTouched={isSaveClicked} fieldAttributes={get(attributes, PlotSearchFieldPaths.SEARCH_CLASS)} name='search_class' overrideValues={{
                    label: PlotSearchFieldTitles.SEARCH_CLASS,
                    required: true,
                    allowEdit: !isLockedForModifications
                  }} enableUiDataEdit uiDataKey={getUiDataPlotSearchKey('search_class')} />
                  </Column>
                </Authorization>
                <Authorization allow={isFieldAllowedToRead(attributes, 'type')}>
                  <Column small={12} medium={6} large={3}>
                    <FormField disableTouched={isSaveClicked} fieldAttributes={get(attributes, PlotSearchFieldPaths.TYPE)} name='type' overrideValues={{
                    label: PlotSearchFieldTitles.TYPE,
                    required: true,
                    allowEdit: !isLockedForModifications
                  }} enableUiDataEdit uiDataKey={getUiDataPlotSearchKey('type')} />
                  </Column>
                </Authorization>
                <Authorization allow={isFieldAllowedToRead(attributes, 'subtype')}>
                  <Column small={12} medium={6} large={3}>
                    <FormField disableTouched={isSaveClicked} fieldAttributes={get(attributes, PlotSearchFieldPaths.SUBTYPE)} name='subtype' overrideValues={{
                    label: PlotSearchFieldTitles.SUBTYPE,
                    options: subTypeOptions,
                    required: true,
                    allowEdit: !isLockedForModifications
                  }} enableUiDataEdit uiDataKey={getUiDataPlotSearchKey('subtype')} />
                  </Column>
                </Authorization>
                <Authorization allow={isFieldAllowedToRead(attributes, 'begin_at')}>
                  <Column small={6} medium={6} large={2}>
                    <FormField disableTouched={isSaveClicked} fieldAttributes={get(attributes, 'begin_at')} name='begin_at' overrideValues={{
                    label: 'Alkupvm ja klo',
                    fieldType: FieldTypeOptions.TIME,
                    allowEdit: !isLockedForModifications
                  }} enableUiDataEdit uiDataKey={getUiDataPlotSearchKey('begin_at')} />
                  </Column>
                </Authorization>
                <Authorization allow={isFieldAllowedToRead(attributes, 'end_at')}>
                  <Column small={6} medium={6} large={2}>
                    <FormField disableTouched={isSaveClicked} fieldAttributes={get(attributes, 'end_at')} name='end_at' overrideValues={{
                    label: 'Loppupvm ja klo',
                    fieldType: FieldTypeOptions.TIME,
                    allowEdit: !isLockedForModifications
                  }} enableUiDataEdit uiDataKey={getUiDataPlotSearchKey('end_at')} />
                  </Column>
                </Authorization>
                <Authorization allow={isFieldAllowedToRead(attributes, 'stage')}>
                  <Column small={12} medium={6} large={2}>
                    <FormField disableTouched={isSaveClicked} fieldAttributes={stageAttributes} name='stage' overrideValues={{
                ...
                <Authorization allow={isFieldAllowedToRead(attributes, 'decisions')}>
                  <FieldArray component={renderDecisions} attributes={attributes} disabled={false} formName={FormNames.PLOT_SEARCH_BASIC_INFORMATION} name={'decisions'} isSaveClicked={isSaveClicked} usersPermissions={usersPermissions} decisionCandidates={decisionCandidates} hasUnidentifiedDecisions={hasUnidentifiedDecisions} />
                </Authorization>
              </Row>
              {!isLockedForModifications && <WhiteBox>
                ...
                <FieldArray component={renderPlotSearchSites} attributes={attributes} isClicked={isSaveClicked} disabled={!hasMinimumRequiredFieldsFilled} formName={FormNames.PLOT_SEARCH_BASIC_INFORMATION} name={'plot_search_targets'} usersPermissions={usersPermissions} onRemove={this.onTargetRemoved} change={change} />
              </WhiteBox>}
