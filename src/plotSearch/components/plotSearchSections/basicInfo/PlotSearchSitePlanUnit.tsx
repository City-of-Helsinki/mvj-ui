...
class PlotSearchSitePlanUnit extends PureComponent<Props, State> {
  ...
  updateComponent() {
    ...
    if (update > 100) this.stopTimer();
  }

  startTimer = () => {
    this.timer = setInterval(() => this.updateComponent(), 10);
  };
  ...
  render() {
    ...
    return <Column large={12}>
        <Collapse className='collapse__secondary greenCollapse' defaultOpen={collapseState !== undefined ? collapseState : true} headerTitle={get(currentPlanUnit, 'identifier') ? `${get(currentPlanUnit, 'identifier')} ${get(currentPlanUnit, 'plan_unit_status')}` : '-'} onToggle={this.handleCollapseToggle}>
          <Row style={{
          marginBottom: 10
        }}>
            {isFetchingPlanUnitAttributes && <LoaderWrapper className='relative-overlay-wrapper'><Loader isLoading={true} /></LoaderWrapper>}
              ...
              <Column small={12} medium={12} large={2}>
                <Row>
                  ...
                  <Column small={8} medium={8} large={12}>
                    ...
                    <FormText>
                      <ExternalLink className='no-margin' href={`${getRouteById(Routes.LEASES)}?search=${leaseIdentifier}`} text={leaseIdentifier || '-'} />
                    </FormText>
                  </Column>
                </Row>
              </Column>
...
export default (connect((state, props) => {
  ...
  receiveCollapseStates
})(PlotSearchSitePlanUnit) as React.ComponentType<OwnProps>);