...
class PlotSearchSiteEditCustomDetailedPlan extends Component<Props, State> {
  render() {
    ...
    const title = get(currentCustomDetailedPlan, 'identifier') ? `${get(currentCustomDetailedPlan, 'identifier')} ${get(currentCustomDetailedPlan, 'state.name')} - OMA MUU ALUE` : '-';
    const customDetailedPlanIntendedUseOptions = getFieldOptions(customDetailedPlanAttributes, 'intended_use');
    const customDetailedPlanStateOptions = getFieldOptions(customDetailedPlanAttributes, 'state');
    const customDetailedPlanTypeOptions = getFieldOptions(customDetailedPlanAttributes, 'type');
    ...
    return <Collapse className='collapse__secondary greenCollapse' defaultOpen={collapseState !== undefined ? collapseState : true} headerTitle={title} onRemove={!disabled ? onRemove : null} hasErrors={isSaveClicked && !isEmpty(plotSearchSiteErrors)} onToggle={handleCollapseToggle}>
        <Row>
          <Column small={12} medium={12} large={2}>
            <Row>
              <Column small={6} medium={6} large={12}>
                ...
                <PlanUnitSelectInput value={customDetailedPlanNew} onChange={handleNew} disabled={disabled} name={`plan-unit`} />
                <div style={{
                display: 'none'
              }}>
                ...
                {(isFetchingCustomDetailedPlanAttributes || isFetchingCustomDetailedPlan) && <LoaderWrapper className='relative-overlay-wrapper'><Loader isLoading={true} /></LoaderWrapper>}
              </Column>
            </Row>
          </Column>
