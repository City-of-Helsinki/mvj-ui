...
import { receiveCollapseStates, receiveIsSaveClicked, fetchPlanUnit, fetchCustomDetailedPlan } from "src/plotSearch/actions";
...
const renderSuggested = ({
  ...
  return <AppConsumer>
      ...
      return <Fragment>
            ...
            return <SuggestedEdit key={index} disabled={disabled} field={field} formName={formName} onRemove={handleRemove} attributes={attributes} isSaveClicked={isSaveClicked} />;
          })}
          ...
          <div role="table">
            ...
            return <Row role="row" key={index}>
                <Column role="cell" small={4} medium={4} large={4}>
                  <FormField fieldAttributes={get(attributes, 'plot_search_targets.child.children.info_links.child.children.description')} name={`${field}.description`} invisibleLabel overrideValues={{
                ...
                <Column role="cell" small={5} medium={5} large={5}>
                  <FormField fieldAttributes={get(attributes, 'plot_search_targets.child.children.info_links.child.children.url')} name={`${field}.url`} invisibleLabel overrideValues={{
                  label: PlotSearchFieldTitles.INFO_LINK_URL
                }} disabled={disabled} disableTouched={isSaveClicked} />
                </Column>
                <Column role="cell" small={2} medium={2} large={2}>
                  <FormField fieldAttributes={get(attributes, 'plot_search_targets.child.children.info_links.child.children.language')} name={`${field}.language`} invisibleLabel overrideValues={{
            ...
            {!disabled && <Row>
              <Column>
                <AddButtonThird label='Lisää lisätietolinkki' onClick={handleAdd} />
              </Column>
            </Row>}
          </div>
        </div>;
...
class PlotSearchSiteEdit extends Component<Props, State> {
  ...
  componentDidUpdate(prevProps: Record<string, any>) {
    if (['plotSearchSite', 'planUnitMap', 'customDetailedPlanMap', 'isFetchingPlanUnit', 'isFetchingPlanUnitAttributes', 'isFetchingCustomDetailedPlan', 'isFetchingCustomDetailedPlanAttributes'].some(key => this.props[key] !== prevProps[key])) {
  ...
  render() {
    ...
    if (customDetailedPlanByValue) {
      return <PlotSearchSiteEditCustomDetailedPlan handleNew={this.handleNew} field={field} isSaveClicked={isSaveClicked} areTargetsAllowedToHaveType={areTargetsAllowedToHaveType} attributes={attributes} collapseState={collapseState} onRemove={onRemove} currentTarget={customDetailedPlanByValue} customDetailedPlanNew={customDetailedPlanNew} disabled={disabled} handleCollapseToggle={this.handleCollapseToggle} plotSearchSiteErrors={plotSearchSiteErrors} customDetailedPlanAttributes={customDetailedPlanAttributes} isFetchingCustomDetailedPlan={isFetchingCustomDetailedPlan} isFetchingCustomDetailedPlanAttributes={isFetchingCustomDetailedPlanAttributes} />;
    }

    return <Collapse className='collapse__secondary greenCollapse' defaultOpen={collapseState !== undefined ? collapseState : true} headerTitle={planUnitTitle} onRemove={!disabled ? onRemove : null} hasErrors={isSaveClicked && !isEmpty(plotSearchSiteErrors)} onToggle={this.handleCollapseToggle}>
        <Row style={{
        marginBottom: 10
      }}>
          {isNewer && <WarningContainer style={{
          marginLeft: 5,
          marginTop: 1,
          marginBottom: 1
        }}> {
          ...
          {isDeleted && <WarningContainer style={{
          marginLeft: 5,
          marginTop: 1,
          marginBottom: 1
        }}> {
          ...
          <Column small={12} medium={12} large={2}>
            <Row>
              <Column small={6} medium={6} large={12}>
                ...
                <PlanUnitSelectInput value={planUnitNew} onChange={this.handleNew} disabled={disabled} name={`plan-unit`} />
                <div style={{
                display: 'none'
              }}>
                  <FormField disableTouched={isSaveClicked} fieldAttributes={get(attributes, 'plot_search_targets.child.children.plan_unit_id')} name={`${field}.plan_unit_id`} />
                  <FormField disableTouched={isSaveClicked} fieldAttributes={get(attributes, 'plot_search_targets.child.children.custom_detailed_plan_id')} name={`${field}.custom_detailed_plan_id`} />
                </div>
                {areTargetsAllowedToHaveType && <>
                  <FormTextTitle>
                    {PlotSearchFieldTitles.TARGET_TYPE}
                  </FormTextTitle>
                  <FormField disableTouched={isSaveClicked} invisibleLabel={true} fieldAttributes={get(attributes, 'plot_search_targets.child.children.target_type')} name={`${field}.target_type`} disabled={disabled} />
                </>}
                {(isFetchingPlanUnitAttributes || isFetchingPlanUnit) && <LoaderWrapper className='relative-overlay-wrapper'><Loader isLoading={true} /></LoaderWrapper>}
              </Column>
            </Row>
          </Column>
            ...
            <Row>
              {planUnitByValue && <Fragment>
                ...
                <Column small={12} medium={12} large={12}>
                  <FieldArray component={renderInfoLinks} attributes={attributes} isSaveClicked={isSaveClicked} disabled={disabled} formName={FormNames.PLOT_SEARCH_BASIC_INFORMATION} name={`${field}.info_links`} />
                </Column>
                <FieldArray component={renderSuggested} attributes={attributes} isSaveClicked={isSaveClicked} disabled={true} formName={FormNames.PLOT_SEARCH_BASIC_INFORMATION} name={`${field}.suggested`} />
