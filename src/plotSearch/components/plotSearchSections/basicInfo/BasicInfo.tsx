...
class BasicInfo extends PureComponent<Props, State> {
  ...
  componentDidMount() {
    const { fetchCustomDetailedPlanAttributes, fetchPlanUnitAttributes } =
      this.props;
    fetchCustomDetailedPlanAttributes();
    fetchPlanUnitAttributes();
  }
  ...
  render() {
    ...
    const subtypeOptions = getFieldOptions(
      attributes,
      PlotSearchFieldPaths.SUBTYPE,
    );
    const stageOptions = getFieldOptions(
      attributes,
      PlotSearchFieldPaths.STAGE,
    );
    const searchClassOptions = getFieldOptions(
      attributes,
    ...
    const requiresOpeningRecord = subtypes.find(
      (type) => type.id === plotSearch.subtype,
    )?.require_opening_record;
    return (
      <Fragment>
        <div className="plot_search__basic-info-header-buttons">
          {isInProcessing &&
            requiresOpeningRecord &&
            relatedApplications.some(
              (application) => application.opening_record === null,
            ) && (
              <Button
                onClick={() => showEditMode(true)}
                text="Avaa hakemukset"
              />
            )}
          {plotSearch.stageType === PlotSearchStageTypes.IN_ACTION && (
            <Button
              onClick={openDirectReservationLinkModal}
              text="Lähetä hakemuslinkki"
            />
          )}
          {isInProcessing && (
            <Button
              onClick={openReservationIdentifiersModal}
              text="Luo varaustunnukset"
            />
          )}
          <DocumentsButton
            onClick={openExportModal}
            label="Tulosta kaikki hakemukset"
          />
        </div>
        ...
        <Row className="summary__content-wrapper plot_search__basic-info">
          <Column small={12}>
            ...
            >
              <Row>
                ...
                >
                  <Column small={12} medium={6} large={4}>
                    ...
                    <FormText>
                      {plotSearch.preparers?.map(getUserFullName).join(", ") ||
                        "-"}
                    </FormText>
                  </Column>
                </Authorization>
                ...
                >
                  <Column small={12} medium={6} large={2}>
                    ...
                    <FormText>
                      {getLabelOfOption(
                        searchClassOptions,
                        plotSearch.search_class,
                      ) || "-"}
                    </FormText>
                  </Column>
                </Authorization>
                <Authorization
                  allow={isFieldAllowedToRead(attributes, "applications")}
                >
                  <Column small={12} medium={6} large={2}>
                    <FormTextTitle
                      uiDataKey={getUiDataPlotSearchKey("applications")}
                    >
                      {PlotSearchFieldTitles.APPLICATIONS}
                    </FormTextTitle>
                    {plotSearch.applications &&
                      plotSearch.applications.map((application, index) => (
                        <FormText key={index}>
                          <ExternalLink
                            className="no-margin"
                            href={`${application.id}`}
              ...
              <Row>
                <Authorization allow={isFieldAllowedToRead(attributes, "type")}>
                  <Column small={12} medium={6} large={2}>
                    ...
                    <FormText>
                      {getLabelOfOption(typeOptions, plotSearch.type) || "-"}
                    </FormText>
                  </Column>
                </Authorization>
                <Authorization
                ...
                >
                  <Column small={12} medium={6} large={2}>
                    ...
                    <FormText>
                      {getLabelOfOption(subtypeOptions, plotSearch.subtype) ||
                        "-"}
                    </FormText>
                  </Column>
                </Authorization>
                ...
                >
                  <Column small={6} medium={3} large={1}>
                    ...
                    <FormText>
                      {formatDate(plotSearch.begin_at) || "-"}
                    </FormText>
                  </Column>
                </Authorization>
                <Authorization
                ...
                >
                  <Column small={6} medium={3} large={1}>
                    <FormTextTitle>{PlotSearchFieldTitles.CLOCK}</FormTextTitle>
                    <FormText>
                      {getHoursAndMinutes(plotSearch.begin_at) || "-"}
                    </FormText>
                  </Column>
                </Authorization>
                <Authorization
                ...
                >
                  <Column small={6} medium={3} large={1}>
                    <FormTextTitle>{PlotSearchFieldTitles.CLOCK}</FormTextTitle>
                    <FormText>
                      {getHoursAndMinutes(plotSearch.end_at) || "-"}
                    </FormText>
                  </Column>
                </Authorization>
                <Authorization
                ...
                >
                  <Column small={6} medium={6} large={2}>
                    <FormTextTitle>{PlotSearchFieldTitles.STAGE}</FormTextTitle>
                    <FormText>
                ...
                <Authorization
                  allow={isFieldAllowedToRead(attributes, "modified_at")}
                >
                  <Column small={12} medium={6} large={2}>
                    <FormTextTitle
                      uiDataKey={getUiDataPlotSearchKey("modified_at")}
                    >
              ...
              {(!plotSearch.decisions || plotSearch.decisions.length === 0) && (
                <FormText>Ei valittuja päätöksiä.</FormText>
              )}
              {!!plotSearch.decisions &&
                plotSearch.decisions.map((decision, index) => (
                  <Row key={index}>
                    <Column
                      small={12}
                      medium={6}
                      large={6}
                      aria-labelledby="plotSearchDecisionTable__decision-header"
                    >
                      <FormText key={index}>
                        <ExternalLink
                          className="no-margin"
  ...
  (state) => {
    return {
      ...
      basicInformationCollapseState: getCollapseStateByKey(
        state,
        `${ViewModes.READONLY}.${FormNames.PLOT_SEARCH_BASIC_INFORMATION}.basic_information`,
      ),
