...
import { getAttributes, getCollapseStateByKey, getIsFetchingCustomDetailedPlanAttributes, getRelatedApplications } from "/src/plotSearch/selectors";
...
import PlotSearchReservationRecipients from "/src/plotSearch/components/plotSearchSections/basicInfo/PlotSearchReservationRecipients";
type OwnProps = {
...
class PlotSearchSiteCustomDetailedPlan extends PureComponent<Props, State> {
  ...
  render() {
    ...
    return <Column large={12}>
        <Collapse className='collapse__secondary greenCollapse' defaultOpen={collapseState !== undefined ? collapseState : true} headerTitle={get(currentCustomDetailedPlan, 'identifier') ? `${get(currentCustomDetailedPlan, 'identifier')} ${get(currentCustomDetailedPlan, 'state.name')} - OMA MUU ALUE` : '-'} onToggle={this.handleCollapseToggle}>
          <Row style={{
          marginBottom: 10
        }}>
            {isFetchingCustomDetailedPlanAttributes && <LoaderWrapper className='relative-overlay-wrapper'><Loader isLoading={true} /></LoaderWrapper>}
              ...
              <Column small={12} medium={12} large={2}>
                <Row>
                  ...
                  <Column small={8} medium={8} large={12}>
                    ...
                    <FormText>
                      {leaseIdentifier ? <ExternalLink className='no-margin' href={`${getRouteById(Routes.LEASES)}?search=${leaseIdentifier}`} text={leaseIdentifier || '-'} /> : <FormText>
                          -
                        </FormText>}
                    </FormText>
                  </Column>
              ...
              <Column small={12} medium={12} large={3}>
                <Row>
                  ...
                  <Column small={4} medium={4} large={12}>
                    ...
                    <FormText>
                      {reservationIdentifier ? <ExternalLink className='no-margin' href={`${getRouteById(Routes.LEASES)}?search=${reservationIdentifier}`} text={reservationIdentifier} /> : '-'}
                    </FormText>
                  </Column>
                  <Column small={8} medium={8} large={12}>
