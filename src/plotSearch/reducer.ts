...
const attributesReducer: Reducer<Attributes> = handleActions(
  {
    ["mvj/plotSearch/RECEIVE_ATTRIBUTES"]: (
      state: Attributes,
...
const pendingPlanUnitFetchesReducer: Reducer<Array<number>> = handleActions(
  {
    ["mvj/plotSearch/FETCH_PLAN_UNIT"]: (state: any, { payload: value }) => {
    ...
    ["mvj/plotSearch/PLAN_UNIT_NOT_FOUND"]: (
      state: Array<number>,
      { payload: id },
    ) => state.filter((item) => item === id),
    ["mvj/plotSearch/RECEIVE_PLAN_UNIT"]: (
      state: Array<number>,
      { payload: result },
...
const planUnitAttributesReducer: Reducer<Attributes> = handleActions(
  {
    ["mvj/plotSearch/RECEIVE_PLAN_UNIT_ATTRIBUTES"]: (
      state: Attributes,
      { payload: attributes }: ReceiveAttributesAction,
    ) => {
...
const customDetailedPlanAttributesReducer: Reducer<Attributes> = handleActions(
  {
    ["mvj/plotSearch/RECEIVE_CUSTOM_DETAILED_PLAN_ATTRIBUTES"]: (
      state: Attributes,
      { payload: attributes }: ReceiveAttributesAction,
    ) => {
      return merge(state, attributes);
...
const customDetailedPlanReducer: Reducer<CustomDetailedPlan> = handleActions(
  {
    ["mvj/plotSearch/RECEIVE_CUSTOM_DETAILED_PLAN"]: (
...
const pendingCustomDetailedPlanFetchesReducer: Reducer<Array<number>> =
  handleActions(
    {
      ...
      ["mvj/plotSearch/CUSTOM_DETAILED_PLAN_NOT_FOUND"]: (
        state: Array<number>,
        { payload: id },
      ) => state.filter((item) => item === id),
      ["mvj/plotSearch/RECEIVE_CUSTOM_DETAILED_PLAN"]: (
...
const isFetchingCustomDetailedPlanAttributesReducer: Reducer<boolean> =
  handleActions(
    {
...
const methodsReducer: Reducer<Methods> = handleActions(
  {
    ["mvj/plotSearch/RECEIVE_METHODS"]: (
      state: Methods,
...
const plotSearchListReducer: Reducer<PlotSearchList> = handleActions(
  {
    ["mvj/plotSearch/RECEIVE_ALL"]: (
      state: PlotSearchList,
      { payload: list }: ReceivePlotSearchListAction,
    ) => list,
  },
  {},
);
const currentPlotSearchReducer: Reducer<PlotSearch> = handleActions(
  {
    ["mvj/plotSearch/RECEIVE_SINGLE"]: (
      state: PlotSearch,
      { payload: plotSearch }: ReceiveSinglePlotSearchAction,
    ) => plotSearch,
  },
  {},
);
const collapseStatesReducer: Reducer<Record<string, any>> = handleActions(
  {
    ["mvj/plotSearch/RECEIVE_COLLAPSE_STATES"]: (
...
const isSaveClickedReducer: Reducer<boolean> = handleActions(
  {
    ["mvj/plotSearch/RECEIVE_SAVE_CLICKED"]: (
      state: boolean,
...
const isFormValidByIdReducer: Reducer<Record<string, any>> = handleActions(
  {
    ["mvj/plotSearch/RECEIVE_FORM_VALID_FLAGS"]: (
      state: Record<string, any>,
...
const templateFormsReducer: Reducer<Record<string, any>> = handleActions(
  {
    ["mvj/plotSearch/RECEIVE_TEMPLATE_FORMS"]: (
      state: Record<string, any>,
      { payload: forms }: ReceiveTemplateFormsAction,
    ) => {
      return forms;
    },
  },
...
const formReducer: Reducer<Record<string, any>> = handleActions(
  {
    ["mvj/plotSearch/RECEIVE_FORM"]: (
      state: Record<string, any>,
      { payload: form }: ReceiveFormAction,
    ) => {
      return form;
    },
  },
...
const decisionCandidateReducer: Reducer<Record<string, any>> = handleActions(
  {
    ["mvj/plotSearch/ADD_PLAN_UNIT_DECISIONS"]: (
      state: Record<string, any>,
      { payload: planUnit }: AddPlanUnitDecisionsAction,
    ) => {
      ...
      return {
        ...state,
        [planUnit.id]:
          planUnit.decisions?.map((decision) =>
            annotatePlanUnitDecision(decision, planUnit),
          ) || [],
      };
    },
    ["mvj/plotSearch/REMOVE_PLAN_UNIT_DECISIONS"]: (
      state: Record<string, any>,
      { payload: planUnitId }: RemovePlanUnitDecisionsAction,
    ) => {
      if (!state[planUnitId]) {
        return state;
      }
...
const stagesReducer: Reducer<Array<Record<string, any>>> = handleActions(
  {
    ...
    ["mvj/plotSearch/RECEIVE_PLOT_SEARCH_STAGES"]: (
      state,
      { payload: stages }: ReceivePlotSearchStagesAction,
    ) => stages,
  },
  [],
);
...
const isBatchCreatingReservationIdentifiersReducer: Reducer<boolean> =
  handleActions(
    {
...
const lastBatchReservationCreationErrorReducer: Reducer<any> = handleActions(
  {
    ...
    ["mvj/plotSearch/RESERVATION_IDENTIFIERS_CREATION_FAILED"]: (
      state,
      { payload: errors },
    ) => errors,
  },
...
const isFetchingReservationIdentifierUnitListsReducer: Reducer<boolean> =
  handleActions(
    {
...
const reservationIdentifierUnitListsReducer: Reducer<Record<
  string,
  any
> | null> = handleActions(
  {
    ["mvj/plotSearch/FETCH_RESERVATION_IDENTIFIER_UNIT_LISTS"]: () => null,
    ["mvj/plotSearch/RECEIVE_RESERVATION_IDENTIFIER_UNIT_LISTS"]: (
      state,
      { payload },
    ) => payload,
...
const sectionEditorCollapseStatesReducer: Reducer<Record<string, boolean>> =
  handleActions(
    {
      ["mvj/plotSearch/CLEAR_SECTION_EDITOR_COLLAPSE_STATES"]: () => ({}),
      ["mvj/plotSearch/SET_SECTION_EDITOR_COLLAPSE_STATE"]: (
        state: Record<string, boolean>,
        { payload }: any,
      ) => ({ ...state, [payload.key]: payload.state }),
      ["mvj/plotSearch/INITIALIZE_SECTION_EDITOR_COLLAPSE_STATES"]: (
        state,
        { payload },
      ) => ({ ...state, ...payload }),
    },
...
const relatedApplicationsReducer: Reducer<Array<Record<string, any>>> =
  handleActions(
    {
      ["mvj/plotSearch/FETCH_RELATED_APPLICATIONS"]: () => [],
      ["mvj/plotSearch/RECEIVE_RELATED_APPLICATIONS"]: (
...
export default combineReducers<Record<string, any>, any>({
  ...
  isFetchingCustomDetailedPlanAttributes:
    isFetchingCustomDetailedPlanAttributesReducer,
  ...
  isBatchCreatingReservationIdentifiers:
    isBatchCreatingReservationIdentifiersReducer,
  lastBatchReservationCreationError: lastBatchReservationCreationErrorReducer,
  isFetchingReservationIdentifierUnitLists:
    isFetchingReservationIdentifierUnitListsReducer,
