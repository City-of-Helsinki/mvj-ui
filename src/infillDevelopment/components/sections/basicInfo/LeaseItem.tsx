...
import { InfillDevelopmentCompensationLeasesFieldPaths, InfillDevelopmentCompensationLeasesFieldTitles, InfillDevelopmentCompensationLeaseDecisionsFieldPaths, InfillDevelopmentCompensationLeaseDecisionsFieldTitles, InfillDevelopmentCompensationLeaseIntendedUsesFieldPaths, InfillDevelopmentCompensationLeaseIntendedUsesFieldTitles } from "infillDevelopment/enums";
import { InfillDevelopmentCompensationAttachmentFieldPaths, InfillDevelopmentCompensationAttachmentFieldTitles } from "infillDevelopmentAttachment/enums";
import { LeaseFieldPaths } from "leases/enums";
import { getContentLeaseAreas, getContentLeaseIdentifier, getContentTenants } from "leases/helpers";
...
import { getAttributes as getInfillDevelopmentAttributes, getCollapseStateByKey } from "infillDevelopment/selectors";
import { getAttributes as getInfillDevelopmentAttachmentAttributes, getMethods as getInfillDevelopmentAttachmentMethods } from "infillDevelopmentAttachment/selectors";
import { getAttributes as getLeaseAttributes, getIsFetchingById, getLeaseById } from "leases/selectors";
...
class LeaseItem extends PureComponent<Props, State> {
  ...
  getTotalCompensation = (lease: Record<string, any>) => {
  ...
  render() {
    ...
    return <Collapse className='collapse__secondary' defaultOpen={collapseState !== undefined ? collapseState : true} headerTitle={<Authorization allow={isFieldAllowedToRead(leaseAttributes, LeaseFieldPaths.IDENTIFIER)}>
            {identifier || '-'}
          </Authorization>} onToggle={this.handleCollapseToggle}>
        ...
        <Authorization allow={isFieldAllowedToRead(infillDevelopmentAttributes, InfillDevelopmentCompensationLeaseDecisionsFieldPaths.DECISIONS)}>
          ...
          {!!decisions.length && <ListItems>
              ...
              {decisions.map((decision, index) => <Row key={index}>
                  <Column small={3} large={2}>
                    <Authorization allow={isFieldAllowedToRead(infillDevelopmentAttributes, InfillDevelopmentCompensationLeaseDecisionsFieldPaths.DECISION_MAKER)}>
                  ...
                  <Column small={3} large={2}>
                    <Authorization allow={isFieldAllowedToRead(infillDevelopmentAttributes, InfillDevelopmentCompensationLeaseDecisionsFieldPaths.REFERENCE_NUMBER)}>
                      {decision.reference_number ? <ListItem>
                          <ExternalLink className='no-margin' href={getReferenceNumberLink(decision.reference_number)} text={decision.reference_number} />
        ...
        <Authorization allow={isFieldAllowedToRead(infillDevelopmentAttributes, InfillDevelopmentCompensationLeaseIntendedUsesFieldPaths.INTENDED_USES)}>
          ...
          {!!intendedUses.length && <ListItems>
              ...
              {intendedUses.map((intendedUse, index) => <Row key={index}>
                  <Column small={3} large={2}>
                    <Authorization allow={isFieldAllowedToRead(infillDevelopmentAttributes, InfillDevelopmentCompensationLeaseIntendedUsesFieldPaths.INTENDED_USE)}>
