...
import { getSplittedDateRangesWithItems, sortByStartAndEndDateDesc } from "/src/util/date";
...
import { addEmptyOption, convertStrToDecimalNumber, fixedLengthNumber, formatDate, formatDateRange, getApiResponseResults, getFieldOptions, getLabelOfOption, isDecimalNumberStr, isEmptyValue, isActive, isActiveOrFuture, isArchived, sortStringAsc, sortStringByKeyAsc, sortStringByKeyDesc } from "/src/util/helpers";
import { getCoordinatesOfGeometry } from "/src/util/map";
...
export const isLeaseEmpty = (lease: Lease): boolean => {
  let empty = true;
  const skipFields = ['id', 'type', 'municipality', 'district', 'identifier', 'state', 'preparer', 'note', 'created_at', 'modified_at'];
 ...
 * @return {boolean}
 */
export const isLeaseCreatedByUser = (lease: Lease, user: Record<string, any>): boolean => {
 ...
 * @return {boolean}
 */
export const isUserAllowedToDeleteEmptyLease = (lease: Lease, comments: CommentList, user: Record<string, any>): boolean => isLeaseEmpty(lease) && comments && !comments.length && isLeaseCreatedByUser(lease, user);
 ...
 * @returns {string}
 */
export const getTitleText = (text: string | null | undefined, maxLength: number): string => {
  if (text) {
...
export const getContentLeaseListLeaseAddresses = (lease: Record<string, any>): Array<Record<string, any>> => {
  ...
  const sortedAddresses = addresses.filter((address, index, self) => self.indexOf(address) == index).sort(sortStringAsc);
 ...
 * @returns {Object[]}
 */
const getContentLeaseInfillDevelopmentCompensations = (lease: Lease): Array<Record<string, any>> => get(lease, 'infill_development_compensations', []).map(item => {
  return {
    id: item.id,
    name: item.name
  };
...
export const getContentLeaseSummary = (lease: Record<string, any>): Record<string, any> => {
  return {
    ...
    contract_numbers: get(lease, 'contracts', []).filter(contract => contract.contract_number).map(contract => contract.contract_number).join(', '),
...
/**
  ...
  * @returns {Object}
  */
export const getContentRelatedLease = (content: Record<string, any>, path: string = 'from_lease'): Record<string, any> => get(content, path, {});
 ...
 * @returns {Object}
 */
export const getLeaseAreaById = (lease: Lease, id: number | null | undefined): Record<string, any> | null | undefined => id ? getContentLeaseAreas(lease).find(area => area.id === id) : null;
 ...
 * @returns {Object[]}
 */
export const getContentDecisionConditions = (decision: Record<string, any>): Array<Record<string, any>> => get(decision, 'conditions', []).map(condition => {
  return {
    id: condition.id,
 ...
 * @returns {Object}
 */
export const getContentDecision = (decision: Record<string, any>): Record<string, any> => {
  return {
    id: decision.id,
 ...
 * @return {Object[]};
 */
export const getDecisionOptions = (lease: Lease): Array<Record<string, any>> => {
 ...
 * @returns {Object}
 */
export const getDecisionById = (lease: Lease, id: number | null | undefined): Record<string, any> | null | undefined => getContentDecisions(lease).find(decision => decision.id === id);
 ...
 * @returns {Object[]}
 */
export const getContentContractChanges = (contract: Record<string, any>): Array<Record<string, any>> => get(contract, 'contract_changes', []).map(change => {
  return {
    id: change.id,
 ...
 * @returns {Object[]}
 */
export const getContentContractCollaterals = (contract: Record<string, any>): Array<Record<string, any>> => get(contract, 'collaterals', []).map(collateral => {
  return {
    id: collateral.id,
 ...
 * @returns {Object}
 */
export const getContentContract = (contract: Record<string, any>): Record<string, any> => {
  return {
    id: contract.id,
 ...
 * @returns {Object}
 */
export const getContentInspection = (inspection: Record<string, any>): Record<string, any> => {
  return {
    id: inspection.id,
 ...
 * @returns {Object[]}
 */
export const getContentEmailLogs = (lease: Lease): Array<Record<string, any>> => {
 ...
 * @returns {Object}
 */
export const getContentInvoiceNote = (invoiceNote: Record<string, any>): Record<string, any> => ({
  id: invoiceNote.id,
 ...
 * @returns {Object[]}
 */
export const getContentInvoiceNotes = (lease: Lease): Array<Record<string, any>> => get(lease, 'invoice_notes', []).map(note => getContentInvoiceNote(note));
 ...
 * @returns {Object[]}
 */
export const getContentTenantRentShares = (tenant: Record<string, any>): Array<Record<string, any>> => {
 ...
 * @returns {Object}
 */
export const getContentContactDetails = (contact: Record<string, any>): Record<string, any> => {
  return {
    id: contact.id,
    type: contact.type,
 ...
 * @returns {Object}
 */
export const getContentTenant = (tenant: Record<string, any>): Record<string, any> => {
 ...
 * @returns {Object[]}
 */
export const getContentTenantBillingPersons = (tenant: Record<string, any>): Array<Record<string, any>> => get(tenant, 'tenantcontact_set', []).filter(x => x.type === TenantContactType.BILLING).map(contact => contact ? getContentContactDetails(contact) : {}).sort((a, b) => sortStringByKeyDesc(a, b, 'start_date'));
 ...
 * @returns {Object[]}
 */
export const getContentTenantContactPersons = (tenant: Record<string, any>): Array<Record<string, any>> => get(tenant, 'tenantcontact_set', []).filter(x => x.type === TenantContactType.CONTACT).map(contact => contact ? getContentContactDetails(contact) : {}).sort((a, b) => sortStringByKeyDesc(a, b, 'start_date'));
 ...
 * @returns {string[]}
 */
export const getTenantShareWarnings = (tenants: Array<Record<string, any>>): Array<string> => {
...
/**
  ...
  * @returns {string[]}
  */
export const getTenantRentShareWarnings = (tenants: Array<Record<string, any>>, leaseAttributes: Attributes): Array<string> => {
  const dateRanges = getSplittedDateRangesWithItems(tenants, 'tenant.start_date', 'tenant.end_date');
  const warnings = [];
 ...
 * @returns {Object[]}
 */
export const getContentPayableRents = (rent: Record<string, any>): Array<Record<string, any>> => get(rent, 'payable_rents', []).map(item => {
  return {
    id: item.id,
    amount: item.amount,
 ...
 * @returns {Object[]}
 */
export const getContentEqualizedRents = (rent: Record<string, any>): Array<Record<string, any>> => get(rent, 'equalized_rents', []).map(item => {
  return {
    id: item.id,
    start_date: item.start_date,
    end_date: item.end_date,
...
export const isRentDeleted = (props: Record<string, any>): boolean => {
  return typeof props.subventionType === "undefined" && typeof props.subventionBasePercent === "undefined" && typeof props.subventionGraduatedPercent === "undefined" && typeof props.managementSubventions === "undefined" && typeof props.temporarySubventions === "undefined";
};
 ...
 * @return {boolean}
 */
export const hasSubventionDataChanged = (prevProps: Record<string, any>, props: Record<string, any>): boolean => {
  if (isRentDeleted(props)) {
    return false;
  }

  return props?.subventionType !== prevProps?.subventionType || props?.subventionBasePercent !== prevProps?.subventionBasePercent || props?.subventionGraduatedPercent !== prevProps?.subventionGraduatedPercent || !isEqual(props?.managementSubventions, prevProps?.managementSubventions) || !isEqual(props?.temporarySubventions, prevProps?.temporarySubventions);
};
 ...
 * @return {number}
 */
export const calculateReLeaseDiscountPercent = (subventionBasePercent: string | null | undefined, subventionGraduatedPercent: string | null | undefined): number => {
 ...
 * @return {string}
 */
export const getBasisOfRentIndexValue = (basisOfRent: Record<string, any>, indexOptions: Array<Record<string, any>>): string | null | undefined => {
...
export const calculateBasisOfRentBasicAnnualRent = (basisOfRent: Record<string, any>, indexValue?: string | null | undefined): number => {
  ...
  return Number(convertStrToDecimalNumber(basisOfRent.amount_per_area)) * Number(convertStrToDecimalNumber(basisOfRent.area)) * Number(isDecimalNumberStr(basisOfRent.profit_margin_percentage) ? Number(convertStrToDecimalNumber(basisOfRent.profit_margin_percentage)) / 100 : 0);
};
 ...
 * @return {number}
 */
export const getBasisOfRentAmountPerArea = (basisOfRent: Record<string, any>, indexValue: string | null | undefined): number => {
  if (basisOfRent.type === CalculatorTypes.LEASE2022) {
 ...
 * @return {number}
 */
export const calculateBasisOfRentAmountPerArea = (basisOfRent: Record<string, any>, indexValue: string | null | undefined): number => {
  if (!isDecimalNumberStr(indexValue) || !isDecimalNumberStr(basisOfRent.amount_per_area)) return 0;
  return roundToFixed(Number(convertStrToDecimalNumber(indexValue)) / 100 * Number(convertStrToDecimalNumber(basisOfRent.amount_per_area)), 2);
};
 ...
 * @return {number}
 */
export const calculateAmountFromValue = (value: string, indexValue: string | null | undefined): number => {
  if (!isDecimalNumberStr(indexValue) || !isDecimalNumberStr(value)) return 0;
  return roundToFixed(Number(convertStrToDecimalNumber(value)) / Number(convertStrToDecimalNumber(indexValue)) * 100, 2);
};
 ...
 * @return {number}
 */
export const calculateBasisOfRentDiscountedInitialYearRentsTotal = (basisOfRents: Record<string, any>[], indexOptions: Record<string, any>[]): number => {
  if (basisOfRents) return Number(basisOfRents.map(basisOfRent => calculateBasisOfRentDiscountedInitialYearRent(basisOfRent, getBasisOfRentIndexValue(basisOfRent, indexOptions))).reduce((sum, cur) => sum + cur));else return 0;
};
 ...
 * @return {number}
 */
export const calculateInitialYearRentsTotal = (basisOfRents: Record<string, any>[], indexOptions: Record<string, any>[]): number => {
  if (basisOfRents) {
 ...
 * @return {number}
 */
export const calculateBasisOfRentDiscountedInitialYearRent = (basisOfRent: Record<string, any>, indexValue: string | null | undefined): number => {
  const basicAnnualRent = calculateBasisOfRentBasicAnnualRent(basisOfRent);
 ...
 * @return {number}
 */
export const calculateBasisOfRentTotalDiscountedInitialYearRent = (basisOfRents: Array<Record<string, any>>, indexOptions: Array<Record<string, any>>): number | null | undefined => {
 ...
 * @return {string}
 */
export const calculateTemporarySubventionDiscountPercentage = (temporarySubventions: Record<string, any>): string => {
  let base = 1;
  temporarySubventions && temporarySubventions.forEach(temporarySubvention => {
    if (temporarySubvention.subvention_percent) base = base * ((100 - parseFloat(temporarySubvention.subvention_percent.replace(',', '.'))) / 100);
  });
 ...
 * @return {number}
 */
export const calculateRentAdjustmentSubventionPercent = (subventionType: string | null | undefined, subventionBasePercent: string | null | undefined, subventionGraduatedPercent: string | null | undefined, managementSubventions: Array<Record<string, any>> | null | undefined, temporarySubventions: Array<Record<string, any>> | null | undefined): number => {
  let discount = 0;
 ...
 * @return {number}
 */
export const calculateBasisOfRentSubventionPercent = (currentAmountPerArea: number, subventionType: string | null | undefined, subventionBasePercent: string | null | undefined, subventionGraduatedPercent: string | null | undefined, managementSubventions: Array<Record<string, any>> | null | undefined, temporarySubventions: Array<Record<string, any>> | null | undefined): number => {
  let discount = 1;
 ...
 * @return {number}
 */
export const calculateRentAdjustmentSubventionPercentCumulative = (subventionType: string | null | undefined, subventionBasePercent: string | null | undefined, subventionGraduatedPercent: string | null | undefined, managementSubventions: Array<Record<string, any>> | null | undefined, temporarySubventions: Array<Record<string, any>> | null | undefined): number => {
  let discount = 1;
 ...
 * @return {boolean}
 */
export const isSubventionTypeSpecified = (subventionType: string | null | undefined): boolean => {
 ...
 * @return {boolean}
 */
export const hasSubventionValues = (managementSubventions: Array<Record<string, any>> | null | undefined, temporarySubventions: Array<Record<string, any>> | null | undefined, subventionBasePercent: string | null | undefined, subventionGraduatedPercent: string | null | undefined): boolean => {
 ...
 * @return {number}
 */
export const calculateTemporaryRent = (price: number | null | undefined, area: number | null | undefined): number => {
  if (!price || !area) return 0;
  const areaDecimal = Number(convertStrToDecimalNumber(area));
  if (areaDecimal > 2000) return Number(price * 2000 + price * 0.5 * (areaDecimal - 2000));else return Number(price * areaDecimal);
};
 ...
 * @return {number}
 */
export const calculateBasicAnnualRentIndexed = (rent: number | null | undefined, indexValue: string | null | undefined): number => {
 ...
 * @return {number}
 */
export const calculateExtraRent = (price: string | null | undefined, area: number | null | undefined): number => {
  if (!price || !area) return 0;
 ...
 * @return {number}
 */
export const calculateFieldsRent = (price: string | null | undefined, area: number | null | undefined): number => {
  if (!price || !area) return 0;
 ...
 * @return {number}
 */
export const calculateRackPrice = (numberOfRacks: number | null | undefined): number => {
 ...
 * @return {number}
 */
export const calcluateHightPrice = (height: number | null | undefined): number => {
  if (!height) return 0;
 ...
 * @return {number}
 */
export const calculateRackAndHeightPrice = (children: Record<string, any> | null | undefined): number => {
  let total = 0;
  if (children && children[0] && children[0].area) total = total + calculateRackPrice(Number(convertStrToDecimalNumber(children[0].area)));
  if (children && children[1] && children[1].area) total = total + calcluateHightPrice(Number(convertStrToDecimalNumber(children[1].area)));
  return total;
};
 ...
 * @return {Object[]}
 */
export const getContentManagementSubventions = (rentAdjustment: Record<string, any>): Array<Record<string, any>> => get(rentAdjustment, 'management_subventions', []).map(item => {
  return {
    id: item.id,
 ...
 * @return {Object[]}
 */
export const getContentTemporarySubventions = (rentAdjustment: Record<string, any>): Array<Record<string, any>> => get(rentAdjustment, 'temporary_subventions', []).map(item => {
  return {
    id: item.id,
 ...
 * @return {Object[]}
 */
export const getContentRentAdjustments = (rent: Record<string, any>): Array<Record<string, any>> => get(rent, 'rent_adjustments', []).map(item => {
  return {
    id: item.id,
    type: item.type,
 ...
 * @return {Object[]}
 */
export const getContentIndexAdjustedRents = (rent: Record<string, any>): Array<Record<string, any>> => get(rent, 'index_adjusted_rents', []).map(item => {
  return {
    item: item.id,
    amount: item.amount,
    intended_use: get(item, 'intended_use.id') || item.intended_use,
 ...
 * @return {Object[]}
 */
export const getContentContractRents = (rent: Record<string, any>): Array<Record<string, any>> => get(rent, 'contract_rents', []).map(item => {
  return {
    id: item.id,
    amount: item.amount,
    period: item.period,
 ...
 * @return {Object[]}
 */
export const getContentFixedInitialYearRents = (rent: Record<string, any>): Array<Record<string, any>> => get(rent, 'fixed_initial_year_rents', []).map(item => {
  return {
    id: item.id,
    intended_use: get(item, 'intended_use.id') || item.intended_use,
    amount: item.amount,
 ...
 * @return {Object[]}
 */
export const getContentRentDueDate = (rent: Record<string, any>, path: string = 'due_dates'): Array<Record<string, any>> => get(rent, path, []).map(date => ({
  id: date.id,
  day: date.day,
  month: date.month
}));
 ...
 * @returns {string[]}
 */
export const getRentWarnings = (rents: Array<Record<string, any>>): Array<string> => {
  const warnings = [];
  rents.forEach(rent => {
...
export const getInvoiceRecipientOptions = (lease: Record<string, any>, addAll: boolean, addTenants: boolean): Array<Record<string, any>> => {
  ...
  if (addTenants) {
    recipients.push(...items.filter(item => isActiveOrFuture(item.tenant)).map(item => {
      return {
        value: get(item, 'id'),
 ...
 * @returns {Object[]}
 */
export const getContentLeasesFeatures = (leases: Array<Record<string, any>>): Array<LeafletFeature> => {
 ...
 * @returns {Object}
 */
export const getContentLeasesGeoJson = (leases: Array<Record<string, any>>): LeafletGeoJson => {
 ...
 * @returns {Object[]}
 */
export const getContentLeaseAreasFeatures = (areas: Array<Record<string, any>>): Array<LeafletFeature> => {
 ...
 * @returns {Object[]}
 */
export const getContentLeasePlotsFeatures = (plots: Array<Record<string, any>>): Array<LeafletFeature> => {
...
export const getContentPlotsGeoJson = (lease: Lease, inContract: boolean = false): LeafletGeoJson => {
  const plots = [];
  get(lease, 'lease_areas', []).filter(area => !area.archived_at).forEach(area => {
    plots.push(...get(area, 'plots', []).filter(plot => plot.in_contract === inContract));
  });
 ...
 * @returns {Object[]}
 */
export const getContentPlanUnitFeatures = (planUnits: Array<Record<string, any>>): Array<LeafletFeature> => {
...
export const getContentPlanUnitsGeoJson = (lease: Lease, inContract: boolean = false): LeafletGeoJson => {
  const planUnits = [];
  get(lease, 'lease_areas', []).filter(area => !area.archived_at).forEach(area => {
    planUnits.push(...get(area, 'plan_units', []).filter(planUnit => planUnit.in_contract === inContract));
  });
 ...
 * @returns {Object[]}
 */
export const getLeaseCoordinates = (lease: Lease): Array<Record<string, any>> => {
  const areas = get(lease, 'lease_areas', []).filter(area => !area.archived_at);
  let coordinates = [];
  areas.forEach(area => {
 ...
 * @returns {Object}
 */
export const addSummaryFormValuesToPayload = (payload: Record<string, any>, formValues: Record<string, any>): Record<string, any> => {
  return { ...payload,
 ...
 * @returns {Object[]}
 */
const getPayloadLeaseAreaAddresses = (area: Record<string, any>) => get(area, 'addresses', []).map(address => {
  return {
    id: address.id,
    address: address.address,
    postal_code: address.postal_code,
 ...
 * @returns {Object[]}
 */
const getPayloadPlots = (area: Record<string, any>): Array<Record<string, any>> => {
 ...
 * @returns {Object[]}
 */
const getPayloadPlanUnits = (area: Record<string, any>): Array<Record<string, any>> => {
 ...
 * @returns {Object}
 */
export const addAreasFormValuesToPayload = (payload: Record<string, any>, values: Record<string, any>): Record<string, any> => {
 ...
 * @returns {Object}
 */
export const addDecisionsFormValuesToPayload = (payload: Record<string, any>, formValues: Record<string, any>): Record<string, any> => {
 ...
 * @returns {Object[]}
 */
const getPayloadContractChanges = (contract: Record<string, any>): Array<Record<string, any>> => {
 ...
 * @returns {Object[]}
 */
const getPayloadCollaterals = (contract: Record<string, any>): Array<Record<string, any>> => {
 ...
 * @returns {Object}
 */
export const addContractsFormValuesToPayload = (payload: Record<string, any>, formValues: Record<string, any>): Record<string, any> => {
 ...
 * @returns {Object}
 */
export const addInspectionsFormValuesToPayload = (payload: Record<string, any>, formValues: Record<string, any>): Record<string, any> => {
 ...
 * @returns {Object[]}
 */
export const getPayloadConstructabilityDescriptions = (area: Record<string, any>): Array<Record<string, any>> => {
  const descriptionsPreconstruction = get(area, 'descriptionsPreconstruction', []).map(description => {
    return { ...description,
      'type': ConstructabilityType.PRECONSTRUCTION
    };
  });
  const descriptionsDemolition = get(area, 'descriptionsDemolition', []).map(description => {
    return { ...description,
      'type': ConstructabilityType.DEMOLITION
    };
  });
  const descriptionsPollutedLand = get(area, 'descriptionsPollutedLand', []).map(description => {
    return { ...description,
      'type': ConstructabilityType.POLLUTED_LAND
    };
  });
  const descriptionsReport = get(area, 'descriptionsReport', []).map(description => {
    return { ...description,
      'type': ConstructabilityType.REPORT
    };
  });
  const descriptionsOther = get(area, 'descriptionsOther', []).map(description => {
    return { ...description,
      'type': ConstructabilityType.OTHER
    };
  });
  const descriptions = [...descriptionsPreconstruction, ...descriptionsDemolition, ...descriptionsPollutedLand, ...descriptionsReport, ...descriptionsOther];
 ...
 * @returns {Object}
 */
export const addConstructabilityFormValuesToPayload = (payload: Record<string, any>, formValues: Record<string, any>): Record<string, any> => {
 ...
 * @returns {Object[]}
 */
export const getPayloadTenantRentShares = (tenant: Record<string, any>): Array<Record<string, any>> => {
  const rentShares = get(tenant, 'rent_shares', []);
  return rentShares.map(rentShare => ({
    id: rentShare.id,
 ...
 * @returns {Object[]}
 */
export const getPayloadTenantContactSet = (tenant: Record<string, any>): Array<Record<string, any>> => {
  const contacts = [];
 ...
 * @returns {Object}
 */
export const addTenantsFormValuesToPayload = (payload: Record<string, any>, formValues: Record<string, any>): Record<string, any> => {
 ...
 * @return {Object[]}
 */
export const getPayloadManagementSubventions = (rentAdjustment: Record<string, any>): Array<Record<string, any>> => get(rentAdjustment, 'management_subventions', []).map(item => {
  return {
    id: item.id,
 ...
 * @return {Object[]}
 */
export const getPayloadTemporarySubventions = (rentAdjustment: Record<string, any>): Array<Record<string, any>> => get(rentAdjustment, 'temporary_subventions', []).map(item => {
  return {
    id: item.id,
    description: item.description,
 ...
 * @return {Object[]}
 */
export const getPayloadChildren = (rentAdjustment: Record<string, any>): Array<Record<string, any>> => get(rentAdjustment, 'children', []).map(item => {
  return {
    intended_use: 7,
    area_unit: 'm2',
 ...
 * @returns {Object[]}
 */
export const getPayloadRentAdjustments = (rent: Record<string, any>): Array<Record<string, any>> => get(rent, 'rent_adjustments', []).map(item => {
  return {
    id: item.id,
    type: item.type,
 ...
 * @returns {Object[]}
 */
export const getPayloadContractRents = (rent: Record<string, any>, rentType: string): Array<Record<string, any>> => get(rent, 'contract_rents', []).map(item => {
 ...
 * @returns {Object[]}
 */
export const getPayloadFixedInitialYearRents = (rent: Record<string, any>): Array<Record<string, any>> => get(rent, 'fixed_initial_year_rents', []).map(item => {
  return {
    id: item.id,
    intended_use: item.intended_use,
    amount: convertStrToDecimalNumber(item.amount),
 ...
 * @returns {Object[]}
 */
export const getPayloadRentDueDates = (rent: Record<string, any>): Array<Record<string, any>> => {
...
export const areaUnit = (item: Record<string, any>): string => {
  if (item.type === CalculatorTypes.LEASE || item.type === CalculatorTypes.LEASE2022) return item.area_unit;
  return 'm2';
};
...
export const intendedUse = (item: Record<string, any>): number => {
  if (item.type === CalculatorTypes.LEASE || item.type === CalculatorTypes.LEASE2022) return item.intended_use;
  return 7;
};
 ...
 * @returns {Object}
 */
export const getBasisOfRentById = (lease: Lease, id: number | null | undefined): Record<string, any> | null | undefined => {
  if (!id) return null;
 ...
 * @returns {Object}
 */
export const addRentsFormValuesToPayload = (payload: Record<string, any>, formValues: Record<string, any>, currentLease: Lease): Record<string, any> => {
  payload.is_rent_info_complete = formValues.is_rent_info_complete ? true : false;
  const basisOfRents = [...get(formValues, 'basis_of_rents', []), ...get(formValues, 'basis_of_rents_archived', [])];
  ...
  const rents = [...get(formValues, 'rents', []), ...get(formValues, 'rentsArchived', [])];
  payload.rents = rents.map(rent => {
    ...
    if (rent.type === RentTypes.INDEX || rent.type === RentTypes.INDEX2022 || rent.type === RentTypes.MANUAL) {
 ...
 * @returns {number}
 */
export const calculateAreasSum = (areas: Array<Record<string, any>>): number => {
  let areasSum = 0;
 ...
 * @returns {number}
 */
export const getZonePriceFromValue = (zone: string | null | undefined): number => {
  let sum = 0;
 ...
 * @returns {Object}
 */
export const mapLeaseSearchFilters = (query: Record<string, any>): Record<string, any> => {
  const searchQuery = { ...query
  };
  searchQuery.lease_state = isArray(searchQuery.lease_state) ? searchQuery.lease_state : searchQuery.lease_state ? [searchQuery.lease_state] : [];
...
export const isAnyLeaseFormDirty = (state: RootState): boolean => {
  ...
  return isEditMode && (isDirty(FormNames.LEASE_CONSTRUCTABILITY)(state) || isDirty(FormNames.LEASE_CONTRACTS)(state) || isDirty(FormNames.LEASE_DECISIONS)(state) || isDirty(FormNames.LEASE_INSPECTIONS)(state) || isDirty(FormNames.LEASE_AREAS)(state) || isDirty(FormNames.LEASE_RENTS)(state) || isDirty(FormNames.LEASE_SUMMARY)(state) || isDirty(FormNames.LEASE_TENANTS)(state));
};
