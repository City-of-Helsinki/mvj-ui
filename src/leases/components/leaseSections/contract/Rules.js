          ...
          header={
            <Row>
              <Column small={4}>
                ...
                <span className='collapse__header-title'>{get(rule, 'rule_maker')} {get(rule, 'rule_clause')}</span>
              </Column>
              <Column small={4}><span className='collapse__header-subtitle'>{helpers.formatDate(get(rule, 'rule_date'))}</span></Column>
              <Column small={4}><span className='collapse__header-subtitle'>{get(rule, 'rule_type')}</span></Column>
            </Row>
          }
        >
          <Rule rule={rule} />
