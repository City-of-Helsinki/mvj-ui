...
import { LeaseContractChangesFieldPaths, LeaseContractChangesFieldTitles, LeaseContractsFieldPaths, LeaseContractsFieldTitles, LeaseContractCollateralsFieldPaths, LeaseContractCollateralsFieldTitles } from "leases/enums";
...
const ContractItem = ({
  ...
  return <Collapse defaultOpen={contractCollapseState !== undefined ? contractCollapseState : false} headerSubtitles={<Fragment>
          <Column>
        ...
        </Authorization>} onToggle={handleContractCollapseToggle}>
      <Row>
        ...
        <Column small={6} medium={4} large={2}>
          <Authorization allow={isFieldAllowedToRead(attributes, LeaseContractsFieldPaths.CONTRACT_NUMBER)}>
            ...
            <FormText>{contract.contract_number ? <a onClick={handleShowContractFileModal}>{contract.contract_number}</a> : '-'}</FormText>
          </Authorization>
        </Column>
        <Column small={6} medium={4} large={2}>
          <Authorization allow={isFieldAllowedToRead(attributes, LeaseContractsFieldPaths.SIGNING_DATE)}>
      ...
      <Row>
        {contract.is_readjustment_decision && <Column small={6} medium={4} large={2}>
        ...
        <Column small={6} medium={4} large={2}>
          <Authorization allow={isFieldAllowedToRead(attributes, LeaseContractsFieldPaths.KTJ_LINK)}>
            ...
            {contract.institution_identifier ? <KtjLink fileKey='vuokraoikeustodistus' fileName='vuokraoikeustodistus' identifier={contract.institution_identifier} idKey='kohdetunnus' label='Vuokraoikeustodistus' /> : <FormText>-</FormText>}
          </Authorization>
        </Column>
        <Column small={6} medium={4} large={2}>
          <Authorization allow={isFieldAllowedToRead(attributes, LeaseContractsFieldPaths.KTJ_LINK)}>
            <FormTextTitle>{LeaseContractsFieldTitles.ENCUMBRANCE}</FormTextTitle>
            {contract.institution_identifier ? <KtjLink fileKey='rasitustodistus' fileName='rasitustodistus' identifier={contract.institution_identifier} idKey='kohdetunnus' label='Rasitustodistus' /> : <FormText>-</FormText>}
          </Authorization>
        </Column>
      </Row>

      <Authorization allow={isFieldAllowedToRead(attributes, LeaseContractChangesFieldPaths.CONTRACT_CHANGES)}>
        <Collapse className='collapse__secondary' defaultOpen={contractChangesCollapseState !== undefined ? contractChangesCollapseState : true} headerTitle={LeaseContractChangesFieldTitles.CONTRACT_CHANGES} onToggle={handleContractChangesCollapseToggle} uiDataKey={LeaseContractChangesFieldPaths.CONTRACT_CHANGES}>
          {!contract.contract_changes || !contract.contract_changes.length && <FormText>Ei sopimuksen muutoksia</FormText>}
          {contract.contract_changes && !!contract.contract_changes.length && <BoxItemContainer>
              {contract.contract_changes.map(change => {
            const decision = getDecisionById(currentLease, change.decision);
            return <BoxItem key={change.id} className='no-border-on-first-child no-border-on-last-child'>
                    <Row>
                      <Column small={6} medium={4} large={2}>
      ...
      <Authorization allow={isFieldAllowedToRead(attributes, LeaseContractCollateralsFieldPaths.COLLATRALS)}>
        <Collapse className='collapse__secondary' defaultOpen={collateralsCollapseState !== undefined ? collateralsCollapseState : true} headerTitle={LeaseContractCollateralsFieldTitles.COLLATRALS} onToggle={handleCollateralsCollapseToggle} uiDataKey={getUiDataLeaseKey(LeaseContractCollateralsFieldPaths.COLLATRALS)}>
          {!contract.collaterals || !contract.collaterals.length && <FormText>Ei vakuuksia</FormText>}
          {contract.collaterals && !!contract.collaterals.length && <BoxItemContainer>
              {contract.collaterals.map(collateral => {
            return <Collateral key={collateral.id} collateral={collateral} typeOptions={collateralTypeOptions} />;
          })}
            </BoxItemContainer>}
        </Collapse>
      </Authorization>
