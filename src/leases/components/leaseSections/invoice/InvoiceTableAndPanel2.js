...
import InvoicePanel from './InvoicePanel2';
...
const getColumns = (invoiceAttributes: Attributes, invoiceSets: ?InvoiceSetList) => {
...
class InvoiceTableAndPanel extends PureComponent<Props, State> {
  ...
  static getDerivedStateFromProps(props: Props, state: State) {
    ...
    if(props.invoiceAttributes !== state.invoiceAttributes ||
      props.invoiceSets !== state.invoiceSets) {
      newState.invoiceAttributes = props.invoiceAttributes;
      newState.invoiceSets = props.invoiceSets;
      newState.columns = getColumns(props.invoiceAttributes, props.invoiceSets);
    }
  ...
  render() {
    const {invoiceToCredit, usersPermissions} = this.props;
