...
import { InvoiceFieldPaths, InvoiceFieldTitles, InvoiceRowsFieldPaths, InvoiceRowsFieldTitles, InvoiceType } from "src/invoices/enums";
import { UsersPermissions } from "src/usersPermissions/enums";
import { getContactFullName } from "src/contacts/helpers";
import { formatReceivableTypesString, getContentIncoive, getContentInvoices, getPayloadEditInvoice, isInvoiceOverdue } from "src/invoices/helpers";
import { findReactById, formatDate, formatDateRange, formatNumber, getFieldOptions, getLabelOfOption, hasPermissions, isFieldAllowedToRead, sortByOptionsAsc, sortByOptionsDesc, sortNumberByKeyAsc, sortNumberByKeyDesc, sortStringAsc, sortStringDesc } from "src/util/helpers";
...
class InvoiceTableAndPanel extends PureComponent<Props, State> {
  ...
  componentDidUpdate(prevProps: Props, prevState: State) {
    ...
    if (this.props.invoiceAttributes !== prevProps.invoiceAttributes || this.props.invoiceSets !== prevProps.invoiceSets || this.props.usersPermissions !== prevProps.usersPermissions) {
      this.setColumns();
    }
  }
  ...
  getColumns = (invoiceAttributes: Attributes, invoiceSets: InvoiceSetList | null | undefined, usersPermissions: UsersPermissionsType) => {
    ...
    const invoiceSetOptions = invoiceSets ? invoiceSets.map(set => ({ ...set,
      invoiceset: set.id
    })) : [];
    const columns = [];
