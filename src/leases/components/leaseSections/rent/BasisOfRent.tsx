...
const BasisOfRent = ({
  ...
  const getReLeaseDiscountPercent = () => {
    return calculateReLeaseDiscountPercent(basisOfRent.subvention_base_percent, basisOfRent.subvention_graduated_percent);
  };
  ...
  const getTotalSubventionPercent = () => {
    ...
    return calculateBasisOfRentSubventionPercent(currentAmountPerArea, basisOfRent.subvention_type, basisOfRent.subvention_base_percent, basisOfRent.subvention_graduated_percent, basisOfRent.management_subventions, basisOfRent.temporary_subventions);
  };

  const getDiscountPercentage = () => {
    if (basisOfRent.subvention_type === SubventionTypes.FORM_OF_MANAGEMENT) {
      if (basisOfRent.management_subventions && !!basisOfRent.management_subventions.length && basisOfRent.management_subventions[0].subvention_amount) return calculateBasisOfRentSubventionPercentage(basisOfRent.management_subventions[0].subvention_amount, currentAmountPerArea);
      return 0;
    }

    if (basisOfRent.subvention_type === SubventionTypes.RE_LEASE) return getReLeaseDiscountPercent();
  };
  ...
  const getSubventionDiscountedInitial = () => {
    ...
    if (basisOfRent.subvention_type === SubventionTypes.FORM_OF_MANAGEMENT) return calculateSubventionDiscountTotal(initialYearRent, managementSubventions, currentAmountPerArea);
    if (basisOfRent.subvention_type === SubventionTypes.RE_LEASE) return calculateSubventionDiscountTotalFromReLease(initialYearRent, getReLeaseDiscountPercent());
  };
  ...
  const temporaryRent = calculateTemporaryRent(zonePrice, basisOfRent.area);
  ...
  // Yksikköhinta (ind)
  const renderUnitPriceFormField = () => <Authorization allow={isFieldAllowedToRead(leaseAttributes, LeaseBasisOfRentsFieldPaths.AMOUNT_PER_AREA) && isFieldAllowedToRead(leaseAttributes, LeaseBasisOfRentsFieldPaths.INDEX)}>
      <>
  ...
  return <BoxItem className='no-border-on-first-child no-border-on-last-child'>
      {(onUnarchive || onRemove) && <ActionButtonWrapper>
          {onUnarchive && <Authorization allow={isFieldAllowedToEdit(leaseAttributes, LeaseBasisOfRentsFieldPaths.ARCHIVED_AT)}>
              <UnarchiveButton onClick={onUnarchive} />
            </Authorization>}
          {onRemove && !basisOfRent.locked_at && <Authorization allow={isFieldAllowedToEdit(leaseAttributes, LeaseBasisOfRentsFieldPaths.BASIS_OF_RENTS)}>
      ...
      <BoxContentWrapper>
        <Row>
          ...
          {(calculatorType === CalculatorTypes.ADDITIONAL_YARD || calculatorType === CalculatorTypes.FIELD) && <Column small={3} medium={2} large={1}>
            <Authorization allow={isFieldAllowedToRead(leaseAttributes, LeaseBasisOfRentsFieldPaths.AMOUNT_PER_AREA)}>
              <>
              <FormTextTitle>
                {'Hinta'}
        ...
        {(basisOfRent.subvention_type || !!temporarySubventions && !!temporarySubventions.length) && (calculatorType === CalculatorTypes.LEASE || calculatorType === CalculatorTypes.LEASE2022) && <WhiteBox>
            ...
            {basisOfRent.subvention_type === SubventionTypes.FORM_OF_MANAGEMENT && <Authorization allow={isFieldAllowedToRead(leaseAttributes, BasisOfRentManagementSubventionsFieldPaths.MANAGEMENT_SUBVENTIONS)}>
                <>
            ...
            {basisOfRent.subvention_type === SubventionTypes.RE_LEASE && <Row>
                <Column small={4} large={2}>
