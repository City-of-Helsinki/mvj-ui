  ...
  render(){
    ...
    const getContractRentPeriodOptions = () => {
      ...
      return (rentType === RentTypes.INDEX || rentType === RentTypes.INDEX2022)
        ? periodOptions.filter((option) => option.value === ContractRentPeriods.PER_YEAR || option.value === '')
        : periodOptions;
    };
      ...
      <>
        ...
        <Row>
          <Authorization
            ...
            errorComponent={<Column><FormText>{getAmountText()}</FormText></Column>}
          >
            <Column small={6}>
              <Authorization allow={isFieldAllowedToRead(leaseAttributes, LeaseRentContractRentsFieldPaths.AMOUNT)}>
                <FormField
                  ...
                  name={`${field}.amount`}
                  unit='€'
                  overrideValues={{label: rentType !== RentTypes.FIXED
      ...
      <>
        ...
        <Row>
          <Authorization
            ...
            errorComponent={<Column><FormText>{getBaseAmountText()}</FormText></Column>}
          >
            <Column small={6}>
              <Authorization allow={isFieldAllowedToRead(leaseAttributes, LeaseRentContractRentsFieldPaths.BASE_AMOUNT)}>
                <FormField
    ...
    const indexField = (
      <Authorization allow={isFieldAllowedToRead(leaseAttributes, LeaseRentContractRentsFieldPaths.INDEX)}>
        <FormField
          disableTouched={isSaveClicked}
          fieldAttributes={{
            ...getFieldAttributes(leaseAttributes, LeaseRentContractRentsFieldPaths.INDEX),
            required: true
          }}
          name={`${field}.index`}
          overrideValues={{ label: LeaseRentContractRentsFieldTitles.INDEX }}
          enableUiDataEdit
