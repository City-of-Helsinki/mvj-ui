...
const EqualizedRents = ({ equalizedRents, leaseAttributes }: Props) => {
  const getColumns = () => {
    ...
    if (
      isFieldAllowedToRead(
        leaseAttributes,
        LeaseEqualizedRentsFieldPaths.PAYABLE_AMOUNT,
      )
    ) {
      columns.push({
        key: "payable_amount",
    ...
    if (
      isFieldAllowedToRead(
        leaseAttributes,
        LeaseEqualizedRentsFieldPaths.EQUALIZATION_FACTOR,
      )
    ) {
      columns.push({
    ...
    if (
      isFieldAllowedToRead(
        leaseAttributes,
        LeaseEqualizedRentsFieldPaths.EQUALIZED_PAYABLE_AMOUNT,
      )
    ) {
      columns.push({
    ...
    if (
      isFieldAllowedToRead(
        leaseAttributes,
        LeaseEqualizedRentsFieldPaths.START_DATE,
      )
    ) {
      columns.push({
        key: "start_date",
    ...
    if (
      isFieldAllowedToRead(
        leaseAttributes,
        LeaseEqualizedRentsFieldPaths.END_DATE,
      )
    ) {
      columns.push({
        key: "end_date",
