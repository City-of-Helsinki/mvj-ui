...
const LeaseArea = ({
  ...
  const getMapLinkUrl = () => {
    ...
    searchQuery.lease_area = area ? area.id : undefined, searchQuery.tab = 7;
  ...
  return <Fragment>
      <Row>
        ...
        <Column small={6} medium={4} large={2}>
          <Authorization allow={isFieldAllowedToRead(attributes, LeaseAreasFieldPaths.GEOMETRY)}>
            {!archived && !isEmpty(area.geometry) && <Link to={mapLinkUrl}>{LeaseAreasFieldTitles.GEOMETRY}</Link>}
          </Authorization>
        </Column>
      </Row>
      ...
      <Authorization allow={isFieldAllowedToRead(attributes, LeasePlotsFieldPaths.PLOTS)}>
        <Row>
          <Column small={12} large={6}>
            <Collapse className='collapse__secondary' defaultOpen={plotsContractCollapseState !== undefined ? plotsContractCollapseState : !archived} headerTitle='Kiinteistöt / määräalat sopimuksessa' onToggle={handlePlotsContractCollapseToggle} uiDataKey={getUiDataLeaseKey(LeasePlotsFieldPaths.PLOTS_CONTRACT)}>
              <BoxItemContainer>
                {!area.plots_contract || !area.plots_contract.length && <FormText>Ei kiinteistöjä/määräaloja sopimuksessa</FormText>}
                {area.plots_contract && !!area.plots_contract.length && area.plots_contract.map((item, index) => <PlotItem key={index} areaArchived={archived} plot={item} />)}
              </BoxItemContainer>
            </Collapse>
          </Column>
          <Column small={12} large={6}>
            <Collapse className='collapse__secondary' defaultOpen={plotsCurrentCollapseState !== undefined ? plotsCurrentCollapseState : !archived} headerTitle='Kiinteistöt / määräalat nykyhetkellä' onToggle={handlePlotsCurrentCollapseToggle} uiDataKey={getUiDataLeaseKey(LeasePlotsFieldPaths.PLOTS)}>
              <BoxItemContainer>
                {!area.plots_current || !area.plots_current.length && <FormText>Ei kiinteistöjä/määräaloja nykyhetkellä</FormText>}

                {area.plots_current && !!area.plots_current.length && area.plots_current.map((item, index) => <PlotItem key={index} areaArchived={archived} plot={item} />)}
              </BoxItemContainer>
            </Collapse>
          </Column>
        </Row>
      ...
      <Authorization allow={isFieldAllowedToRead(attributes, LeasePlanUnitsFieldPaths.PLAN_UNITS)}>
        <Row>
          <Column small={12} large={6}>
            <Collapse className='collapse__secondary' defaultOpen={planUnitsContractCollapseState !== undefined ? planUnitsContractCollapseState : !archived} headerTitle='Kaavayksiköt sopimuksessa' onToggle={handlePlanUnitContractCollapseToggle} uiDataKey={getUiDataLeaseKey(LeasePlanUnitsFieldPaths.PLAN_UNITS_CONTRACT)}>
              <BoxItemContainer>
                {!area.plan_units_contract || !area.plan_units_contract.length && <FormText>Ei kaavayksiköitä sopimuksessa</FormText>}
                {area.plan_units_contract && !!area.plan_units_contract.length && area.plan_units_contract.map((item, index) => <PlanUnitItem key={index} areaArchived={archived} planUnit={item} />)}
              </BoxItemContainer>
            </Collapse>
          </Column>
          <Column small={12} large={6}>
            <Collapse className='collapse__secondary' defaultOpen={planUnitsCurrentCollapseState !== undefined ? planUnitsCurrentCollapseState : !archived} headerTitle='Kaavayksiköt nykyhetkellä' onToggle={handlePlanUnitCurrentCollapseToggle} uiDataKey={getUiDataLeaseKey(LeasePlanUnitsFieldPaths.PLAN_UNITS)}>
              <BoxItemContainer>
                {!area.plan_units_current || !area.plan_units_current.length && <FormText>Ei kaavayksiköitä nykyhetkellä</FormText>}
                {area.plan_units_current && !!area.plan_units_current.length && area.plan_units_current.map((item, index) => <PlanUnitItem key={index} areaArchived={archived} planUnit={item} />)}
              </BoxItemContainer>
            </Collapse>
          </Column>
          ...
          <Column small={12} large={6}>
            <Collapse className='collapse__secondary' defaultOpen={planUnitsCurrentCollapseState !== undefined ? planUnitsCurrentCollapseState : !archived} headerTitle='Vireillä olevat kaavayksiköt' onToggle={handlePlanUnitCurrentCollapseToggle} uiDataKey={getUiDataLeaseKey(LeasePlanUnitsFieldPaths.PLAN_UNITS)}>
              <BoxItemContainer>
                {!area.plan_units_pending || !area.plan_units_pending.length && <FormText>Ei vireillä olevia kaavayksiköitä</FormText>}
                {area.plan_units_pending && !!area.plan_units_pending.length && area.plan_units_pending.map((item, index) => <PlanUnitItem key={index} areaArchived={archived} planUnit={item} />)}
              </BoxItemContainer>
            </Collapse>
          </Column>
        </Row>
      ...
      <Authorization allow={isFieldAllowedToRead(attributes, LeaseAreaCustomDetailedPlanFieldPaths.CUSTOM_DETAILED_PLAN)}>
        <Row>
          ...
          <Column small={12} large={6}>
            <Collapse className='collapse__secondary' defaultOpen={customDetailedPlanCollapseState !== undefined ? customDetailedPlanCollapseState : !archived} headerTitle='Oma muu alue' onToggle={handleCustomDetailedPlanCollapseToggle} uiDataKey={getUiDataLeaseKey(LeaseAreaCustomDetailedPlanFieldPaths.CUSTOM_DETAILED_PLAN)}>
              <BoxItemContainer>
                {!area.custom_detailed_plan ? <FormText>Ei omaa muuta aluetta</FormText> : <CustomDetailedPlan customDetailedPlan={area.custom_detailed_plan} />}
              </BoxItemContainer>
            </Collapse>
          </Column>
        </Row>
