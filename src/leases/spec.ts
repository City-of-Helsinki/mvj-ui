...
import { attributesNotFound, clearFormValidFlags, copyAreasToContract, copyDecisionToLeases, createCharge, createLease, createLeaseAndUpdateCurrentLease, deleteLease, fetchAttributes, fetchLeaseById, fetchLeases, fetchLeasesByBBox, fetchSingleLease, fetchSingleLeaseAfterEdit, hideAttachDecisionModal, hideCreateModal, hideEditMode, notFound, notFoundByBBox, notFoundById, patchLease, patchLeaseInvoiceNotes, receiveAttributes, receiveCollapseStates, receiveFormValidFlags, receiveIsSaveClicked, receiveLeaseById, receiveLeases, receiveLeasesByBBox, receiveMethods, receiveSingleLease, sendEmail, setRentInfoComplete, setRentInfoUncomplete, showAttachDecisionModal, showCreateModal, showEditMode, startInvoicing, stopInvoicing, fetchLeasesForContractNumber, receiveLeasesForContractNumbers } from "./actions";
  ...
  describe('Reducer', () => {
    ...
    describe('leasesReducer', () => {
      ...
      it('fetchSingleLeaseAfterEdit function should not change isFetcihng flag', () => {
        const state = leasesReducer({}, fetchSingleLeaseAfterEdit({
          leaseId: 1
        }));
      ...
      it('should update isAttachDecisionModalOpen flag to true', () => {
        const newState = { ...defaultState,
          isAttachDecisionModalOpen: true
        };
      ...
      it('should update leaseById when receiving lease by id', () => {
        ...
        const state = leasesReducer({}, receiveLeaseById({
          leaseId: leaseId,
          lease: dummyLease
        }));
        expect(state).to.deep.equal(newState);
      ...
      it('createCharge should not change state', () => {
        ...
        const state = leasesReducer({}, createCharge({
          data: {},
          leaseId: 1
        }));
        expect(state).to.deep.equal(newState);
      ...
      it('should update collapseStates', () => {
        ...
        let state = leasesReducer({}, receiveCollapseStates({
          foo: 'bar'
        }));
        state = leasesReducer(state, receiveCollapseStates({
          foo2: 'bar2'
        }));
        expect(state).to.deep.equal(newState);
      });
