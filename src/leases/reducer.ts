...
const isFetchingByIdReducer: Reducer<Record<string, any>> = handleActions(
  {
    ["mvj/leases/FETCH_BY_ID"]: (
      state: Record<string, any>,
      { payload: id }: FetchLeaseByIdAction,
    ) => {
      return { ...state, [id]: true };
    },
    ["mvj/leases/RECEIVE_BY_ID"]: (
      state: Record<string, any>,
      { payload }: ReceiveLeaseByIdAction,
    ) => {
      return { ...state, [payload.leaseId]: false };
    },
    ["mvj/leases/NOT_FOUND_BY_ID"]: (
      state: Record<string, any>,
      { payload: id }: LeaseNotFoundByIdAction,
    ) => {
...
const leasesListReducer: Reducer<LeaseList> = handleActions(
  {
    ["mvj/leases/RECEIVE_ALL"]: (
      state: LeaseList,
      { payload: leases }: ReceiveLeasesAction,
    ) => {
      return leases;
    },
  },
...
const listByBBoxReducer: Reducer<LeaseList> = handleActions(
  {
    ["mvj/leases/RECEIVE_BY_BBOX"]: (
      state: LeaseList,
      { payload: leases }: ReceiveLeasesByBBoxAction,
    ) => {
      return leases;
    },
  },
...
const currentLeaseReducer: Reducer<Lease> = handleActions(
  {
    ["mvj/leases/RECEIVE_SINGLE"]: (
      state: Lease,
      { payload: lease }: ReceiveSingleLeaseAction,
    ) => {
      return lease;
    },
  },
...
const byIdReducer: Reducer<Lease> = handleActions(
  {
    ["mvj/leases/RECEIVE_BY_ID"]: (
      ...
      return { ...state, [payload.leaseId]: payload.lease };
    },
  },
  {},
);
...
const leasesForContractNumbersReducer: Reducer<LeaseList> = handleActions(
  {
    ["mvj/leases/RECEIVE_LEASES_FOR_CONTRACT_NUMBERS"]: (
      state: LeaseList,
      { payload: leases }: ReceiveLeasesForContractNumbersAction,
    ) => {
      return leases;
    },
  },
...
const isFetchingLeasesForContractNumbersReducer: Reducer<boolean> =
  handleActions(
    {
