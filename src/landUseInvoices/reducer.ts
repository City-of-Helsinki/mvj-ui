...
const isCreatePanelOpenReducer: Reducer<boolean> = handleActions(
  {
    ["mvj/landUseInvoices/RECEIVE_IS_CREATE_PANEL_OPEN"]: (
      state: boolean,
...
const isCreditPanelOpenReducer: Reducer<boolean> = handleActions(
  {
    ["mvj/landUseInvoices/RECEIVE_IS_CREDIT_PANEL_OPEN"]: (
      state: boolean,
...
const isCreateClickedReducer: Reducer<boolean> = handleActions(
  {
    ["mvj/landUseInvoices/RECEIVE_CREATE_CLICKED"]: (
      state: boolean,
...
const isCreditClickedReducer: Reducer<boolean> = handleActions(
  {
    ["mvj/landUseInvoices/RECEIVE_CREDIT_CLICKED"]: (
      state: boolean,
...
const isEditClickedReducer: Reducer<boolean> = handleActions(
  {
    ["mvj/landUseInvoices/RECEIVE_EDIT_CLICKED"]: (
      state: boolean,
...
const attributesReducer: Reducer<Attributes> = handleActions(
  {
    ["mvj/landUseInvoices/RECEIVE_ATTRIBUTES"]: (
      state: Attributes,
...
const methodsReducer: Reducer<Methods> = handleActions(
  {
    ["mvj/landUseInvoices/RECEIVE_METHODS"]: (
      state: Methods,
...
const byLandUseContractReducer: Reducer<InvoiceListMap> = handleActions(
  {
    ["mvj/landUseInvoices/RECEIVE_BY_LAND_USE_CONTRACT"]: (
      state: InvoiceListMap,
      { payload }: ReceiveInvoicesByLandUseContractAction,
    ) => {
      return { ...state, [payload.id]: payload.invoices };
    },
  },
  {},
);
const invoiceToCreditReducer: Reducer<string | null | undefined> =
  handleActions(
    {
      ["mvj/landUseInvoices/RECEIVE_INVOICE_TO_CREDIT"]: (
...
const patchedInvoiceReducer: Reducer<Invoice | null | undefined> =
  handleActions(
    {
      ["mvj/landUseInvoices/RECEIVE_PATCHED"]: (
...
const collapseStatesReducer: Reducer<Record<string, any>> = handleActions(
  {
    ["mvj/landUseInvoices/RECEIVE_COLLAPSE_STATES"]: (
