...
import { receiveAttributes, receiveMethods, fetchAttributes, attributesNotFound, receiveInvoicesByLandUseContract, receiveIsCreateInvoicePanelOpen, receiveIsCreditInvoicePanelOpen, receiveIsCreateClicked, receiveIsCreditClicked, receiveIsEditClicked, fetchInvoicesByLandUseContract, createInvoice, patchInvoice, exportInvoiceToLaskeAndUpdateList, receivePatchedInvoice, clearPatchedInvoice, notFound, receiveInvoiceToCredit, creditInvoice, deleteInvoice, receiveCollapseStates, startInvoicing, stopInvoicing } from "./actions";
  ...
  describe('Reducer', () => {
    ...
    describe('invoiceReducer', () => {
      ...
      it('should update invoices received by lease', () => {
        ...
        const state = invoiceReducer({}, receiveInvoicesByLandUseContract({
          id: 1,
      ...
      it('should update isCreditClicked flag to true', () => {
        ...
        const newState = { ...defaultState,
          isCreditClicked: isCreditClicked
        };
      ...
      it('should update isSaving flag to true when exporting invoice to laske', () => {
        ...
        const state = invoiceReducer({}, exportInvoiceToLaskeAndUpdateList({
          id: 1,
          lease: 1
        }));
        expect(state).to.deep.equal(newState);
      ...
      it('should delete isSaving flag to true when deleting invoice', () => {
        ...
        const state = invoiceReducer({}, deleteInvoice({
          invoice: 'Invoice'
        }));
        expect(state).to.deep.equal(newState);
      });
      ...
      it('should update collapseStates', () => {
        ...
        let state = invoiceReducer({}, receiveCollapseStates({
          foo: 'bar'
        }));
        state = invoiceReducer(state, receiveCollapseStates({
          foo2: 'bar2'
        }));
        expect(state).to.deep.equal(newState);
      });
