...
export const fetchAttributes = () => {
  return callApi(
    new Request(createUrl("land_use_agreement_invoice/"), {
      method: "OPTIONS",
    }),
  );
};
...
export const creditInvoice = (
  ...
  return callApi(
    new Request(
      createUrl(
        `land_use_agreement_invoice_credit/?land_use_agreement_invoice=${invoiceId}`,
      ),
      {
        method: "POST",
        body,
...
export const patchInvoice = (invoice: Invoice): Generator<any, any, any> => {
  ...
  const body = JSON.stringify(invoice);
  return callApi(
    new Request(createUrl(`land_use_agreement_invoice/${id}/`), {
      method: "PATCH",
      body,
    }),
  );
...
export const exportInvoiceToLaske = (
  ...
  return callApi(
    new Request(
      createUrl(`land_use_agreement_invoice_export_to_laske/?invoice=${id}`),
      {
        method: "POST",
      },
    ),
