...
function* fetchInvoicesByLandUseContractSaga({payload: search}): Generator<any, any, any> {
  try {
    let {response: {status: statusCode}, bodyAsJson: body} = yield call(fetchInvoices, {land_use_agreement: search});
...
function* exportInvoiceToLaskeAndUpdateListSaga({payload: {id, landUseContract}}): Generator<any, any, any> {
  try {
...
function* creditInvoiceSaga({payload: {creditData, invoiceId, landUseContract}}): Generator<any, any, any> {
  try {
