...
function* fetchHistoryByBusinessIdSaga({payload: id}): Generator<any, any, any> {
  try {
    const {response: {status: statusCode}, bodyAsJson} = yield call(fetchHistoryBusinessId, id);

    switch (statusCode) {
      case 200:
        yield put(receiveHistoryByBusinessId({[id.toString()]: bodyAsJson}));
        break;
      default:
...
function* fetchHistoryByContactIdSaga({payload: id}): Generator<any, any, any> {
  try {
    const {response: {status: statusCode}, bodyAsJson} = yield call(fetchHistoryContactId, id);

    switch (statusCode) {
      case 200:
        yield put(receiveHistoryByContactId({[id.toString()]: bodyAsJson}));
        break;
      default:
        console.error('Failed to fetch history');
...
function* fetchCreditDecisionByBusinessIdSaga({payload: id}): Generator<any, any, any> {
  try {
    const {response: {status: statusCode}, bodyAsJson} = yield call(fetchCreditDecisionBusinessId, id);

    switch (statusCode) {
      case 200:
        const result = bodyAsJson[0];
        yield put(receiveCreditDecisionByBusinessId({[id.toString()]: result}));
        yield put(fetchHistoryByBusinessId(id));
        break;
      case 400: // Cannot find business id or identity number
...
function* fetchCreditDecisionByContactIdSaga({payload: id}): Generator<any, any, any> {
  try {
    const {response: {status: statusCode}, bodyAsJson} = yield call(fetchCreditDecisionContactId, id);

    switch (statusCode) {
      case 200:
        const result = bodyAsJson[0];
        yield put(receiveCreditDecisionByContactId({[id.toString()]: result}));
...
function* fetchCreditDecisionByNinSaga({payload: id}): Generator<any, any, any> {
  try {
    const {response: {status: statusCode}, bodyAsJson} = yield call(fetchCreditDecisionNin, id);

    switch (statusCode) {
      case 200:
        const result = bodyAsJson[0];
        yield put(receiveCreditDecisionByNin({[id.toString()]: result}));
        break;
      case 400: // Cannot find business id or identity number
        const error = bodyAsJson;
        console.error('Failed to fetch with error "%s"', error.message);
