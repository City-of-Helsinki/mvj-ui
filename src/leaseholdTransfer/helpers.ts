 ...
 * @returns {Object}
 */
export const mapLeaseholdTransferSearchFilters = (
 ...
 * @returns {Object[]}
 */
export const getContentLeaseholdTransfers = (
  list: LeaseholdTransferList,
): Array<Record<string, any>> =>
  getApiResponseResults(list).map((item) => {
    return {
      id: item.id,
      properties: item.properties,
      institution_identifier: item.institution_identifier,
      decision_date: item.decision_date,
      acquirers: get(item, "parties", []).filter(
        (party) => party.type === LeaseholdTransferPartyTypes.ACQUIRER,
      ),
      conveyors: get(item, "parties", []).filter(
        (party) => party.type === LeaseholdTransferPartyTypes.CONVEYOR,
      ),
