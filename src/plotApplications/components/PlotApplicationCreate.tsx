...
import { receiveCollapseStates, receiveIsSaveClicked } from "plotApplications/actions";
import { getCollapseStateByKey, getIsSaveClicked, getErrorsByFormName } from "plotApplications/selectors";
import { getInitialApplication, getSectionTargetFromMeta } from "plotApplications/helpers";
import { getIsFetching as getIsFetchingPlotSearchList, getIsFetchingForm, getPlotSearchList } from "plotSearch/selectors";
import { getCurrentEditorTargets, getCurrentPlotApplication } from "plotApplications/selectors";
import ApplicationSubsection from "application/components/ApplicationSubsection";
import { receiveFormValidFlags, receiveSinglePlotApplication, setCurrentEditorTargets } from "plotApplications/actions";
import Loader from "components/loader/Loader";
import { getAttributes, getFieldTypeMapping, getFormAttributes, getIsFetchingApplicantInfoCheckAttributes, getIsFetchingAttachmentAttributes, getIsFetchingFormAttributes } from "application/selectors";
import { getInitialApplicationForm, getSectionTemplate, getTargetTitle } from "application/helpers";
import { fetchAttachmentAttributes, fetchFormAttributes, fetchPendingUploads } from "application/actions";
...
class PlotApplicationCreate extends PureComponent<Props, State> {
  ...
  initializeFormValues = formId => {
    ...
    this.props.change('formEntries', getInitialApplicationForm(this.props.fieldTypeMapping, matchingSearch?.form, null));
  ...
  updateTargetSections = (newTargetIds, _, oldTargetIds) => {
    ...
    added.forEach(id => {
      array.push(targetSectionsArrayPath, { ...getSectionTemplate(FormNames.PLOT_APPLICATION, 'formEntries', TARGET_SECTION_IDENTIFIER),
        metadata: {
          identifier: id
        }
      });
    ...
    const deletedIndices = deleted.map(id => oldValues.findIndex(item => item.metadata?.identifier === id)).sort((a, b) => a < b ? 1 : -1);
    deletedIndices.forEach(index => array.remove(targetSectionsArrayPath, index));
  };
  ...
  render() {
    ...
    return <form className="PlotApplicationEdit">
        <Title>
          Hakemus
        </Title>
        <Divider />
        <Row className='summary__content-wrapper'>
          <Column small={12}>
            <Collapse defaultOpen={collapseStateCommon !== undefined ? collapseStateCommon : true} hasErrors={isSaveClicked && !isEmpty(errors)} headerTitle='Hakemus' onToggle={this.handleCommonFieldsCollapseToggle}>
              <Row>
                <Authorization allow={true}>
                  <Column small={12} medium={6} large={4}>
                    <FormField disableTouched={isSaveClicked} fieldAttributes={{ ...get(attributes, 'form'),
                    label: 'Tonttihaku',
                    choices: plotSearchChoices
                  }} overrideValues={{
                    allowEdit: !isFormFixed
                  }} name='formId' onChange={this.initializeFormValues} />
                  </Column>
                </Authorization>
                <Authorization allow={true}>
                  <Column small={6} medium={4} large={2}>
                    <FormField disableTouched={isSaveClicked} fieldAttributes={get(attributes, 'id')} name='id' disabled />
                  </Column>
                </Authorization>
              </Row>
            </Collapse>
            ...
            {this.state.form && <>
              ...
              <Row>
                <Column small={12}>
                  <FormField disableTouched={isSaveClicked} fieldAttributes={{ ...get(attributes, 'targets'),
                  type: 'multiselect',
                  label: 'Haettavat kohteet'
                }} overrideValues={{
                  options: targetOptions
                }} name='targets' onChange={this.updateTargetSections} />
                </Column>
              </Row>
              <Row>
                <Column small={12}>
                  {this.state.form.sections.map(section => <ApplicationSubsection path={['formEntries.sections']} section={section} headerTag="h2" key={section.id} formName={FormNames.PLOT_APPLICATION} formPath='formEntries' sectionTitleTransformers={[(title, section, identifier) => getSectionTargetFromMeta(identifier) || title]} answerId={currentPlotApplication?.id || null} />)}
                </Column>
              </Row>
            </>}
          </Column>
