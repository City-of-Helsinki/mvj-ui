...
type Props = {
  ...
  formValues: Object,
  isPerformingFileOperation: boolean,
  submissionErrors: ?Object | ?Array<Object>,
  isSaving: boolean,
  isSaveClicked: boolean,
};
...
class PlotApplicationTargetInfoCheck extends PureComponent<Props> {
  renderErrors(): React$Node {
    ...
    try {
      if (submissionErrors instanceof Array) {
        ...
        const errorObject: Object = submissionErrors.error;

        content = <ul>
          {Object.keys(errorObject).map((key) => {
            const fieldLabelKey = Object.keys(PlotApplicationTargetInfoCheckFieldPaths).find(
              (path) => PlotApplicationTargetInfoCheckFieldPaths[path] === `target_statuses.child.children.${key}`);
  ...
  render(): React$Node {
    const {attributes, formValues, isPerformingFileOperation, submissionErrors, isSaveClicked, isSaving} = this.props;

    const targetInfoCheck = formValues?.id;

    return (
      <PlotApplicationInfoCheckCollapse className="PlotApplicationTargetInfoCheckEdit" headerTitle="Kohteen käsittelytiedot">
        <form>
          <Row>
