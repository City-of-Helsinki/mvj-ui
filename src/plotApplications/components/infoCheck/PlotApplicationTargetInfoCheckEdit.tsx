...
import { getIsPerformingFileOperation, getIsSaveClicked, getIsSaving, getTargetInfoCheckSubmissionErrors } from "/src/plotApplications/selectors";
...
import { getMeetingMemoDownloadLink, getTargetInfoCheckFormName } from "/src/plotApplications/helpers";
import { deleteTargetInfoCheckMeetingMemo, uploadTargetInfoCheckMeetingMemo } from "/src/plotApplications/actions";
        ...
        dispatch
          ...
          return <Row role="row" key={index}>
                <Column role="cell" small={4} medium={4} large={2}>
                  <FormField fieldAttributes={get(attributes, PlotApplicationTargetInfoCheckFieldPaths.PROPOSED_MANAGEMENT_TYPE)} name={`${field}.proposed_management`} invisibleLabel overrideValues={{
                label: PlotApplicationTargetInfoCheckFieldTitles.PROPOSED_MANAGEMENT_TYPE
              }} disabled={disabled} disableTouched={isSaveClicked} />
                </Column>
                <Column role="cell" small={4} medium={4} large={2}>
                  <FormField fieldAttributes={get(attributes, PlotApplicationTargetInfoCheckFieldPaths.PROPOSED_MANAGEMENT_FINANCING)} name={`${field}.proposed_financing`} invisibleLabel overrideValues={{
                label: PlotApplicationTargetInfoCheckFieldTitles.PROPOSED_MANAGEMENT_FINANCING
              }} disabled={disabled} disableTouched={isSaveClicked} />
                </Column>
                <Column role="cell" small={3} medium={3} large={2}>
                  <FormField fieldAttributes={get(attributes, PlotApplicationTargetInfoCheckFieldPaths.PROPOSED_MANAGEMENT_HITAS)} name={`${field}.hitas`} invisibleLabel overrideValues={{
            ...
            {!disabled && <Row>
                <Column>
                  <AddButtonThird label='Lisää rahoitus- ja hallintamuoto' onClick={handleAdd} />
                </Column>
              </Row>}
          </div>}
      </AppConsumer>;
        ...
        dispatch
          ...
          return <Row role="row" key={index}>
                <Column role="cell" small={11} medium={11} large={11}>
                  <FormField fieldAttributes={get(attributes, PlotApplicationTargetInfoCheckFieldPaths.RESERVATION_CONDITIONS)} name={field} invisibleLabel overrideValues={{
          ...
          {isPerformingFileOperation ? <LoaderWrapper className='small-inline-wrapper'>
            ...
            return <Row key={index}>
                    ...
                    /* not having an associated file shouldn't be possible, but just in case */
                      ...
                      <Column small={3} large={2}>
                        <RemoveButton className='third-level' onClick={handleRemove} title="Poista muistio" />
                      </Column>
                    </Row>;
          })}
              </Fragment> : <FormText>Ei lisättyjä muistioita.</FormText>}
            {!disabled && <AddFileButton label='Lisää tiedosto' name={`addFileButton--${targetInfoCheck}--${fields.name}`} onChange={handleAdd} />}
          </>}
        </div>}
    </AppConsumer>;
});
  ...
  renderErrors(): React.ReactNode {
    ...
    try {
      if (submissionErrors instanceof Array) {
        content = <ul>
          {submissionErrors.map((error, i) => <li key={i}>{error}</li>)}
        </ul>;
        ...
        content = <ul>
          {Object.keys(errorObject).map(key => {
            const fieldLabelKey = Object.keys(PlotApplicationTargetInfoCheckFieldPaths).find(path => PlotApplicationTargetInfoCheckFieldPaths[path] === `target_statuses.child.children.${key}`);
            return <li key={key}>
              {fieldLabelKey ? PlotApplicationTargetInfoCheckFieldTitles[fieldLabelKey] : key}:{' '}
              {submissionErrors[key].length !== undefined ? errorObject[key].join(', ') : errorObject[key]}
            </li>;
          })}
        </ul>;
      } else {
        ...
        <form>
          <Row>
            <Authorization allow={isFieldAllowedToRead(attributes, PlotApplicationTargetInfoCheckFieldPaths.RESERVED)}>
              <Column small={6} medium={4} large={2}>
                <FormField name="reserved" fieldAttributes={getFieldAttributes(attributes, PlotApplicationTargetInfoCheckFieldPaths.RESERVED)} disableTouched={isSaveClicked} disabled={isSaving} overrideValues={{
            ...
            <Authorization allow={isFieldAllowedToRead(attributes, PlotApplicationTargetInfoCheckFieldPaths.SHARE_OF_RENT_INDICATOR)}>
              <Column small={6} medium={4} large={2}>
                ...
                <Row>
                  <Column large={6}>
                    <FormField name="share_of_rental_indicator" fieldAttributes={getFieldAttributes(attributes, PlotApplicationTargetInfoCheckFieldPaths.SHARE_OF_RENT_INDICATOR)} disableTouched={isSaveClicked} disabled={isSaving} invisibleLabel overrideValues={{
                    label: PlotApplicationTargetInfoCheckFieldTitles.SHARE_OF_RENT_INDICATOR
                  }} />
                  </Column>
                  <Column large={6}>
                    <FormField name="share_of_rental_denominator" fieldAttributes={getFieldAttributes(attributes, PlotApplicationTargetInfoCheckFieldPaths.SHARE_OF_RENT_DENOMINATOR)} disableTouched={isSaveClicked} disabled={isSaving} invisibleLabel className='with-slash' overrideValues={{
            ...
            <Authorization allow={isFieldAllowedToRead(attributes, PlotApplicationTargetInfoCheckFieldPaths.NEGOTIATION_DATE)}>
              <Column small={6} medium={4} large={2}>
                <FormField name="counsel_date" fieldAttributes={getFieldAttributes(attributes, PlotApplicationTargetInfoCheckFieldPaths.NEGOTIATION_DATE)} overrideValues={{
                label: PlotApplicationTargetInfoCheckFieldTitles.NEGOTIATION_DATE,
                fieldType: FieldTypes.DATE
              }} disableTouched={isSaveClicked} disabled={isSaving} />
              </Column>
            </Authorization>
            <Authorization allow={isFieldAllowedToRead(attributes, PlotApplicationTargetInfoCheckFieldPaths.DECLINE_REASON)}>
              <Column small={6} medium={4} large={3}>
                <FormField name="decline_reason" fieldAttributes={getFieldAttributes(attributes, PlotApplicationTargetInfoCheckFieldPaths.DECLINE_REASON)} overrideValues={{
            ...
            <Authorization allow={isFieldAllowedToRead(attributes, PlotApplicationTargetInfoCheckFieldPaths.ADDED_TARGET_TO_APPLICANT)}>
              <Column small={6} medium={4} large={3}>
                <FormField name="added_target_to_applicant" fieldAttributes={getFieldAttributes(attributes, PlotApplicationTargetInfoCheckFieldPaths.ADDED_TARGET_TO_APPLICANT)} overrideValues={{
          ...
          <Row>
            <Authorization allow={isFieldAllowedToRead(attributes, PlotApplicationTargetInfoCheckFieldPaths.PROPOSED_MANAGEMENTS)}>
              <Column small={12} medium={12} large={12}>
                <FieldArray name="proposed_managements" component={PlotApplicationTargetInfoCheckManagements} attributes={attributes} targetInfoCheck={targetInfoCheck} disableTouched={isSaveClicked} disabled={isSaving} />
              </Column>
            </Authorization>
          </Row>
          <Row>
            <Authorization allow={isFieldAllowedToRead(attributes, PlotApplicationTargetInfoCheckFieldPaths.RESERVATION_CONDITIONS)}>
              <Column small={12} medium={12} large={12}>
                <FieldArray name="reservation_conditions" component={PlotApplicationTargetInfoCheckConditions} attributes={attributes} targetInfoCheck={targetInfoCheck} disableTouched={isSaveClicked} disabled={isSaving} />
              </Column>
            </Authorization>
          </Row>
          <Row>
            <Authorization allow={isFieldAllowedToRead(attributes, PlotApplicationTargetInfoCheckFieldPaths.ARGUMENTS)}>
              <Column small={12} medium={12} large={12}>
                <FormField name="arguments" fieldAttributes={getFieldAttributes(attributes, PlotApplicationTargetInfoCheckFieldPaths.ARGUMENTS)} overrideValues={{
                fieldType: FieldTypes.TEXTAREA,
          ...
          <Row>
            <Authorization allow={isFieldAllowedToRead(attributes, PlotApplicationTargetInfoCheckFieldPaths.MEETING_MEMOS)}>
              <Column small={12} medium={12} large={12}>
                <FieldArray name="meeting_memos" component={PlotApplicationTargetInfoCheckMeetingMemos} meetingMemoAttributes={get(attributes, PlotApplicationTargetInfoCheckFieldPaths.MEETING_MEMOS)} targetInfoCheck={targetInfoCheck} isPerformingFileOperation={isPerformingFileOperation} disableTouched={isSaveClicked} disabled={isSaving} />
              </Column>
            </Authorization>
          </Row>
        </form>
...
export default (flowRight(connect((state: RootState, props: Props) => {
  const formValues = getFormValues(getTargetInfoCheckFormName(props.targetId))(state);
  return {
    attributes: getAttributes(state),
    form: getTargetInfoCheckFormName(props.targetId),
    formValues,
    isPerformingFileOperation: getIsPerformingFileOperation(state),
    submissionErrors: getTargetInfoCheckSubmissionErrors(state, formValues?.id),
