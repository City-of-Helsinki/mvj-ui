...
import { getCollapseStateByKey, getCurrentPlotApplication, getIsFetchingApplicationRelatedForm, getApplicationRelatedForm } from "plotApplications/selectors";
...
import { getApplicationRelatedPlotSearch, getIsFetchingApplicationRelatedPlotSearch } from "plotApplications/selectors";
...
import { getApplicationRelatedAttachments, getAttributes, getFormAttributes, getIsFetchingApplicationRelatedAttachments, getIsFetchingFormAttributes } from "/src/application/selectors";
...
const PlotApplicationSectionExtras = ({
  ...
  return <>
    {topLevel && section.identifier === TARGET_SECTION_IDENTIFIER && (answer.metadata?.identifier ? <PlotApplicationTargetInfoCheck section={section} identifier={identifier} targetId={(answer.metadata.identifier as any)} /> : <Loader isLoading={true} />)}
    {topLevel && section.identifier === APPLICANT_SECTION_IDENTIFIER && <PlotApplicationApplicantInfoCheck section={section} identifier={identifier} answer={answer} />}
  </>;
};
  ...
  render() {
    ...
    return <div className="PlotApplication">
        ...
        <Row className='summary__content-wrapper'>
          <Column small={12}>
            {plotApplication.opening_record && <Collapse defaultOpen={true} headerTitle='Hakemuksen avaaminen'>
              <Row>
            ...
            <Collapse defaultOpen={applicationCollapseState !== undefined ? applicationCollapseState : true} headerTitle='Hakemuksen käsittelytiedot' onToggle={this.handleBasicInfoCollapseToggle}>
              <Row>
            ...
            {!isLoading && fieldTypes && orderBy(form.sections, 'order').filter(section => section.visible).map(section => <ApplicationAnswersSection section={section} answer={answerData.sections[section.identifier]} topLevel fieldTypes={fieldTypes} key={section.identifier} plotSearch={plotSearch} editMode={false} sectionExtraComponent={PlotApplicationSectionExtras} sectionTitleTransformers={[transformTargetSectionTitle(plotSearch), transformApplicantSectionTitle]} />)}
          </Column>
        </Row>
      </div>;
  }
