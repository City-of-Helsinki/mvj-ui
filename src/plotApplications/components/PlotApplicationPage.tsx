...
import PlotApplicationEdit from "@/plotApplications/components/PlotApplicationEdit";
import { getPlotSearchList } from "@/plotSearch/selectors";
import {
...
class PlotApplicationPage extends Component<Props & WithRouterProps, State> {
  ...
  saveUnsavedChanges = () => {
    const { currentPlotApplication, isFormDirty, getValuesForForm } =
      this.props;
    ...
    if (anyOtherFormsDirty) {
      setSessionStorageItem(
        "plotApplicationOpeningRecord",
        getValuesForForm(FormNames.PLOT_APPLICATION_OPENING),
      );
    } else {
  ...
  getAreFormsValid = () => {
    ...
    const { targetInfoCheckFormNames } = this.state;
  ...
  initializeInfoCheckForms = () => {
    ...
    Object.keys(applicantInfoChecksByApplicantId).forEach((key) => {
      const infoChecks = getApplicantInfoCheckItems(
        applicantInfoChecksByApplicantId[key],
      );
      infoChecks.forEach((infoCheck) => {
        initialize(getApplicantInfoCheckFormName(infoCheck.data.id), infoCheck);
        applicantInfoCheckFormNames.push(
  ...
  saveChanges = () => {
    ...
    if (areFormsValid) {
      ...
      if (isFormDirty(FormNames.PLOT_APPLICATION_OPENING)) {
        const openingRecordValues = getValuesForForm(
          FormNames.PLOT_APPLICATION_OPENING,
        ).opening_record;
  ...
  restoreUnsavedChanges = () => {
    const { showEditMode, change } = this.props;
    showEditMode();
    clearFormValidFlags();
    this.destroyAllForms();
    this.initializeInfoCheckForms();
    const storedInfoCheckData = getSessionStorageItem(
      "plotApplicationInfoChecks",
    );
    ...
    const storedOpeningRecordData = getSessionStorageItem(
      "plotApplicationOpeningRecord",
    );
    ...
    if (storedOpeningRecordData) {
      change(
        FormNames.PLOT_APPLICATION_OPENING,
        "opening_record",
        storedOpeningRecordData.opening_record,
      );
    }
            ...
            buttonComponent={
              <ControlButtons
                ...
                deleteModalTexts={{
                  ...
                  buttonText:
                    ConfirmationModalTexts.DELETE_PLOT_APPLICATION.BUTTON,
