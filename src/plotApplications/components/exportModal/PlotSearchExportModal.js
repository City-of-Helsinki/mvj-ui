...
import PlotSearchExportModalTargetTable
  from '$src/plotApplications/components/exportModal/PlotSearchExportModalTargetTable';
...
type Props = {
  ...
  items: Array<Object>,
  isFetching: boolean,
  fetchTargetInfoChecksForPlotSearch: Function,
};
...
const ModalDetailColumns: { [string]: Column } = {
  CHECKBOX: {
    key: 'checkbox',
    text: 'Tulosta',
    sortable: false,
...
class PlotSearchExportModal extends Component<Props> {
  componentDidUpdate(prevProps: Props): void {
    const {isOpen, reset, fetchTargetInfoChecksForPlotSearch, plotSearchId} = this.props;
  ...
  updateTargetItemSelection = (changedItem: Object, newValue: boolean) => {
    ...
    switch (selectedMode) {
      ...
      case ExportModes.APPLICATIONS_BY_TARGET_PDF:
        matchingItems = items
          .filter((item) => item.target_identifier === changedItem.target_identifier);
        break;
      case ExportModes.APPLICATIONS_BY_APPLICANT_PDF:
        matchingItems = items
          .filter((item) => item.answer_id === changedItem.answer_id);
        break;
    }
  ...
  render(): React$Node {
    ...
    let guideText: React$Node =
      <p>Voit tulostaa hakemuksien tietoja tiedostoon. Aloita valitsemalla tulostusalue.</p>;
    ...
    return (
      ...
      >
        <FormField
          ...
          disableDirty
          onChange={() => resetSection('selectedApplicationTargetItems', 'items')}
        />
        {guideText}
        {(columns.length > 0 && !isFetching) && <PlotSearchExportModalTargetTable
          items={items}
          onItemChange={this.updateTargetItemSelection}
          columns={columns}
          firstCheckboxOnly={selectedMode === ExportModes.APPLICATIONS_BY_APPLICANT_PDF}
        />}
...
)(PlotSearchExportModal): React$ComponentType<OwnProps>);
