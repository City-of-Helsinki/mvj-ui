...
const PlotSearchExportModalTargetTable = ({
  ...
  return <SortableTable listTable className="PlotSearchExportModalTargetTable" data={Object.keys(groupedItems).map(name => {
    ...
    return {
      id: name,
      checkbox: <>
          {(firstCheckboxOnly ? group.slice(0, 1) : group).map(item => <PlotSearchExportModalTargetTableCheckbox item={item} onItemChange={(event, value) => onItemChange(item, value)} key={item.application_identifier} />)}
        </>,
