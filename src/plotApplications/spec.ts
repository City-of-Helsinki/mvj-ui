...
import { fetchPlotApplicationsList, receivePlotApplicationsList, fetchSinglePlotApplication, receiveSinglePlotApplication, hideEditMode, showEditMode, receiveIsSaveClicked, receiveCollapseStates, receiveFormValidFlags, clearFormValidFlags, editPlotApplication } from "src/plotApplications/actions";
  ...
  describe('Reducer', () => {
    ...
    describe('plotApplicationReducer', () => {
      ...
      it('should update isFormValidById', () => {
        ...
        const state = plotApplicationReducer({}, receiveFormValidFlags({
      ...
      it('should clear isFormValidById', () => {
        ...
        let state = plotApplicationReducer({}, receiveFormValidFlags({
          ['plot-application']: false
        }));
      ...
      it('should update collapseStates', () => {
        ...
        let state = plotApplicationReducer({}, receiveCollapseStates({
          foo: 'bar'
        }));
        state = plotApplicationReducer(state, receiveCollapseStates({
          foo2: 'bar2'
        }));
        expect(state).to.deep.equal(newState);
      });
