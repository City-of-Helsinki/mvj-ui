...
const listByBBoxReducer: Reducer<PlotApplicationsList> = handleActions(
  {
    ["mvj/plotApplications/RECEIVE_BY_BBOX"]: (
      state: PlotApplicationsList,
      { payload: plotApplications }: ReceivePlotApplicationsByBBoxAction,
    ) => {
...
const plotApplicationsListReducer: Reducer<PlotApplicationsList> =
  handleActions(
    {
      ["mvj/plotApplications/RECEIVE_ALL"]: (
        state: PlotApplicationsList,
        { payload: list }: ReceivePlotApplicationsListAction,
      ) => list,
    },
    {},
  );
const currentPlotApplicationReducer: Reducer<PlotApplication> = handleActions(
  {
    ["mvj/plotApplications/RECEIVE_SINGLE"]: (
      state: PlotApplication,
      { payload: plotApplications }: ReceiveSinglePlotApplicationAction,
    ) => plotApplications,
  },
  {},
);
const isSaveClickedReducer: Reducer<boolean> = handleActions(
  {
    ["mvj/plotApplications/RECEIVE_SAVE_CLICKED"]: (
      state: boolean,
...
const collapseStatesReducer: Reducer<Record<string, any>> = handleActions(
  {
    ["mvj/plotApplications/RECEIVE_COLLAPSE_STATES"]: (
...
const isFormValidByIdReducer: Reducer<Record<string, any>> = handleActions(
  {
    ["mvj/plotApplications/RECEIVE_FORM_VALID_FLAGS"]: (
      state: Record<string, any>,
...
const formReducer: Reducer<Record<string, any>> = handleActions(
  {
    ...
    ["mvj/plotApplications/RECEIVE_FORM"]: (
      state: Record<string, any>,
      { payload: form }: ReceiveApplicationRelatedFormAction,
    ) => {
      return form;
    },
  },
...
const plotSearchReducer: Reducer<Record<string, any>> = handleActions(
  {
    ...
    ["mvj/plotApplications/RECEIVE_PLOT_SEARCH"]: (
      state: Record<string, any>,
      { payload: plotSearch }: ReceiveApplicationRelatedPlotSearchAction,
    ) => {
      return plotSearch;
    },
  },
...
const currentEditorTargetsReducer: Reducer<Array<Record<string, any>>> =
  handleActions(
    {
      ["mvj/plotApplications/SET_CURRENT_EDITOR_TARGETS"]: (
        state,
        { payload: targets },
      ) => targets || [],
    },
...
const targetInfoCheckAttributesReducer: Reducer<Attributes> = handleActions(
  {
    ["mvj/plotApplications/RECEIVE_TARGET_INFO_CHECK_ATTRIBUTES"]: (
      state: Attributes,
      { payload: attributes }: ReceiveTargetInfoCheckAttributesAction,
    ) => attributes,
    ["mvj/plotApplications/TARGET_INFO_CHECK_ATTRIBUTES_NOT_FOUND"]: () => null,
  },
  null,
);
const isFetchingTargetInfoCheckAttributesReducer: Reducer<boolean> =
  handleActions(
    {
      ["mvj/plotApplications/FETCH_TARGET_INFO_CHECK_ATTRIBUTES"]: () => true,
      ["mvj/plotApplications/RECEIVE_TARGET_INFO_CHECK_ATTRIBUTES"]: () =>
        false,
      ["mvj/plotApplications/TARGET_INFO_CHECK_ATTRIBUTES_NOT_FOUND"]: () =>
        false,
    },
    false,
  );
const infoCheckBatchEditErrorsReducer: Reducer<InfoCheckBatchEditErrors> =
  handleActions(
    {
      ...
      ["mvj/plotApplications/RECEIVE_INFO_CHECK_BATCH_EDIT_FAILURE"]: (
        state,
        { payload }: ReceivePlotApplicationInfoCheckBatchEditFailureAction,
      ) => payload,
    },
    {
      target: [],
...
const targetInfoChecksForCurrentPlotSearchReducer: Reducer<
...
> = handleActions(
  {
    ...
    ["mvj/plotApplications/RECEIVE_TARGET_INFO_CHECKS_FOR_PLOT_SEARCH"]: (
      state: Attributes,
      { payload: infoChecks }: ReceiveTargetInfoChecksForPlotSearchAction,
...
const isFetchingTargetInfoChecksForCurrentPlotSearchReducer: Reducer<boolean> =
  handleActions(
    {
      ["mvj/plotApplications/FETCH_TARGET_INFO_CHECKS_FOR_PLOT_SEARCH"]: () =>
        true,
...
export default combineReducers<Record<string, any>, Action<any>>({
  ...
  isFetchingTargetInfoCheckAttributes:
    isFetchingTargetInfoCheckAttributesReducer,
  ...
  targetInfoChecksForCurrentPlotSearch:
    targetInfoChecksForCurrentPlotSearchReducer,
  isFetchingTargetInfoChecksForCurrentPlotSearch:
    isFetchingTargetInfoChecksForCurrentPlotSearchReducer,
}) as any;
