...
const isSaveClickedReducer: Reducer<boolean> = handleActions(
  {
    ["mvj/rentforperiod/RECEIVE_SAVE_CLICKED"]: (
      state: boolean,
...
const byLeaseReducer: Reducer<RentForPeriod> = handleActions(
  {
    ["mvj/rentforperiod/RECEIVE_BY_LEASE"]: (
      state: RentForPeriod,
      { payload }: ReceiveRentForPeriodByLeaseAction,
    ) => {
      const rents = isArray(state[payload.leaseId])
        ? [...state[payload.leaseId], payload.rent]
        : [payload.rent];
      return { ...state, [payload.leaseId]: rents };
    },
    ["mvj/rentforperiod/DELETE_BY_LEASE"]: (
      state: RentForPeriod,
      { payload }: DeleteRentForPeriodByLeaseAction,
    ) => {
      const rents = isArray(state[payload.leaseId])
