...
import { receiveRentForPeriodByLease, deleteRentForPeriodByLease, fetchRentForPeriodByLease, receiveIsSaveClicked, notFound } from "./actions";
  ...
  describe('Reducer', () => {
    describe('rentForPeriodReducer', () => {
      it('should update rent for period list', () => {
        ...
        const newState1 = { ...defaultState,
          byLease: {
            [leaseId]: [dummyRentForPeriod1]
          }
        };
        const newState2 = { ...defaultState,
          byLease: {
            [leaseId]: [dummyRentForPeriod1, dummyRentForPeriod2]
          }
        };
        let state = rentForPeriodReducer({}, receiveRentForPeriodByLease({
          leaseId: leaseId,
          rent: dummyRentForPeriod1
        }));
        expect(state).to.deep.equal(newState1);
        state = rentForPeriodReducer(state, receiveRentForPeriodByLease({
          leaseId: leaseId,
      ...
      it('should delete rent for period', () => {
        ...
        state = rentForPeriodReducer(state, deleteRentForPeriodByLease({
          leaseId: leaseId1,
          id: rentId1
        }));
        expect(state).to.deep.equal(newState1);
