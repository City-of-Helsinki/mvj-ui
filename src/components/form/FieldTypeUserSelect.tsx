...
const FieldTypeUserSelect = ({
  ...
  const [initialUser, setInitialUser] = useState<Record<string, any> | null>(null);
  ...
  const getUsers = debounce(async (inputValue: string, callback: (...args: Array<any>) => any) => {
  ...
  return <AsyncSelect disabled={disabled} displayError={displayError} getOptions={getUsers} input={input} isDirty={isDirty} isLoading={isLoadingSingleUser && !singleUserLoadCancelled.current} onChange={handleChange} placeholder={placeholder} multiSelect={multiSelect} defaultOptions={initialUser ? [initialUser] : []} initialValues={initialUser} />;
};
