...
export const integer = (value: any, error?: string): ?string =>
  (isEmptyValue(value) || (Number.isInteger(Number(value)))
    ? undefined
    : (error ? error : 'Arvon tulee olla kokonaisluku'));

export const isDate = (value: any, error?: string): ?string =>
  (isEmptyValue(value) || isValidDate(new Date(value))
    ? undefined
    : (error ? error : 'Arvon tulee olla päivämäärä'));

export const decimalNumber = (value: any, error?: string): ?string =>
  (isEmptyValue(value) || !isNaN(
    value.toString().replace(',', '.').replace(/\s+/g, ''),
  )
    ? undefined
    : (error ? error : 'Arvon tulee olla numero'));

export const min = (value: any, min: number, error?: string): ?string =>
  (isEmptyValue(value) || (Number(value) >= min)
    ? undefined
    : (error ? error : `Minimiarvo on ${min}`));

export const max = (value: any, max: number, error?: string): ?string =>
  (isEmptyValue(value) || (Number(value) <= max)
    ? undefined
    : (error ? error : `Maksimiarvo on ${max}`));

export const maxLength = (value: any, max: number, error?: string): ?string =>
  ((!value || value.length <= max)
    ? undefined
    : (error ? error : `Maksimipituus on ${max}`));

export const digitsMaxLength = (value: any, max: number, error?: string): ?string =>
  ((isEmptyValue(value) || parseInt(value).toString().length <= max)
    ? undefined
    : (error ? error : `Kokonaislukuosan maksimipituus on ${max}`));

export const decimalsMaxLength = (value: any, max: number, error?: string): ?string =>
  ((isEmptyValue(value) || decimalPlaces(value.toString().replace(',', '.')) <= max)
    ? undefined
    : (error ? error : `Desimaaliosan maksimipituus on ${max}`));

export const dateGreaterOrEqual = (date: ?string, otherDate: ?string, error?: string): ?string => {
...
export const internalOrder = (value: any, error?: string): ?string => {
...
export const year = (value: any, error?: string): ?string => {
  if (isEmptyValue(value)) {
    return undefined;
  }
  const numbers = /^[0-9]+$/;
...
export const genericValidator = (value: any, options: Object): any => {
  if (!options) {
    return undefined;
  }
  let error = '';
