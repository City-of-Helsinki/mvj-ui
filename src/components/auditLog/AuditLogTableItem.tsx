    ...
    return <Fragment>
        ...
        'collapsed': collapse
          ...
          <td className='collapse-arrow-column'>
            {showCollapseIcon && <a className='sortable-table-row-collapse-link' onClick={this.handleCollapseArrowIconClick} onKeyDown={this.handleCollapseArrowIconKeyDown} tabIndex={0}>
          ...
          <td>{item.content_type_name} ({item.object_id})</td>
          <td></td>
          <td></td>
        </tr>
        {collapse && !!changes && changes.map((change, index) => {
        return <AuditLogTableItemChange key={index} change={change} contentType={item.content_type} />;
      })}
      </Fragment>;
  }
