...
export const validateRentCalculatorForm = (values: Record<string, any>) => {
  ...
  switch (values.type) {
    case RentCalculatorTypes.YEAR:
      if (isEmptyValue(values.year)) errors.yearErrors = 'Vuosi on pakollinen';else errors.yearErrors = year(values.year);
      break;
    ...
    case RentCalculatorTypes.RANGE:
      if (!values.billing_start_date || !values.billing_end_date) {
        ...
        errors.rangeErrors = 'Alku- ja loppupäivämäärän tulee olla samana vuonna';
      } else if (values.billing_end_date < values.billing_start_date) {
        errors.rangeErrors = 'Loppupäivämäärän tulee olla alkupäivämäärän jälkeen';
      }

      break;

    case RentCalculatorTypes.BILLING_PERIOD:
      if (isEmptyValue(values.billing_period)) errors.billingPeriodErrors = 'Laskutuskausi on pakollinen';
      break;
  }
