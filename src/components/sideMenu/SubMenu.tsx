...
type Props = {
  ...
  menuKey: string;
  onHeaderClick: (...args: Array<any>) => any;
};
...
class SubMenu extends PureComponent<Props> {
  ...
  onDocumentKeyDown: (arg0: KeyboardEvent) => void = event => {
    const {
      isOpen,
      onHeaderClick
    } = this.props;
  ...
  handleHeaderKeyDown: (arg0: React.KeyboardEvent<HTMLAnchorElement>) => void = e => {
  ...
  render(): React.ReactNode {
    ...
    return <li className={classNames('side-menu__submenu', {
      'side-menu__submenu--is-open': isOpen
    })}>
        <a className='side-menu__submenu_header' onKeyDown={this.handleHeaderKeyDown} onClick={this.handleHeaderClick} tabIndex={0}>
          <span>{header}</span>
          <BackIcon className='icon-small' />
        </a>
        <ul className='side-menu__submenu_items'>
          {items.map((item, index) => {
          return <Authorization allow={item.allow} key={index}>
