...
const SelectList = ({
  ...
  const renderItems = () => {
    return options.map((o, i) => <li className='multi-select__select-list-item' key={i}>
        <SelectItem focused={focusIndex === i} option={o} onSelectionChanged={c => handleSelectionChanged(o, c)} checked={selected.includes(o.value)} onClick={e => onClick(e, i)} ItemRenderer={ItemRenderer} disabled={disabled} />
      </li>);
  };

  return <ul className='multi-select__select-list'>
    {renderItems()}
  </ul>;
};
