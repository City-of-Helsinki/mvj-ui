...
function* createInvoiceNoteAndFetchListSaga({payload: {data, query}}): Generator<any, any, any> {
  try {
    const {response: {status: statusCode}, bodyAsJson} = yield call(createInvoiceNote, data);

    switch (statusCode) {
      case 201:
        yield put(hideCreateInvoiceNoteModal());
        displayUIMessage({title: '', body: 'Laskujen tiedote tallennettu'});
