...
import { formatDate, getApiResponseCount, getApiResponseResults, getApiResponseMaxPage, getSearchQuery, getUrlParams, isFieldAllowedToRead, isMethodAllowed } from "src/util/helpers";
...
const getColumns = (invoiceNoteAttributes: Attributes) => {
  ...
  if (isFieldAllowedToRead(invoiceNoteAttributes, InvoiceNoteFieldPaths.LEASE)) {
    columns.push({
      ...
      renderer: val => val ? <ExternalLink className='no-margin' href={`${getRouteById(Routes.LEASES)}/${val.id}?tab=6`} text={getContentLeaseIdentifier(val) || '-'} /> : '-'
    });
  }
  ...
  render() {
    ...
    return <PageContainer>
        <Authorization allow={isMethodAllowed(invoiceNoteMethods, Methods.POST)}>
          <CreateInvoiceNoteModal isOpen={isCreateModalOpen} onClose={this.hideCreateInvoiceNoteModal} onSubmit={this.createInvoiceNote} />
        </Authorization>
        <Row>
          <Column small={12} large={8}>
            <Authorization allow={isMethodAllowed(invoiceNoteMethods, Methods.POST)}>
              <AddButtonSecondary className='no-top-margin' label='Luo tiedote' onClick={this.showCreateInvoiceNoteModal} />
            </Authorization>
          </Column>
        ...
        <TableWrapper>
          ...
          <SortableTable columns={columns} data={invoiceNotes} listTable sortable={false} />
...
export default flowRight(withInvoiceNoteAttributes, withRouter, connect(state => {
  return {
