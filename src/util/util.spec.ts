...
describe("utils", () => {
  describe("date.js", () => {
    ...
    it("should return current year", () => {
      expect(getCurrentYear()).to.deep.equal(
    ...
    it("should return original ranges", () => {
      ...
      expect(splitDateRanges(a, b)).to.deep.equal([a, b]);
    });
    it("should return combined ranges", () => {
      ...
      expect(splitDateRanges(a, b)).to.deep.equal([a]);
    });
  ...
  describe("helpers.js", () => {
    it("Should return correct title", () => {
      expect(composePageTitle("foo")).to.deep.equal(
        "foo | Maanvuokrausjärjestelmä | Helsingin Kaupunki",
      );
      expect(composePageTitle()).to.deep.equal(
        "Maanvuokrausjärjestelmä | Helsingin Kaupunki",
      );
    ...
    it("Should return search query as string from an object", () => {
      ...
      expect(
        getUrlParams(
          "?foo3=123&foo4=false&foo5=test&foo6=1,2&foo6=3,4&foo6=5&foo7=test1&foo7=test2",
        ),
      ).to.deep.equal({
        foo3: "123",
        foo4: "false",
    ...
    it("should return value with thousand separator", () => {
      expect(formatNumberWithThousandSeparator("10000000.1234")).to.deep.equal(
        "10 000 000.1234",
      );
      expect(formatNumberWithThousandSeparator("10000000")).to.deep.equal(
        "10 000 000",
      );
      expect(formatNumberWithThousandSeparator(null)).to.deep.equal("");
      expect(
        formatNumberWithThousandSeparator("10000000,1234", "."),
      ).to.deep.equal("10.000.000,1234");
    });
    ...
    it("should covert string to a decimal number", () => {
      ...
      expect(convertStrToDecimalNumber("123 456,123")).to.deep.equal(
        123456.123,
      );
    });
    ...
    it("should format date  to a default form", () => {
      expect(formatDate("2019-05-09T15:26:54.889853")).to.deep.equal(
        "09.05.2019",
      );
    });
    it("should format date to a custom form", () => {
      expect(
        formatDate("2019-05-09T15:26:54.889853", "dd.MM.yyyy H.mm"),
      ).to.deep.equal("09.05.2019 15.26");
    });
    ...
    it("should format date range", () => {
      expect(
        formatDateRange(32345678912, "2019-05-09T15:26:54.889853"),
      ).to.deep.equal("10.01.1971–09.05.2019");
      expect(formatDateRange(null, "2019-05-09T15:26:54.889853")).to.deep.equal(
        "–09.05.2019",
      );
    ...
    it("should get reference number link", () => {
      expect(getReferenceNumberLink("123")).to.deep.equal(
        "https://paatokset.hel.fi/asia/123",
      );
      expect(getReferenceNumberLink("123 456")).to.deep.equal(
        "https://paatokset.hel.fi/asia/123-456",
      );
    ...
    it("should find item from collection by id", () => {
      ...
      expect(findItemById(items, 2)).to.deep.equal({
        id: 2,
      });
    });
    ...
    it("should sort numerical values in ascending order (1.)", () => {
      ...
      expect(
        items.sort((a, b) => sortNumberByKeyAsc(a, b, "value")),
      ).to.deep.equal(sortedItems);
    });
    it("should sort numerical values in ascending order (2.)", () => {
      ...
      expect(
        items.sort((a, b) => sortNumberByKeyAsc(a, b, "test")),
      ).to.deep.equal(items);
    });
    ...
    it("should sort numerical values in descending order (1.)", () => {
      ...
      expect(
        items.sort((a, b) => sortNumberByKeyDesc(a, b, "value")),
      ).to.deep.equal(sortedItems);
    });
    it("should sort numerical values in descending order (2.)", () => {
      ...
      expect(
        items.sort((a, b) => sortNumberByKeyDesc(a, b, "test")),
      ).to.deep.equal(items);
    });
    ...
    it("should sort string in ascending order", () => {
      const items = ["qwerty", "324", null, "ytrewq", "123"];
      const sortedItems = [null, "123", "324", "qwerty", "ytrewq"];
      expect(items.sort(sortStringAsc)).to.deep.equal(sortedItems);
    });
    it("should sort string in descending order", () => {
      const items = ["qwerty", "324", null, "ytrewq", "123"];
      const sortedItems = ["ytrewq", "qwerty", "324", "123", null];
    ...
    it("should sort object by a string in ascending order", () => {
      ...
      expect(
        items.sort((a, b) => sortStringByKeyAsc(a, b, "value")),
      ).to.deep.equal(sortedItems);
    });
    it("should sort object by a string in descending order", () => {
      ...
      expect(
        items.sort((a, b) => sortStringByKeyDesc(a, b, "value")),
      ).to.deep.equal(sortedItems);
    });
    it("should sort object by an option label in ascending order", () => {
      ...
      expect(
        items.sort((a, b) => sortByOptionsAsc(a, b, "value", options)),
      ).to.deep.equal(sortedItems);
    });
    it("should sort object by an option label in descending order", () => {
      ...
      expect(
        items.sort((a, b) => sortByOptionsDesc(a, b, "value", options)),
      ).to.deep.equal(sortedItems);
    });
    ...
    it("field should not be allowed to edit", () => {
      ...
      expect(isFieldAllowedToEdit(attributes, "field_not_found")).to.deep.equal(
        false,
      );
    });
    ...
    it("field should not be allowed to edit", () => {
      ...
      expect(isFieldAllowedToRead(attributes, "field_not_found")).to.deep.equal(
        false,
      );
    });
    ...
    it("should return field options with empty option", () => {
      ...
      expect(
        getFieldAttributeOptions(fieldAttributes, true, (item) => item.label),
    ...
    it("should return field options without empty option", () => {
      ...
      expect(getFieldAttributeOptions(fieldAttributes, false)).to.deep.equal(
        options,
      );
    });
    it("should return sorted field options without empty option", () => {
      ...
      expect(
        getFieldAttributeOptions(fieldAttributes, false, null, (a, b) =>
          sortNumberByKeyAsc(a, b, "value"),
        ),
      ).to.deep.equal(options);
    });
    ...
    it("should find property from ocd string", () => {
      expect(
        findFromOcdString("property1:test/property2:test2", "property2"),
      ).to.deep.equal("test2");
    });
    it("should not find property from ocd string", () => {
      expect(
        findFromOcdString("property1:test/property2:test2", "property3"),
      ).to.deep.equal(null);
    });
    it("should return PTP plan report url", () => {
      expect(createPTPPlanReportUrl("123")).to.deep.equal(
        "https://ptp.hel.fi/DataForms/planreport/?id=123",
      );
    });
    it("should return PTP plot division url", () => {
      expect(createPTPPlotDivisionUrl("123")).to.deep.equal(
        "https://ptp.hel.fi/DataHandlers/Tonttijaot/?q=123",
      );
    });
    ...
    it("should return api response results", () => {
      ...
      expect(getApiResponseResults(apiResponse)).to.deep.equal([
        { a: 1 },
        { b: 2 },
        { c: 3 },
      ]);
  ...
  describe("map.js", () => {
    ...
    const center = [60.1783025, 24.9513505];
