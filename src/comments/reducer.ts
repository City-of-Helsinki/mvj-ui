...
import type { CommentListMap, ReceiveAttributesAction, ReceiveMethodsAction, ReceiveCommentsByLeaseAction, HideEditModeByIdAction, ShowEditModeByIdAction, ReceiveIsSaveClickedAction } from "./types";
...
const isEditModeByIdReducer: Reducer<CommentListMap> = handleActions({
  ['mvj/comments/HIDE_BY_ID']: (state: Record<string, any>, {
    payload: id
  ...
  ['mvj/comments/SHOW_BY_ID']: (state: Record<string, any>, {
    payload: id
...
const attributesReducer: Reducer<Attributes> = handleActions({
  ['mvj/comments/RECEIVE_ATTRIBUTES']: (state: Attributes, {
    payload: attributes
...
const methodsReducer: Reducer<Methods> = handleActions({
  ['mvj/comments/RECEIVE_METHODS']: (state: Methods, {
    payload: methods
...
const byLeaseReducer: Reducer<CommentListMap> = handleActions({
  ['mvj/comments/RECEIVE_BY_LEASE']: (state: Record<string, any>, {
    payload: list
...
const isSaveClickedReducer: Reducer<boolean> = handleActions({
  ['mvj/comments/RECEIVE_SAVE_CLICKED']: (state: boolean, {
    payload: isClicked
