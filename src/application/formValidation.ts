...
export const personalIdentifierValidator = (value: any, error?: string): string | null | undefined => {
  if (value === '') {
    return;
  }
...
export const companyIdentifierValidator = (value: any, error?: string): string | null | undefined => {
  if (value === '') {
    return;
  }
...
export const emailValidator = (value: any, error?: string): string | null | undefined => {
  if (!value) {
    return;
  }
...
export const validateApplicationForm: (arg0: string) => (arg0: Record<string, any>) => Record<string, any> = (pathPrefix: string) => (values: Record<string, any>) => {
  let sum = 0;
  const errors = {};
  ...
  const searchSingleSection = (section, path) => {
    ...
    if (section.sections) {
      Object.keys(section.sections).map(identifier => searchSection(section.sections[identifier], `${path}.sections.${identifier}`));
    }
  };
  ...
  Object.keys(root.sections).map(identifier => searchSection(root.sections[identifier], `${pathPrefix}.sections.${identifier}`));
