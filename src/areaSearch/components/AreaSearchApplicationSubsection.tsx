...
type ApplicationFormFileFieldOwnProps = {
...
type ApplicationFormFileFieldProps = ApplicationFormFileFieldOwnProps & {
  ...
  deleteUploadedAttachment: (...args: any) => void;
  change: (...args: any) => void;
  pendingUploads: UploadedFileMeta[];
  existingUploads: UploadedFileMeta[];
...
type ApplicationFormSubsectionFieldArrayProps = {
  fields: any;
  section: FormSection;
  headerTag: string | React.ComponentType<{ children?: JSX.Element | string }>;
  formName: string;
  formPath: string;
  sectionTitleTransformers: Array<
    (arg0: string, arg1: FormSection, arg2: string) => string
...
const ApplicationFormFileField = connect(
  (state, props: ApplicationFormFileFieldOwnProps) => {
    ...
    return {
      ...
      attachmentIds: formValueSelector(formName)(
        state,
        (formPath ? `${formPath}.` : "") + "attachments",
      ),
    };
  },
  {
    ...
    <AppConsumer>
      {({ dispatch }) => {
        ...
        const addId = (newId: number) => {
          ...
          change(formName, (formPath ? `${formPath}.` : "") + "attachments", [
            ...attachmentIds,
            newId,
          ]);
        };
        ...
        const removeId = (newId: number) => {
          change(
            formName,
            `${fieldName}.value`,
            fieldFileIds.filter((id) => id !== newId),
          );
          change(
            formName,
            (formPath ? `${formPath}.` : "") + "attachments",
            attachmentIds.filter((id) => id !== newId),
          );
        };
        ...
        const uploads = (isNew ? pendingUploads : existingUploads).filter(
          (file) => fieldFileIds.includes(file.id),
        );
        ...
        return (
          <Column small={12} medium={12} large={12}>
            ...
            {!isFetchingAttachmentAttributes && uploads.length > 0 && (
              <Row>
                <Column small={6} large={5}>
                  <Authorization
                    allow={isFieldAllowedToRead(attachmentAttributes, "name")}
                  >
                    <FormTextTitle>Tiedoston nimi</FormTextTitle>
                  </Authorization>
                </Column>
                <Column small={3} large={3}>
                  ...
                  >
                    <FormTextTitle>{"Ladattu"}</FormTextTitle>
                  </Authorization>
                </Column>
              </Row>
            )}
            ...
            >
              {busy ? (
                ...
                <AddFileButton
                  ...
                  name={fieldName}
                  onChange={(e) => submitFile(field.id, e)}
                />
              )}
            </Authorization>
          </Column>
...
const ApplicationFormSubsectionFields = connect(
  (state, props: ApplicationFormSubsectionFieldsOwnProps) => ({
    ...
    <>
      <Row>
        {section.fields.map((field) => (
          <Fragment key={field.id}>{renderField(identifier, field)}</Fragment>
        ))}
      </Row>
    ...
    <div className="ApplicationFormSubsectionFieldArray">
      {fields.map((identifier, i) => {
        const subtitle: string = (sectionTitleTransformers || []).reduce(
          (title, transformer) => transformer(title, section, identifier),
          `${section.title} (${i + 1})`,
        );
        return (
          <div
      ...
      {section.identifier !== TARGET_SECTION_IDENTIFIER && (
        <AddButton
          onClick={() =>
            fields.push(
              getSectionTemplate(formName, formPath, section.identifier),
            )
          }
...
const AreaSearchApplicationSubsection = ({
  path,
  section,
...
export default AreaSearchApplicationSubsection;
