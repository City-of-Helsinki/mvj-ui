...
const methodsReducer: Reducer<Methods> = handleActions({
  ['mvj/areaSearch/RECEIVE_METHODS']: (state: Methods, {
    payload: methods
...
const listAttributesReducer: Reducer<Attributes> = handleActions({
  ['mvj/areaSearch/RECEIVE_LIST_ATTRIBUTES']: (state: Attributes, {
    payload: attributes
  }: ReceiveAttributesAction) => {
    return attributes;
  }
...
const listMethodsReducer: Reducer<Methods> = handleActions({
  ['mvj/areaSearch/RECEIVE_LIST_METHODS']: (state: Methods, {
    payload: methods
  }: ReceiveMethodsAction) => {
    return methods;
  }
...
const currentAreaSearchReducer: Reducer<Record<string, any> | null> = handleActions({
  ['mvj/areaSearch/RECEIVE_SINGLE']: (state: Record<string, any>, {
    payload: response
  }: ReceiveSingleAreaSearchAction) => {
    return response || null;
  },
  ['mvj/areaSearch/CREATE_SPECS']: () => null,
  ['mvj/areaSearch/RECEIVE_SPECS_CREATED']: (state, {
    payload: result
  }) => result,
  ['mvj/areaSearch/SET_ATTACHMENTS']: (state, {
    payload: attachments
  }) => {
    const {
...
const collapseStatesReducer: Reducer<Record<string, any>> = handleActions({
  ['mvj/areaSearch/RECEIVE_COLLAPSE_STATES']: (state: Record<string, any>, {
    payload: states
...
const isSaveClickedReducer: Reducer<boolean> = handleActions({
  ['mvj/areaSearch/RECEIVE_SAVE_CLICKED']: (state: boolean, {
    payload: isClicked
...
const isFormValidByIdReducer: Reducer<Record<string, any>> = handleActions({
  ['mvj/areaSearch/RECEIVE_FORM_VALID_FLAGS']: (state: Record<string, any>, {
    payload: valid
...
const lastAreaSearchEditErrorReducer: Reducer<any> = handleActions({
  ...
  ['mvj/areaSearch/RECEIVE_EDIT_FAILED']: (state, {
    payload: error
  }: ReceiveAreaSearchEditFailedAction) => error
}, null);
