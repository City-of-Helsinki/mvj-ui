...
function* fetchCompanyNoticeByIdSaga({payload: businessId}): Generator<any, any, any> {
  try {
    const {response: {status: statusCode}, bodyAsJson} = yield call(fetchCompanyNotice, businessId);

    switch (statusCode) {
      case 200:
        yield put(receiveTradeRegisterCompanyNoticeById({[businessId.toString()]: bodyAsJson.companyNoticeInfoResponseTypeDetails}));
        break;
      default:
...
function* fetchCompanyRepresentByIdSaga({payload: businessId}): Generator<any, any, any> {
  try {
