...
const attributesReducer: Reducer<Attributes> = handleActions(
  {
    ["mvj/contacts/RECEIVE_ATTRIBUTES"]: (
      state: Attributes,
...
const contactsListReducer: Reducer<ContactList> = handleActions(
  {
    ["mvj/contacts/RECEIVE_ALL"]: (
      state: ContactList,
      { payload: contacts }: ReceiveContactsAction,
    ) => {
      return contacts;
    },
  },
...
const contactModalSettingsReducer: Reducer<ContactModalSettings> =
  handleActions(
    {
      ["mvj/contacts/RECEIVE_CONTACT_SETTINGS"]: (
        state: ContactModalSettings,
        { payload: settings }: ReceiveContactModalSettingsAction,
      ) => {
        return settings;
      },
    },
...
const contactReducer: Reducer<Contact> = handleActions(
  {
    ["mvj/contacts/RECEIVE_SINGLE"]: (
      state: Contact,
      { payload: contact }: ReceiveSingleContactAction,
    ) => {
      return contact;
    },
  },
...
const initialValuesReducer: Reducer<Contact> = handleActions(
  {
    ["mvj/contacts/INITIALIZE_FORM"]: (
      state: Contact,
      { payload: contact }: InitializeContactFormValuesAction,
    ) => {
      return contact;
    },
  },
...
const isContactFormValidReducer: Reducer<boolean> = handleActions(
  {
    ["mvj/contacts/RECEIVE_CONTACT_FORM_VALID"]: (
      state: boolean,
      { payload: valid }: ReceiveContactFormValidAction,
    ) => {
      return valid;
    },
  },
...
const isSaveClickedReducer: Reducer<boolean> = handleActions(
  {
    ["mvj/contacts/RECEIVE_SAVE_CLICKED"]: (
      state: boolean,
