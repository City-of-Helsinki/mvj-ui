...
import { fetchContacts, initializeContactForm } from "../actions";
...
import { getApiResponseCount, getApiResponseMaxPage, getApiResponseResults, getFieldOptions, getLabelOfOption, getSearchQuery, getUrlParams, isFieldAllowedToRead, isMethodAllowed, setPageTitle } from "src/util/helpers";
...
class ContactListPage extends Component<Props, State> {
  ...
  getColumns = () => {
    ...
    if (isFieldAllowedToRead(contactAttributes, ContactFieldPaths.FIRST_NAME) || isFieldAllowedToRead(contactAttributes, ContactFieldPaths.LAST_NAME) || isFieldAllowedToRead(contactAttributes, ContactFieldPaths.NAME)) {
      columns.push({
        key: 'names',
        text: 'Nimi',
  ...
  render() {
    ...
    return <PageContainer>
        <Row>
          <Column small={12} large={4}>
            <Authorization allow={isMethodAllowed(contactMethods, Methods.POST)}>
              <AddButtonSecondary className='no-top-margin' label='Luo asiakas' onClick={this.handleCreateButtonClick} />
            </Authorization>
          </Column>
          <Column small={12} large={8}>
            {userActiveServiceUnit && <Search isSearchInitialized={isSearchInitialized} onSearch={this.handleSearchChange} sortKey={sortKey} sortOrder={sortOrder} />}
        ...
        <TableWrapper>
          ...
          <SortableTable columns={columns} data={contacts} listTable onRowClick={this.handleRowClick} onSortingChange={this.handleSortingChange} serverSideSorting sortable sortKey={sortKey} sortOrder={sortOrder} />
...
export default flowRight(withContactAttributes, withRouter, connect((state: RootState) => {
  return {
