...
export const fetchContacts = async(query?: Object) => {
  const {response: {status}, bodyAsJson} = await callApiAsync(new Request(createUrl('contact/', query)));
...
export const contactExists = async(identifier: string) => {
  const {response: {status}, bodyAsJson} = await callApiAsync(new Request(createUrl(`contact_exists/?identifier=${identifier}`)));
