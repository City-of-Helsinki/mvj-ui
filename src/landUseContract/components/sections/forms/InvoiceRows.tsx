...
import { InvoiceRowsFieldPaths, InvoiceRowsFieldTitles } from "@/invoices/enums";
...
const InvoiceRows = ({
  ...
  return <Row>
      <Column small={12}>
        ...
        {!!rows.length && <BoxItemContainer>
            {rows.map(row => {
          const contact = get(getContentTenant(row.tenantFull), 'contact');
          return <BoxItem key={row.id} className='no-border-on-first-child no-border-on-last-child'>
                  <Row>
                    <Column small={4}>
