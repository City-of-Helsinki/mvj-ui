...
const Conditions = ({
  ...
  const formOfManagementOptions = getFieldOptions(
    attributes,
  ...
  return (
    <Fragment>
      ...
      <Collapse
        defaultOpen={
          ConditionsCollapseState !== undefined ? ConditionsCollapseState : true
        }
          ...
          conditions.map((condition, index) => (
            <Row key={index}>
              <Column small={6} medium={4} large={2}>
                ...
                <FormText>
                  {getLabelOfOption(
                    formOfManagementOptions,
                    condition.form_of_management,
                  ) || "-"}
                </FormText>
              </Column>
              <Column small={6} medium={4} large={2}>
  ...
  (state) => {
    return {
      ...
      ConditionsCollapseState: getCollapseStateByKey(
        state,
        `${ViewModes.READONLY}.${FormNames.LAND_USE_CONTRACT_CONDITIONS}.conditions`,
      ),
