...
import { getInvoicesByLandUseContractId, getIsFetchingAttributes as getIsFetchingLandUseInvoiceAttributes, getAttributes as getLandUseInvoiceAttributes } from "@/landUseInvoices/selectors";
import { clearFormValidFlags, editLandUseContract, fetchSingleLandUseContract, hideEditMode, receiveFormValidFlags, receiveIsSaveClicked, receiveSingleLandUseContract, showEditMode, deleteLandUseContract } from "@/landUseContract/actions";
...
import { getCurrentLandUseContract, getIsEditMode, getIsFormValidById, getIsFormValidFlags, getIsSaveClicked, getIsFetching } from "@/landUseContract/selectors";
...
class LandUseContractPage extends Component<Props, State> {
    ...
    if (areFormsValid) {
      ...
      if (isBasicInformationFormDirty) {
        if (basicInformationFormValues.estate_ids) {
          const plotsFromEstate = { ...basicInformationFormValues,
      ...
      if (isCompensationsFormDirty) {
        ...
        payload = { ...payload,
          ...convertedCompensationsFormValues
        };
      }
  ...
  getAreFormsValid = () => {
    ...
    return isBasicInformationFormValid && isCompensationsFormValid && isContractsFormValid && isDecisionsFormValid && isInvoicesFormValid && isLitigantsFormValid && isConditionsFormValid;
  };
  isAnyFormDirty = () => {
    const {
      isBasicInformationFormDirty,
    ...
    return isBasicInformationFormDirty || isCompensationsFormDirty || isContractsFormDirty || isDecisionsFormDirty || isInvoicesFormDirty || isLitigantsFormDirty || isConditionsFormDirty;
  };
  ...
  render() {
    ...
    return <FullWidthContainer>
        <PageNavigationWrapper>
          <ControlButtonBar buttonComponent={<ControlButtons allowEdit={true} isCancelDisabled={false} isCopyDisabled={true} isEditDisabled={false} isEditMode={isEditMode} isSaveDisabled={isSaveClicked && !areFormsValid} onCancel={this.cancelChanges} onEdit={this.handleShowEditMode} onSave={this.saveChanges} showCommentButton={false} showCopyButton={false} onDelete={this.handleDelete} deleteModalTexts={{
        ...
        }} allowDelete={true} />} infoComponent={<h1>{identifier}</h1>} onBack={this.handleBack} />
          ...
          hasError: isSaveClicked && (!isDecisionsFormValid || !isContractsFormValid)
        }, {
          label: 'Korvaukset',
          allow: true,
...
export default flowRight(withRouter, withLandUseContractAttributes, withUiDataList, connect((state, props: Props) => {
  return {
