...
const methodsReducer: Reducer<Methods> = handleActions({
  ['mvj/landUseContract/RECEIVE_METHODS']: (state: Methods, {
    payload: methods
...
const landUseContractListReducer: Reducer<LandUseContractList> = handleActions({
  ['mvj/landUseContract/RECEIVE_ALL']: (state: LandUseContractList, {
    payload: list
  }: ReceiveLandUseContractListAction) => list
}, {});
const currentLandUseContractReducer: Reducer<LandUseContract> = handleActions({
  ['mvj/landUseContract/RECEIVE_SINGLE']: (state: LandUseContract, {
    payload: contract
  }: ReceiveSingleLandUseContractAction) => contract
}, {});
const isFormValidByIdReducer: Reducer<Record<string, any>> = handleActions({
  ['mvj/landUseContract/RECEIVE_FORM_VALID_FLAGS']: (state: Record<string, any>, {
    payload: valid
...
const isSaveClickedReducer: Reducer<boolean> = handleActions({
  ['mvj/landUseContract/RECEIVE_SAVE_CLICKED']: (state: boolean, {
    payload: isClicked
...
const collapseStatesReducer: Reducer<Record<string, any>> = handleActions({
  ['mvj/landUseContract/RECEIVE_COLLAPSE_STATES']: (state: Record<string, any>, {
    payload: states
