...
function* fetchLandUseContractsSaga({payload: search}): Generator<any, any, any> {
  try {
    const {response: {status: statusCode}, bodyAsJson} = yield call(fetchLandUseContracts, search);
...
function* editLandUseContractSaga({payload: landUseContract}): Generator<any, any, any> {
  try {
    const {response: {status: statusCode}, bodyAsJson} = yield call(editLandUseContract, landUseContract);
...
function* fetchSingleLandUseContractAfterEditSaga({payload}): Generator<any, any, any> {
  try {
    const callbackFunctions = payload.callbackFunctions;
    const {response: {status: statusCode}, bodyAsJson} = yield call(fetchSingleLandUseContract, payload.id);
...
function* deleteLandUseSaga({payload: id}): Generator<any, any, any> {
  try {
    const {response: {status: statusCode}, bodyAsJson} = yield call(deleteLandUseContract, id);
    ...
    switch (statusCode) {
      case 204:
        ...
        displayUIMessage({title: '', body: 'Maankäyttösopimus poistettu'});
        break;
      case 400:
      case 401:
        yield put(notFound());
