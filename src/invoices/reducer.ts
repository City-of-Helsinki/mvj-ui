...
const isCreatePanelOpenReducer: Reducer<boolean> = handleActions({
  ['mvj/invoices/RECEIVE_IS_CREATE_PANEL_OPEN']: (state: boolean, {
    payload: isOpen
  }: ReceiveIsCreateInvoicePanelOpenAction) => {
    return isOpen;
  }
...
const isCreditPanelOpenReducer: Reducer<boolean> = handleActions({
  ['mvj/invoices/RECEIVE_IS_CREDIT_PANEL_OPEN']: (state: boolean, {
    payload: isOpen
...
const isCreateClickedReducer: Reducer<boolean> = handleActions({
  ['mvj/invoices/RECEIVE_CREATE_CLICKED']: (state: boolean, {
    payload: isClicked
...
const isCreditClickedReducer: Reducer<boolean> = handleActions({
  ['mvj/invoices/RECEIVE_CREDIT_CLICKED']: (state: boolean, {
    payload: isClicked
...
const isEditClickedReducer: Reducer<boolean> = handleActions({
  ['mvj/invoices/RECEIVE_EDIT_CLICKED']: (state: boolean, {
    payload: isClicked
...
const attributesReducer: Reducer<Attributes> = handleActions({
  ['mvj/invoices/RECEIVE_ATTRIBUTES']: (state: Attributes, {
    payload: attributes
...
const methodsReducer: Reducer<Methods> = handleActions({
  ['mvj/invoices/RECEIVE_METHODS']: (state: Methods, {
    payload: methods
...
const byLeaseReducer: Reducer<InvoiceListMap> = handleActions({
  ['mvj/invoices/RECEIVE_BY_LEASE']: (state: InvoiceListMap, {
    payload
...
const invoiceToCreditReducer: Reducer<string | null | undefined> = handleActions({
  ['mvj/invoices/RECEIVE_INVOICE_TO_CREDIT']: (state: string | null | undefined, {
    payload: invoiceId
  }: ReceiveInvoiceToCreditAction) => {
    return invoiceId;
  }
}, null);
const patchedInvoiceReducer: Reducer<Invoice | null | undefined> = handleActions({
  ['mvj/invoices/RECEIVE_PATCHED']: (state: Invoice | null | undefined, {
    payload: invoice
