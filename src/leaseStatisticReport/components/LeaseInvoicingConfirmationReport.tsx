...
import { getDisplayName, getFormattedValue, getOutputFields, getReportTypeOptions } from "@/leaseStatisticReport/helpers";
import { getIsFetchingLeaseInvoicingConfirmationReport, getLeaseInvoicingConfirmationReport, getPayload } from "@/leaseStatisticReport/selectors";
    ...
    outputFields.map(field => {
      columns.push({
        ...
        renderer: value => {
          ...
          return <FormText className='no-margin' style={{
            whiteSpace: 'nowrap'
          }}>
  ...
  render() {
    ...
    return <Fragment>
        <Row>
          ...
          {payload && <ExcelLink fileName='Raportti' identifier={getLabelOfOption(reportTypeOptions, payload.report_type)} url={payload.url} query={payload.query} label='VIE EXCELIIN' />}
        </Row>
        <SortableTable columns={columns} data={reportData} style={{
        marginBottom: 10
      }} defaultSortKey='lease_id' defaultSortOrder={TableSortOrder.ASCENDING} sortable={isSortable} />
      </Fragment>;
  }

}

export default flowRight(withLeaseInvoicingConfirmationReportAttributes, connect(state => {
  return {
