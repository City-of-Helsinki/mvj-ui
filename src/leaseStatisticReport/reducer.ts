...
import type { ReceiveAttributesAction, ReceiveLeaseInvoicingConfirmationReportAttributesAction, ReceiveLeaseInvoicingConfrimationReportsAction, ReceiveReportsAction, ReceiveReportDataAction, SetOptionsAction, SetPayloadAction, ReceiveOptionsAction } from "./types";
...
const attributesReducer: Reducer<Attributes> = handleActions({
  ['mvj/leaseStatisticReport/RECEIVE_ATTRIBUTES']: (state: Attributes, {
    payload: attributes
...
const leaseInvoicingConfirmationReportAttributesReducer: Reducer<Attributes> = handleActions({
  ['mvj/leaseStatisticReport/RECEIVE_LEASE_INVOICING_CONFIRMATION_REPORT_ATTRIBUTES']: (state: Attributes, {
    payload: attributes
...
const leaseInvoicingConfirmationReportReducer: Reducer<Attributes> = handleActions({
  ['mvj/leaseStatisticReport/RECEIVE_LEASE_INVOICING_CONFIRMATION_REPORTS']: (state: ApiResponse, {
    payload
...
const reportsReducer: Reducer<Attributes> = handleActions({
  ['mvj/leaseStatisticReport/RECEIVE_REPORTS']: (state: Reports, {
    payload: reports
...
const reportDataReducer: Reducer<Record<string, any>> = handleActions({
  ['mvj/leaseStatisticReport/RECEIVE_REPORT_DATA']: (state: Reports, {
    payload: reportData
