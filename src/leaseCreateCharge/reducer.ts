...
const attributesReducer: Reducer<Attributes> = handleActions(
  {
    ["mvj/leaseCreateCharge/RECEIVE_ATTRIBUTES"]: (
      state: Attributes,
...
const ReceivableTypesReducer: Reducer<Record<string, any>> = handleActions(
  {
    ["mvj/leaseCreateCharge/RECEIVE_RECEIVABLE_TYPES"]: (
      state: Record<string, any>,
      { payload: receivableTypes }: ReceiveReceivableTypesAction,
    ) => {
